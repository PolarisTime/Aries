...
test('debug sales order imported row columns', async ({ page }) => {
  ...
  const purchaseOrderNo = `PO-SOC-DBG-${suffix}`
  const salesOrderNo = `SO-COL-DBG-${suffix}`
  ...
  const headerTexts = await salesOrderOverlay
    .locator('.module-detail-table thead th')
    .allTextContents()
  const spinValues = await salesOrderOverlay
    .locator('.module-detail-table tbody tr:not(.ant-table-measure-row)')
    .first()
  ...
  const rowTexts = await salesOrderOverlay
    .locator('.module-detail-table tbody tr:not(.ant-table-measure-row)')
    .first()
    .locator('td')
    .allTextContents()
