...
test.describe('global search coverage', () => {
  test('opens purchase order detail from header search', async ({
    page,
    assertNoFatalUiErrors,
  }) => {
    const collection = await fetchCollection(page.request, 'purchase-order')
    ...
    if (!collection.ok || collection.records.length === 0) {
      await expect(
        page.getByRole('combobox', { name: '搜索单号、合同号、对账单号' }),
      ).toBeVisible()
