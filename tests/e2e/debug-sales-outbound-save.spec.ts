...
test('debug sales outbound save result', async ({ page }) => {
  ...
  const purchaseOrderNo = `PO-SOB-DBG-${suffix}`
  const salesOrderNo = `SO-SOB-DBG-${suffix}`
  const salesOutboundNo = `SOB-DBG-${suffix}`
  ...
  await purchaseOrderOverlay
    ...
    .filter({ hasText: /^保存$/ })
    .click()
  await waitForSaveOutcome(page, purchaseOrderOverlay, purchaseOrderNo)
  ...
  await salesOrderOverlay.locator('#salesName').fill('test9')
  ...
  const salesOrderRow = salesOrderOverlay
    .locator('.module-detail-table tbody tr:not(.ant-table-measure-row)')
    .first()
  ...
  await salesOrderOverlay
    .locator('button.overlay-action-button')
    .filter({ hasText: /^保存$/ })
    .click()
  await waitForSaveOutcome(page, salesOrderOverlay, salesOrderNo)
  ...
  const responsePromise = page
    .waitForResponse(
      (response) =>
        response.url().includes('/api/sales-outbound') &&
