...
test.describe('system route coverage', () => {
  test.beforeEach(async ({ page }) => {
    await primeApiKeySession(page)
  })

  for (const route of systemRoutes) {
    test(`loads ${route.title} at ${route.path}`, async ({
      page,
  ...
  test('loads API Key detail route for a live record', async ({
    page,
    assertNoFatalUiErrors,
  }) => {
