...
test('debug sales order save result', async ({ page }) => {
  ...
  const purchaseOrderNo = `PO-SO-DBG-${suffix}`
  const salesOrderNo = `SO-DBG-${suffix}`
  ...
  const responsePromise = page
    .waitForResponse(
      (response) =>
        response.url().includes('/api/sales-order') &&
