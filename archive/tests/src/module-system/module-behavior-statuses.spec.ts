...
describe('module-behavior-statuses', () => {
  ...
  it('limits sales order audit sources to draft status', () => {
    expect(mockedRegister).toHaveBeenCalledWith('sales-order', {
      auditSourceStatuses: ['草稿'],
    })
  })

  it('does not register legacy pre-outbound audit sources', () => {
    const calls = mockedRegister.mock.calls.filter(
      ([key]) => key === 'sales-outbound',
    )
    expect(
      calls.some(
        ([, config]) =>
  ...
  it('registers auditStatus for receipt', () => {
    ...
    expect(
      receiptCalls.some(([, config]) => config.auditStatus === '已审核'),
    ).toBe(true)
  })
  ...
  it('registers auditStatus for payment', () => {
    ...
    expect(
      paymentCalls.some(([, config]) => config.auditStatus === '已审核'),
    ).toBe(true)
  })

  it('registers auditStatus for cash reversal', () => {
    const calls = mockedRegister.mock.calls.filter(
      ([key]) => key === 'cash-reversal',
    )
    expect(calls.some(([, config]) => config.auditStatus === '已审核')).toBe(
      true,
    )
  })
  ...
  describe('protectedEditStatuses', () => {
    ...
    it('contains expected statuses', () => {
      ...
      expect(protectedEditStatuses).toContain('交付核定')
  ...
  describe('protectedDeleteStatuses', () => {
    ...
    it('contains expected statuses', () => {
      ...
      expect(protectedDeleteStatuses).toContain('交付核定')
