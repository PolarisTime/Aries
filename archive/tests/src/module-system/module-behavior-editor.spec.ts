...
describe('module-behavior-editor', () => {
  ...
  it('keeps generated purchase refunds read-only and non-deletable', () => {
    ...
    expect(config?.protectedEditStatuses).toEqual(['草稿', '已审核'])
    expect(config?.protectedDeleteStatuses).toEqual(['草稿', '已审核'])
  })
  ...
  it('keeps the legacy supplier refund receipt operator metadata only', () => {
    const config = moduleBehaviorRegistry.get('supplier-refund-receipt')

    expect(config?.defaultOperatorField).toBe('operatorName')
    expect(config?.defaultDraftValues).toBeUndefined()
  })
  ...
  it('sales-order defaultDraftValues returns deliveryDate for today', () => {
    ...
    expect(values.salesMode).toBe('NORMAL')
    expect(config?.clearLineItemsOnFieldChange).toEqual(['salesMode'])
    expect(config?.clearEditorFieldsOnFieldChange).toEqual({
      salesMode: ['purchaseInboundNo', 'purchaseOrderNo'],
    })
  })

  it('sales-outbound only keeps outbound date, remark and actual weight editable after generation', () => {
    const config = moduleBehaviorRegistry.get('sales-outbound')
    ...
    expect(config?.parentImportedItemEditableColumns).toEqual([
      'actualWeightTon',
      'weighWeightTon',
    ])
  })
  ...
  it('cash reversal source changes clear every stale source snapshot', () => {
    const form: Record<string, unknown> = {
      sourceType: '收款单',
      sourceDocumentNo: 'PAY001',
      originalPaymentId: '101',
      originalReceiptId: '102',
      counterpartyType: '供应商',
      counterpartyId: '201',
      counterpartyCode: 'SUP001',
      counterpartyName: '旧供应商',
      settlementCompanyId: '301',
      settlementCompanyName: '旧主体',
      amount: 100,
    }

    getSyncEditorForm('cash-reversal')(form, {
      changedKeys: new Set(['sourceType']),
    })
  ...
  it('registers purchase-inbound with readonlyItemColumns', () => {
    ...
    expect(config?.allowsManualLineItems).toBe(false)
    expect(config?.readonlyItemColumns).toContain('warehouseName')
    expect(config?.readonlyItemColumns).not.toContain('quantity')
