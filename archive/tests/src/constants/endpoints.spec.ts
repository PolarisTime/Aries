...
describe('ENDPOINTS', () => {
  ...
  it('has all required import candidate endpoints', () => {
    ...
    expect(ENDPOINTS.SALES_ORDER_PURCHASE_SOURCE_CANDIDATES).toBe(
      '/sales-orders/source-candidates',
    )
    expect(ENDPOINTS.SALES_ORDER_FREIGHT_IMPORT_CANDIDATES).toBe(
  ...
  it('has purchase finance flow query and export endpoints', () => {
    expect(ENDPOINTS.PURCHASE_FINANCE_DOCUMENT_FLOW).toBe(
      '/purchase-finance/document-flow',
    )
    expect(ENDPOINTS.PURCHASE_FINANCE_DOCUMENT_FLOW_EXPORT).toBe(
  ...
  it('has all expected keys', () => {
    const expectedKeys = [
      ...
      'SALES_ORDER_PURCHASE_SOURCE_CANDIDATES',
