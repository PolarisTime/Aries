...
describe('paymentPageConfigs', () => {
  it('contains receipt, payment, and cash-reversal', () => {
    const keys = Object.keys(paymentPageConfigs)
    expect(keys).toContain('receipt')
    expect(keys).toContain('payment')
    expect(keys).toContain('cash-reversal')
    expect(keys).not.toContain('supplier-refund-receipt')
    expect(keys).not.toContain('ledger-adjustment')
  })
