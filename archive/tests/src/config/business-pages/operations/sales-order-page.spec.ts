...
describe('salesOrdersPageConfig', () => {
  ...
  it('has parentImport config', () => {
    ...
    expect(
      ...
      currentSalesOrderId: '700520000000000099',
      salesMode: 'NORMAL',
    })
    expect(
  ...
  it('maps parent purchase order number into a sales order draft', () => {
    ...
    expect(
      ...
      purchaseInboundNo: 'PI-001, PI-002',
    })
    expect(
      parentImport.mapParentToDraft?.({
        purchaseOrderNo: 'PO-002',
        salesMode: 'PRESALE',
        items: [{ inboundNo: 'PI-003' }],
      }),
    ).toEqual({
      purchaseOrderNo: 'PO-002',
      purchaseInboundNo: '',
  ...
  it('requires sales mode before opening purchase source selector', () => {
    const parentImport = salesOrdersPageConfig.parentImport!

    expect(parentImport.validateBeforeOpen?.({ salesMode: '' })).toContain(
      '销售模式',
    )
    expect(
      parentImport.validateBeforeOpen?.({ salesMode: 'NORMAL' }),
    ).toBeNull()
  })

  it('transforms normal sales candidates into inbound source lines', () => {
    const items = salesOrdersPageConfig.parentImport!.transformItems?.({
      salesMode: 'NORMAL',
      items: [
        {
          id: 'inbound-item-1',
          sourcePurchaseOrderItemId: 'purchase-item-1',
          remainingQuantity: 2,
          quantity: 3,
          weightTon: 1.5,
          pieceWeightTon: 0.5,
    ...
    expect(items?.[0].sourcePurchaseOrderItemId).toBeUndefined()
  })
