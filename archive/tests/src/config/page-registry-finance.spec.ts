...
describe('page-registry-finance', () => {
  ...
  it('defines cash reversal page and removes legacy cash entries', () => {
    const page = financePageDefinitions.find((d) => d.key === 'cash-reversal')
    expect(page).toBeDefined()
    expect(page!.title).toBe('资金冲销单')
    expect(page!.menuParent).toBe('finance')
    expect(page!.moduleKey).toBe('cash-reversal')
    expect(page!.searchable).toBe(true)
    expect(page!.icon).toBe('RollbackOutlined')
    expect(
      financePageDefinitions.some((d) => d.key === 'supplier-refund-receipt'),
    ).toBe(false)
    expect(
      financePageDefinitions.some((d) => d.key === 'ledger-adjustment'),
    ).toBe(false)
  })
  ...
  it('defines the purchase finance document flow as a receivable-payable alias', () => {
    const page = financePageDefinitions.find(
      (d) => d.key === 'purchase-finance-flow',
    )
