...
function renderHook<Result, Props>(
  ...
  return testingLibraryRenderHook(callback, { ...options, wrapper: Wrapper })
}
...
describe('useBusinessGridPage', () => {
  ...
  it('does not expose allocation actions for an audited purchase prepayment', async () => {
    const payment = {
      id: 'payment-1',
    ...
    expect(
      result.current.visibleToolbarActions.some((action) =>
        action.key?.includes('allocation'),
      ),
    ).toBe(false)
  })
