...
describe('ModuleEditorFormSection', () => {
  ...
  it('keeps the parent selector visible when finance actions use the overlay footer', () => {
    render(
      <ModuleEditorFormSection
        {...defaultProps}
        showActions={false}
        ...
        config={{
          ...defaultProps.config,
          formFields: [createField({ key: 'sourceType' })],
          parentImport: {
    ...
    expect(screen.getByRole('button', { name: '选择原资金单据' })).toBeTruthy()
