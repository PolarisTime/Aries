...
vi.mock('@/module-system/module-adapter-parent-import', () => ({
  ...
  resolveParentImportDefinition: vi.fn((definition) => definition),
}))
...
describe('useModuleEditorWorkspace', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveParentImportDefinition).mockImplementation(
  ...
  it('shows the backend audit error without legacy pre-outbound guidance', async () => {
    const errorMessage =
  ...
  it('keeps the resolved dynamic parent selector for the active session', () => {
    const parentImport = {
      parentModuleKey: 'payment',
      label: '原资金单据',
      parentFieldKey: 'sourceDocumentNo',
      parentDisplayFieldKey: 'paymentNo',
      resolveParentSelector: vi.fn(),
    }
    vi.mocked(resolveParentImportDefinition).mockReturnValue({
      ...parentImport,
      parentModuleKey: 'receipt',
      parentDisplayFieldKey: 'receiptNo',
    })
    const { result } = renderWorkspace({
      config: cfg({ parentImport }),
    })
    ...
    expect(result.current.parentSelectorOpen).toBe(true)
    expect(result.current.parentSelectorModuleKey).toBe('receipt')
    expect(result.current.parentSelectorDisplayFieldKey).toBe('receiptNo')
  })
