...
describe('BusinessGridRouteContent', () => {
  ...
  it('does not create records when manual creation is disabled', () => {
    const state = createGridState({
      config: { readOnly: false, allowManualCreate: false },
    })
    mocks.useBusinessGridPage.mockReturnValue(state)
    ...
    mocks.gridContentProps?.onCreate()
    expect(state.openEditor).not.toHaveBeenCalled()
    expect(mocks.gridOverlayProps?.canSave).toBe(false)
  })
