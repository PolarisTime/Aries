...
usage() {
  cat <<'EOF'
用法:
  bash aries/scripts/deploy/trigger-production-deploy.sh [选项]

选项:
  --aries-ref <ref>       前端部署 ref，默认使用 aries 当前分支
  --workflow-ref <ref>    触发 workflow 所在 ref，默认 main
  --repo <owner/repo>     GitHub 仓库，默认 PolarisTime/Aries
...
示例:
  bash aries/scripts/deploy/trigger-production-deploy.sh --dry-run
  bash aries/scripts/deploy/trigger-production-deploy.sh --confirm-production --aries-ref main --watch
  bash aries/scripts/deploy/trigger-production-deploy.sh --confirm-production --deploy-target ssh --aries-ref main --watch
EOF
}
...
本脚本会触发 GitHub Actions 的 frontend-production-deploy 工作流，并在 dry_run=false 时发布前端到指定生产目标。
