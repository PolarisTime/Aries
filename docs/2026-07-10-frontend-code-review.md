...
- `pnpm typecheck`
- `pnpm lint`：串行执行 Biome 与 ESLint；Biome 检查 995 个文件，两层检查均无错误
- HTTP/认证相关定向测试：153 项通过
- 本轮后端联动定向测试：14 个测试文件、335 项测试通过
- `pnpm test:unit`：448 个测试文件通过、1 个跳过；5921 个测试通过、4 个跳过
- `pnpm build`：TypeScript 检查和生产构建均成功
...
- 后端联动新增涉及的 4 个 Ant Design 生产文件再次逐文件执行 `antd lint --format json`，均为 0 issue
...
原始审查中的 4 个 ESLint error 已修复：附件存储类型移除退化联合类型，待收票 E2E 移除无效赋值，E2E 覆盖率工具移除不必要的类型断言。

`pnpm lint` 已改为通过 `run-s` 串行执行 `lint:biome` 与 `eslint`。现有 CI 的 Lint job 无需增加重复步骤即可同时执行两套规则，任一失败都会阻断任务。
...
| 3.4 软删除父单 | 已完成（主要来源链路） | 前端候选过滤首先排除软删除，详情解析后及 `onSelect` 前再次校验；后端已在采购入库、销售订单、销售出库、物流单、开收票及三类对账单的创建/更新事务内锁定并重新校验来源 | 采购合同仍是快照导入，未持久化来源明细关系；销售出库候选查询还应显式排除软删除记录 |
...
| 3.8 父单分页过滤 | 部分完成 | 采购订单、销售订单出库、物流单及三类对账单已有专用候选分页接口，前端按接口返回的过滤后 `total` 展示；供应商对账状态已与后端统一为 `INBOUND_COMPLETED` | 开票、收票和采购合同仍使用通用列表；采购订单与销售订单候选仍有服务端内存分页的规模风险 |
...
| 后端变化 | 前端处理 | 状态与边界 |
|---|---|---|
| OOBE 写接口要求 `X-Setup-Token` | 初始化向导在两个步骤均采集凭证，三个现用写请求只通过 Header 传递；凭证不进入请求体、Zustand 或 Web Storage | 已适配 |
| 供应商对账来源状态改为 `INBOUND_COMPLETED` | 候选过滤和导入前校验统一为“完成入库”，移除候选接口不接收的 `status` 参数 | 已适配 |
| 库存响应区分 `onHand/reserved/available` | 主表展示三类数量和三类重量，概览分别汇总三类重量；流水明细继续使用自身的 `quantity/weightTon` | 页面已适配；后端 Excel 导出仍只包含现存量 |
| 修改密码递增凭据版本并撤销会话 | 修改成功后通过统一 Auth Store 清理本地身份并跳转登录页 | 已适配 |
| 核心聚合乐观锁冲突返回业务码 `4090` | HTTP 规范化错误保留 status、code 与 traceId；编辑保存冲突后清理草稿、刷新查询并退出陈旧编辑会话 | 已适配；后端尚未提供客户端 `expectedVersion/ETag` 协议 |
| 待收票聚合和分页下推数据库 | 列表字段无需变化；移除后端不存在的导出动作，避免固定 404 | 分页已适配；导出需后端提供正式端点后再恢复 |

本轮已完成可由现有后端契约支撑的前端联动。剩余后端边界包括：附件原子追加/移除或集合版本协议、打印模板保存侧严格白名单、开票和收票专用候选分页、采购合同来源关系设计、库存三态导出，以及客户端版本协商协议。

本轮没有修改后端、数据库或本地测试 API Key，也没有执行提交或推送。

## 9. 财务专项联动实施状态（2026-07-11）

本节是原代码审查之后的增量记录。前端财务闭环已随 `feat(frontend): 统一业务单据交互并补齐退款与预付款流程` 落地；
后端对应代码和 V12 至 V18 迁移仍处于最终验证与部署准备阶段。完整财务事件模型和上线风险记录在
`leo/docs/2026-07-11-finance-audit.md`。

| 联动范围 | 前端实现 | 契约边界 | 状态 |
| --- | --- | --- | --- |
| 采购退款单 | 新增列表、筛选、详情、编辑、来源选择和退款预览；展示未入库数量、重量与金额 | 退款值只展示后端按采购订单减有效采购入库计算的结果，前端不自行决定财务口径 | 已实现；全量前端验证通过 |
| 采购预付款 | 付款单新增“付款用途”，采购预付款从采购订单专用候选分页导入供应商编码、名称和结算主体快照 | 不复用普通采购订单列表分页；采购预付款与普通对账付款字段按用途互斥 | 已实现；全量前端验证通过 |
| 预付款后续核销 | 已付款的采购预付款提供供应商对账分配弹窗，展示可分配余额并提交替换后的核销明细 | 最终金额上限、供应商编码一致性、结算主体一致性和对账单状态由后端事务再次校验 | 已实现；全量前端验证通过 |
| 供应商退款到账单 | 新增列表、详情、编辑及采购退款父单导入，自动带出供应商稳定编码和结算主体 | 前端允许分次登记；累计到账上限和退款状态由后端校验 | 已实现；全量前端验证通过 |
| 应收应付与台账调整 | 展示供应商编码、结算主体和新增财务事件字段；台账调整单补充结算主体 | 相同供应商的不同结算主体必须显示为独立账簿分组，不能在前端合并 | 已实现；全量前端验证通过 |
| 物流商稳定身份 | 物流商选项返回编码；物流单和物流对账单的表单、父单导入、列表筛选及请求响应均传递 `carrierCode` | 同名不同编码不可合并；编辑来源时同时校验物流商编码和结算主体 | 已实现；131 项 `carrierCode` 闭环定向测试曾通过 |

### 本轮验证口径

- 第 5 节中的 5921 项单测等数字属于 2026-07-10 历史整改批次，不能作为本次财务专项的最终全量结果。
- 本轮最终结果为 456 个测试文件通过、1 个跳过；6073 项测试通过、4 项跳过，且未再出现测试环境销毁后的 `window is not defined`。
- TypeScript、Biome、ESLint、生产构建和 `git diff --check` 均通过；React Doctor 对本轮差异复查为零问题。
- Ant Design CLI 未发现废弃 API 或可访问性问题，仅保留 `OssSettingsView` 禁用 Select 虚拟滚动这一条既有性能警告。

### 前端验收重点

1. 采购预付款来源选择必须请求专用候选分页，并使用服务端返回的 `total`；切换回普通对账付款时清除采购订单快照字段。
2. 采购退款页面不得允许用户改写后端计算的差额明细；重新打开或更换来源时必须重新请求退款预览。
3. 供应商退款到账按采购退款单导入稳定供应商编码和结算主体，分次到账时列表与详情金额不得被前端自行汇总后覆盖服务端结果。
4. 预付款核销弹窗必须阻止超出可分配余额的提交，但不能以该前端校验替代后端事务校验。
5. 物流单和物流对账单必须在创建、编辑、父单导入、筛选和详情回显的每条路径保留 `carrierCode`，不能回退为只传名称。
