...
- 现有外壳同时支持侧边导航和顶部导航，并承载全局搜索、个人设置、水印、后端在线状态及会话守卫。
...
API Key 和编号规则只将可稳定声明的输入、选择、数字输入与文本域迁移为 ProForm 字段。资源分组、预设联动、模板正文等复杂业务交互继续使用原生 Ant Design 组件。
...
打印模板、动态行项目和单据工作区保持领域组件，不纳入全局配置化页面模板。
