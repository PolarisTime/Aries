...
describe('ENDPOINTS', () => {
  ...
  it('has all required API key endpoints', () => {
    ...
    expect(ENDPOINTS.API_KEYS_RESOURCE_OPTIONS).toBe('/auth/api-keys/resource-option')
    expect(ENDPOINTS.API_KEYS_ACTION_OPTIONS).toBe('/auth/api-keys/action-option')
  })
  ...
  it('has all required settings endpoints', () => {
    ...
    expect(ENDPOINTS.GENERAL_SETTING_CLIENT_SETTINGS).toBe('/general-settings/client-setting')
  ...
  it('has permission and role endpoints', () => {
    ...
    expect(ENDPOINTS.ROLE_PERMISSION_OPTIONS).toBe('/role-settings/permission-option')
  })

  it('has all expected keys', () => {
    const expectedKeys = [
      'AUTH_LOGIN', 'AUTH_LOGIN_2FA', 'AUTH_REFRESH', 'AUTH_LOGOUT', 'AUTH_PING', 'AUTH_CAPTCHA',
      'API_KEYS', 'API_KEYS_USER_OPTIONS', 'API_KEYS_RESOURCE_OPTIONS', 'API_KEYS_ACTION_OPTIONS',
      'REFRESH_TOKENS', 'REFRESH_TOKENS_SUMMARY',
      'ACCOUNT_SECURITY_STATUS', 'ACCOUNT_PASSWORD', 'ACCOUNT_2FA_SETUP', 'ACCOUNT_2FA_ENABLE', 'ACCOUNT_2FA_DISABLE',
      'USER_ACCOUNTS', 'USER_ACCOUNTS_LOGIN_NAME_CHECK', 'USER_ACCOUNT_PREFERENCES',
      'PERMISSION_CATALOG',
      'ROLE_SETTINGS', 'ROLE_PERMISSION_OPTIONS',
      'SYSTEM_MENUS_TREE',
      'COMPANY_SETTINGS', 'COMPANY_SETTINGS_CURRENT', 'COMPANY_NAME',
      'HEALTH',
      'DASHBOARD_SUMMARY',
      'GLOBAL_SEARCH',
      'DATABASE_STATUS', 'DATABASE_MONITORING',
      'SECURITY_KEYS',
      'PRINT_TEMPLATES',
      'SETUP_STATUS', 'SETUP_INITIALIZE', 'SETUP_ADMIN_2FA', 'SETUP_ADMIN', 'SETUP_COMPANY',
      'ATTACHMENTS_UPLOAD', 'ATTACHMENTS_BINDINGS',
      'NUMBER_RULES_NEXT', 'GENERAL_SETTING_CLIENT_SETTINGS', 'UPLOAD_RULE',
      'MATERIALS_IMPORT', 'MATERIALS_TEMPLATE', 'MATERIALS_EXPORT', 'MATERIALS_SEARCH', 'MATERIAL_GRADES',
      'DEPARTMENTS_OPTIONS',
      'WAREHOUSES_OPTIONS', 'CUSTOMERS_OPTIONS', 'SUPPLIERS_OPTIONS', 'CARRIERS_OPTIONS', 'MATERIAL_CATEGORIES',
    ]
