...
import { moduleBehaviorRegistry, registerModuleBehavior } from './module-behavior-registry-core'
...
describe('registerModuleBehavior', () => {
  it('registers a new module behavior', () => {
    ...
    expect(moduleBehaviorRegistry.get('test-module')).toEqual({ supportsLineItems: true })
  })
  ...
  it('overwrites existing properties', () => {
    ...
    expect(moduleBehaviorRegistry.get('test-module')?.defaultStatus).toBe('已审核')
  })
  ...
  it('handles multiple modules independently', () => {
    ...
    expect(moduleBehaviorRegistry.get('module-a')).toEqual({ supportsLineItems: true })
    expect(moduleBehaviorRegistry.get('module-b')).toEqual({ computesAmounts: true })
  })
