export const enUS = {
  priceRule: {
    title: 'Price Rules',
    hint: 'Configure multiple net-price adjustments; pick one during delivery verification (single select, no stacking).',
    name: 'Name',
    namePlaceholder: 'e.g. Tax-inclusive',
    adjust: 'Adjust',
    add: 'Markup',
    subtract: 'Markdown',
    amount: 'Amount (CNY/ton)',
    amountPlaceholder: 'e.g. 30',
    remark: 'Remark',
    remarkPlaceholder: 'Optional note',
    addRow: 'Add rule',
    remove: 'Remove rule',
    save: 'Save price rules',
    saved: 'Price rules saved',
    nameRequired: 'Please enter a rule name',
    nameDuplicate: 'Rule names must be unique',
  },
  appUpdate: {
    title: 'System Updated',
    description:
      'A new system version is available. Clear the cache and refresh to continue.',
    unsavedWarning: 'Refreshing will discard any unsaved changes.',
    refreshButton: 'Clear Cache and Refresh',
  },
  errorBoundary: {
    retry: 'Retry',
    accessDenied: 'Access denied',
    serverBusy: 'Server is busy, please try again later',
    networkError: 'Network connection error',
    timeout: 'Request timed out',
    serverError: 'Server response error, please try again later',
    networkHint: 'Check your network connection and try again',
    forbiddenHint: 'Contact your administrator to request access',
    runtimeHint:
      'Refresh the page and try again; contact support if the problem persists',
    runtimeTitle: 'Page failed to render',
    errorSummary: 'Error summary',
    copyFailed: 'Copy failed. Please copy manually',
  },
  result: {
    '403': {
      subTitle: 'Access denied',
    },
    '404': {
      subTitle: 'Sorry, the page you visited does not exist',
    },
    '500': {
      subTitle: 'Sorry, the server encountered an error',
    },
    success: {
      title: 'Operation Successful',
    },
    error: {
      title: 'Operation Failed',
      subTitle: 'Please try again later',
    },
    info: {
      title: 'Info',
    },
    warning: {
      title: 'Warning',
    },
    homeButton: 'Back to Home',
    backButton: 'Go Back',
  },
  toolbar: {
    searchPlaceholder: 'Search...',
    refresh: 'Refresh',
    create: 'Create',
  },
  validation: {
    default: 'Validation failed',
    required: 'Please enter {label}',
    enum: '{label} is out of range',
    whitespace: '{label} cannot be empty',
    date: {
      format: '{label} has an invalid format',
      parse: '{label} could not be parsed as a date',
      invalid: '{label} is not a valid date',
    },
    types: {
      string: '{label} has an invalid format',
      method: '{label} has an invalid format',
      array: 'Please select {label}',
      object: '{label} has an invalid format',
      number: '{label} must be a number',
      date: '{label} is not a valid date',
      boolean: '{label} has an invalid format',
      integer: '{label} must be an integer',
      float: '{label} must be a number',
      regexp: '{label} has an invalid format',
      email: '{label} is not a valid email',
      url: '{label} is not a valid URL',
      hex: '{label} has an invalid format',
    },
    string: {
      len: '{label} must be {len} characters',
      min: '{label} must be at least {min} characters',
      max: '{label} must be at most {max} characters',
      range: '{label} must be between {min} and {max} characters',
    },
    number: {
      len: '{label} must equal {len}',
      min: '{label} cannot be less than {min}',
      max: '{label} cannot be greater than {max}',
      range: '{label} must be between {min} and {max}',
    },
    array: {
      len: '{label} must have {len} item(s)',
      min: '{label} must have at least {min} item(s)',
      max: '{label} must have at most {max} item(s)',
      range: '{label} must have between {min} and {max} item(s)',
    },
    pattern: {
      mismatch: '{label} has an invalid format',
    },
  },
  dashboard: {
    greeting: {
      welcome: 'Welcome back, {{name}}',
    },
    actions: {
      forceRefresh: 'Force refresh',
    },
    metrics: {
      purchaseAudit: 'Purchases to Approve',
      outboundTask: 'Outbound Tasks',
      statementConfirm: 'Statements to Confirm',
      statementAmount: 'Pending amount {{amount}}',
      monthOutbound: 'Outbound this month',
      monthInbound: 'Inbound this month',
      monthReceipt: 'Receipts this month',
      monthUnit: 'Documents this month',
      awaitingInbound: 'Purchase awaiting inbound',
      awaitingInboundHint: 'Audited, not yet inbound',
    },
    recent: {
      title: 'Recent documents',
      hint: 'Newest first, click to open details',
      empty: 'No documents',
      loadFailed: 'Failed to load recent documents. Please retry later.',
    },
    modules: {
      title: 'All modules',
      byFlow: 'View by workflow',
      create: 'Create',
    },
    todo: {
      title: 'To-Do Workbench',
      tabs: {
        all: 'All To-Dos',
        purchaseAudit: 'Purchase Approval',
        salesDelivery: 'Sales Delivery',
        financeReconcile: 'Finance Reconciliation',
      },
      columns: {
        docNo: 'Document No.',
        bizType: 'Business Type',
        relatedDocument: 'Related Document',
        counterparty: 'Counterparty',
        businessDate: 'Business Date',
        action: 'Action',
      },
      goProcess: 'Process',
      empty: 'No to-dos',
      loadFailed: 'Failed to load to-dos. Please retry later.',
    },
    account: {
      title: 'Account Overview',
    },
    flow: {
      material: {
        title: 'Material',
        hint: 'Maintain product names, specs, and brands',
      },
      supplier: {
        title: 'Supplier',
        hint: 'Procurement entity master data',
      },
      customer: {
        title: 'Customer',
        hint: 'Sales entity master data',
      },
      warehouse: {
        title: 'Warehouse',
        hint: 'Maintain warehouse and bin capabilities',
      },
      purchaseOrder: {
        title: 'Purchase Order',
        hint: 'Generate purchase execution order',
      },
      purchaseInbound: {
        title: 'Purchase Inbound',
        hint: 'Confirm inbound quantity and weight',
      },
      project: {
        title: 'Customer Project',
        hint: 'Maintain customer project records',
      },
      payment: {
        title: 'Payment',
        hint: 'Complete payment registration',
      },
      salesOrder: {
        title: 'Sales Order',
        hint: 'Issue customer order',
      },
      salesOutbound: {
        title: 'Sales Outbound',
        hint: 'Confirm shipment and outbound',
      },
      customerStatement: {
        title: 'Customer Statement',
        hint: 'Summarize sales receivables',
      },
      receipt: {
        title: 'Receipt',
        hint: 'Complete receipt registration',
      },
      carrier: {
        title: 'Carrier',
        hint: 'Maintain carrier entity',
      },
      freightBill: {
        title: 'Freight Bill',
        hint: 'Record transportation process',
      },
      freightStatement: {
        title: 'Freight Statement',
        hint: 'Summarize freight settlement',
      },
      loadingDescription: 'Loading business flow description...',
    },
    info: {
      userName: 'Current User',
      loginName: 'Login Account',
      companyName: 'Company',
      lastLogin: 'Last Login',
      accountInfo: 'Account Info',
      activeSessions: 'Active Sessions',
    },
    sections: {
      overview: 'Overview',
      businessFlow: 'Business Flow',
    },
    alerts: {
      title: 'Current Focus',
      loadFailed: 'Failed to load dashboard summary. Please try again later.',
    },
    fields: {
      serverTime: 'Server Time',
    },
    values: {
      unconfigured: 'Not configured',
    },
  },
  common: {
    confirm: 'Confirm',
    cancel: 'Cancel',
    save: 'Save',
    delete: 'Delete',
    edit: 'Edit',
    create: 'Create',
    search: 'Search',
    reset: 'Reset',
    export: 'Export',
    import: 'Import',
    refresh: 'Refresh',
    submit: 'Submit',
    back: 'Back',
    close: 'Close',
    ok: 'OK',
    yes: 'Yes',
    no: 'No',
    more: 'More',
    expand: 'Expand',
    collapse: 'Collapse',
    actions: 'Actions',
    status: 'Status',
    remark: 'Remark',
    operation: 'Operation',
    createdAt: 'Created',
    updatedAt: 'Updated',
    keyword: 'Keyword',
    detail: 'Detail',
    loading: 'Loading...',
    loadingIndicatorAnnouncing: 'Loading data…',
    noData: 'No Data',
    total: '{{count}} total',
    selected: '{{count}} selected',
    clearSelection: 'Clear selection',
    pleaseSelect: 'Please select',
    pleaseInput: 'Please input',
    confirmLogout: 'Confirm Logout',
    logout: 'Sign Out',
    saveSuccess: 'Saved Successfully',
    addSuccess: 'Added Successfully',
    editSuccess: 'Updated Successfully',
    deleteSuccess: 'Deleted Successfully',
    deleteConfirm: 'Confirm Delete?',
    unsavedChangesTitle: 'Unsaved Changes',
    unsavedChangesContent:
      'Your changes have not been saved. Continuing will discard them.',
    discardChanges: 'Discard Changes',
    saveInProgressTitle: 'Saving',
    saveInProgressContent:
      'This document is being saved. Wait for it to finish before leaving.',
    columnSettings: 'Column Settings',
    resetColumnWidths: 'Reset column widths',
    saveAndAudit: 'Save & Audit',
    auditConfirm:
      'After audit, the document will be locked and cannot be edited. Continue?',
    confirmLogoutContent: 'Are you sure you want to sign out?',
    displaySettingsSaved: 'Display settings saved',
    brandSubtitle: 'Steel Trading Business Platform',
    productCopyright: '© {{year}} Leo',
    frontendVersion:
      'Frontend v{{version}} · Build time {{buildTime}} · Build SHA-1 {{gitCommit}}',
    backendVersion:
      'Backend v{{version}} · Build time {{buildTime}} · Build SHA-1 {{gitCommit}}',
    versionUnknown: '--',
    confirmAudit: 'Confirm Audit',
    saveFailed: 'Save failed',
    saveFailedRetry: 'Save failed, please try again later',
    query: 'Query',
    masterData: 'Master Data',
    purchaseChain: 'Procurement',
    salesChain: 'Sales',
    masterDataDesc:
      'Maintain business master data first, which can be referenced in subsequent documents.',
    purchaseChainDesc:
      'From contracts and orders, through inbound and reconciliation, to payment.',
    salesChainDesc:
      'From contracts and orders, through outbound and reconciliation, to receipt.',
    prevPage: 'Previous',
    nextPage: 'Next',
    importParentFailed: 'Failed to import parent document',
    logisticsChain: 'Logistics',
    logisticsChainDesc:
      'Logistics documents can flow independently or be embedded in procurement and sales chains.',
    pleaseSelectWith: 'Please select {{label}} first',
    importParentSuccess:
      'Imported {{parentCount}} parent document(s), {{itemCount}} line item(s)',
    importParentSuccessSimple: 'Imported {{itemCount}} line item(s)',
    disabled: 'Disabled',
    batchDelete: 'Batch Delete',
  },
  finance: {
    filters: {
      advanced: 'Advanced Filters',
    },
    overview: {
      title: 'Finance Overview',
      description:
        'Review receivables, payables, and counterparty balances by settlement entity and date.',
      refreshAria: 'Refresh finance overview',
      emptyBalances: 'No balances yet',
      selectSettlementCompany: 'Select a settlement entity',
    },
    cashLedger: {
      title: 'Cash Ledger',
      description:
        'Review payment flows and period balance changes for a settlement entity.',
      exportFiltered: 'Export filtered results',
      refreshAria: 'Refresh cash ledger',
    },
  },
  inventory: {
    title: 'Inventory Query',
    description:
      'Review inventory balances and stock movements by material and warehouse.',
    refreshAria: 'Refresh inventory data',
    loadFailed: 'Failed to load inventory data',
    loadFailedHint: 'Please try again later',
    tabs: {
      balances: 'Inventory Balances',
      transactions: 'Stock Movements',
    },
    filters: {
      keyword: 'Keyword',
      balanceKeywordPlaceholder: 'Material code / brand / material / spec',
      transactionKeywordPlaceholder:
        'Transaction no. / material / source document no.',
      warehouse: 'Warehouse',
      warehousePlaceholder: 'All warehouses',
      transactionType: 'Transaction Type',
      transactionTypePlaceholder: 'All transaction types',
      occurredAt: 'Occurred Date',
    },
    columns: {
      materialCode: 'Material Code',
      brand: 'Brand',
      material: 'Material',
      spec: 'Spec',
      length: 'Length',
      unit: 'Unit',
      warehouse: 'Warehouse',
      batchNo: 'Batch No.',
      quantity: 'Quantity',
      avgUnitCost: 'Avg Unit Cost',
      amount: 'Amount',
      transactionNo: 'Transaction No.',
      transactionType: 'Type',
      direction: 'Direction',
      unitCost: 'Unit Cost',
      sourceDocumentNo: 'Source Document No.',
      occurredAt: 'Occurred At',
    },
    direction: {
      in: 'In',
      out: 'Out',
    },
    transactionType: {
      PURCHASE_INBOUND: 'Purchase Inbound',
      PURCHASE_RETURN: 'Purchase Return Outbound',
      SALES_OUTBOUND: 'Sales Outbound',
      SALES_RETURN: 'Sales Return Inbound',
      INBOUND: 'Inbound',
      OUTBOUND: 'Outbound',
      ADJUSTMENT_IN: 'Stock Gain',
      ADJUSTMENT_OUT: 'Stock Loss',
      TRANSFER_IN: 'Transfer In',
      TRANSFER_OUT: 'Transfer Out',
      INITIAL_BALANCE: 'Opening Balance',
    },
    empty: {
      balances: 'No inventory balances',
      transactions: 'No stock movements',
    },
  },
  auth: {
    login: 'Sign In',
    logout: 'Sign Out',
    loginName: 'Username',
    password: 'Password',
    remember: 'Remember me',
    loginSuccess: 'Login successful',
    loginFailed: 'Login failed',
    logoutSuccess: 'Logged out',
    sessionExpired: 'Session expired, please sign in again',
    initialSetup: 'Initial Setup',
    loginview: {
      heroSubtitle:
        'An integrated operations hub for purchasing, sales, inventory, and finance',
      backendOnline: 'Backend service online',
      backendOffline: 'Backend service offline',
      setupReady: 'System ready',
      setupPending: 'Initialization pending',
    },
    loginform: {
      step: 'Password Verification',
      title: 'Sign In',
      description: 'Enter your account credentials to access the system.',
      loginNameLabel: 'Login Name',
      loginNameRequired: 'Please enter your username',
      loginNamePlaceholder: 'Enter username',
      passwordLabel: 'Password',
      passwordRequired: 'Please enter your password',
      passwordPlaceholder: 'Enter password',
      remember: 'Remember sign-in state',
      submit: 'Sign In',
    },
    initialsetup: {
      checking: 'Checking initialization status...',
      completedTitle: 'System initialization is complete',
      guideTitle: 'Initialize the Only Account',
      defaultAccountUserName: 'System User',
      setupTokenLabel: 'Setup Token',
      setupTokenPlaceholder: 'Enter the deployment setup token',
      setupTokenRequired: 'Please enter the setup token',
      setupTokenInvalid: 'The setup token format is invalid',
      alreadyCompletedRedirect:
        'System initialization is complete. Redirecting to sign-in.',
      loadStatusFailed: 'Failed to load initialization status',
      operationFailed: 'Operation failed',
      passwordMismatch: 'The two passwords do not match',
      accountCreateSuccess: 'Account created',
      account: {
        loginNameLabel: 'Login Name',
        loginNameRequired: 'Please enter login name',
        loginNamePlaceholder: 'Login name',
        passwordLabel: 'Password',
        passwordRequired: 'Password must be at least 8 characters',
        passwordPlaceholder: 'At least 8 characters',
        confirmPasswordLabel: 'Confirm Password',
        confirmPasswordRequired: 'Please confirm password',
        confirmPasswordPlaceholder: 'Enter password again',
        userNameLabel: 'Name',
        userNamePlaceholder: 'System User',
        submit: 'Finish Initialization',
      },
    },
    validation: {
      required: 'This field is required',
    },
    error: {
      network: 'Network error, please check your connection',
      timeout: 'Request timed out, please try again',
      sessionExpired: 'Session expired, please sign in again',
      forbidden: 'Access denied',
      notFound: 'API endpoint not found',
      serverError: 'Service temporarily unavailable, please try again later',
      generic: 'Request failed, please try again later',
      loginFailed: 'Login failed',
      missingTokenOrUser: 'Login response missing token or user info',
      sessionRestoreFailed: 'Session restore failed',
    },
  },
  table: {
    create: 'Create',
    edit: 'Edit',
    delete: 'Delete',
    batchDelete: 'Batch Delete',
    deleteConfirm: 'Confirm deletion?',
    deleteSuccess: 'Deleted successfully',
    saveSuccess: 'Saved successfully',
    createSuccess: 'Created successfully',
    updateSuccess: 'Updated successfully',
    operationSuccess: 'Operation successful',
    operationFailed: 'Operation failed',
    readOnly: 'Read-only mode',
  },
  attachment: {
    title: 'Attachments',
    upload: 'Upload',
    preview: 'Preview',
    download: 'Download',
    delete: 'Delete',
    uploadSuccess: 'Upload successful',
    notSupported: 'Preview not supported',
  },
  api: {
    requestFailed: 'Request failed',
    loadFailed: 'Load failed',
    saveFailed: 'Save failed',
    deleteFailed: 'Delete failed',
    updateFailed: 'Update failed',
    createFailed: 'Create failed',
    exportFailed: 'Export failed',
    importFailed: 'Import failed',
    validationFailed: 'Validation failed',
    networkError: 'Network error, please check your connection',
    timeout: 'Request timeout, please try again later',
    unauthorized: 'Unauthorized, please log in again',
    forbidden: 'Access denied',
    notFound: 'Resource not found',
    serverError: 'Server error, please try again later',
    moduleNotConfigured: 'Module endpoint not configured',
    authServiceUnavailable: 'Authentication service unavailable',
    backendServiceUnavailable: 'Backend service unavailable',
    responseValidationFailed: 'Response data validation failed',
    pageDataValidationFailed: 'Page data validation failed',
    loadAccountColumnSettingsFailed: 'Failed to load account column settings',
    saveAccountColumnSettingsFailed: 'Failed to save account column settings',
    resultIncomplete:
      'Result incomplete: client filter limit reached. Please narrow your filters.',
    loadCompanyInfoFailed: 'Failed to load settlement entity info',
    saveCompanyInfoFailed: 'Failed to save settlement entity info',
    loadCurrentAccountFailed: 'Failed to load the current account',
    saveCurrentAccountFailed: 'Failed to save the current account',
    loadSearchResultsFailed: 'Failed to load search results',
    getInitStatusFailed: 'Failed to get initialization status',
    accountInitFailed: 'Account initialization failed',
    importResponseValidationFailed: 'Import response validation failed',
    queryStatementCandidatesFailed: 'Failed to query statement candidates',
    loginExpiring: 'Login expiring',
    loginExpiringDescription:
      'Your login will expire tomorrow. Please save your work and log in again.',
    refreshLoginStatusFailed: 'Failed to refresh login status',
    loginStatusExpired: 'Login status expired, please log in again',
    errorMessages: {
      requiredEmpty: 'Required field cannot be empty',
      networkError:
        'Network connection failed, please check if the service is available',
      timeout: 'Request timed out, please try again later',
      sessionExpired: 'Login session expired, please sign in again',
      forbidden: 'Access denied',
      notFound: 'Requested endpoint does not exist',
      requestFailed: 'Request failed, please try again later',
      missingRequiredField: 'Missing required field: {{fieldName}}',
      serviceUnavailable:
        'Service temporarily unavailable, please try again later',
    },
  },
  modules: {
    actions: {
      create: 'Create',
      export: 'Export',
    },
    purchasePickupList: {
      action: 'View Pickup List ({{count}})',
      title: 'Purchase Order Pickup List ({{count}})',
      tooManyOrders: 'Up to 50 purchase orders can be viewed at once',
      loadFailed: 'Failed to load pickup list',
      orderCount: 'Orders',
      supplierCount: 'Suppliers',
      itemCount: 'Items',
      totalQuantity: 'Total Qty',
      totalWeight: 'Total Weight (ton)',
      itemName: 'Item',
      pickupQuantity: 'Quantity',
      pieceWeight: 'Piece Weight (ton)',
      pickupWeight: 'Weight (ton)',
      groupItemCount: '{{count}} item(s)',
      groupTotalQuantity: 'Total Qty',
      groupTotalWeight: 'Total Weight',
      groupByWarehouse: 'Group by Warehouse',
      unassignedWarehouse: 'Unassigned Warehouse',
      addGroup: 'Add Group',
      groupLabel: 'Group {{index}}',
      groupRemarkPlaceholder: 'Add a group note',
      lockGroup: 'Lock group {{index}}',
      unlockGroup: 'Unlock group {{index}}',
      removeGroup: 'Remove group {{index}}',
      emptyGroup: 'No items',
      dragGroup: 'Drag group {{index}}',
      dragRow: 'Drag row {{index}}',
      restoreDefault: 'Restore Default Order',
      splitItem: 'Split quantity',
      mergeItem: 'Merge split',
      splitPartLabel: 'Part {{index}}/{{total}}',
      removeSplitPart: 'Remove row {{index}}',
      splitRecorded:
        'Split quantities are for this view and grouping only, and are not written back to purchase orders.',
      splitTitle: 'Split quantity',
      splitConfirm: 'Split',
      splitPieceCountLabel: 'Pieces per part',
      splitPieceCountPlaceholder: 'Enter pieces per part',
      splitTotal: '{{total}} pieces in total',
      splitPreview: 'Split preview',
      splitPartShort: 'Part {{index}}: {{quantity}} pcs',
      splitPartTotal: '{{count}} parts (~{{pieceWeight}} t each piece)',
      splitInvalid: 'Pieces per part must be an integer from 1 to {{max}}',
    },
    overview: {
      recordCount: 'Records',
      totalWeight: 'Total Weight',
      totalAmount: 'Total Amount',
      documentCount: 'Document count',
      masterDataCount: 'Master Data',
      normalCount: 'Normal',
      currentAmount: 'Current Amount',
      settledAmount: 'Settled',
      statementCount: 'Statements',
      balance: 'Balance',
    },
    status: {
      draft: 'Draft',
      unaudited: 'Unaudited',
      audited: 'Audited',
      reviewed: 'Reviewed',
      issued: 'Issued',
      executing: 'Executing',
      signed: 'Signed',
      unsigned: 'Unsigned',
      archived: 'Archived',
      voided: 'Voided',
      completed: 'Completed',
      completedPurchase: 'Purchase Done',
      completedSales: 'Sales Done',
      deliveryVerification: 'Delivery Verification',
      completedInbound: 'Inbound Done',
      confirmed: 'Confirmed',
      pendingConfirm: 'Pending Confirm',
      pendingAudit: 'Pending Audit',
      pendingApproval: 'Pending Approval',
      approved: 'Approved',
      paid: 'Paid',
      received: 'Received',
      partialInbound: 'Partial Inbound',
      partialOutbound: 'Partial Outbound',
      partialSettled: 'Partial Settled',
      normal: 'Normal',
      disabled: 'Disabled',
      success: 'Success',
      failed: 'Failed',
      deleted: 'Deleted',
    },
    statusActions: {
      audit: 'Audit',
      reverseAudit: 'Reverse Audit',
      confirm: 'Confirm',
      reverseConfirm: 'Reverse Confirmation',
      approve: 'Approve',
      reverseApprove: 'Reverse Approval',
      reopenDeliveryVerification: 'Reopen Verification',
    },
    editor: {
      edit: 'Edit',
      create: 'Create',
      title: '{{mode}} — {{title}}',
      missingQuantityUnit:
        'Material "{{material}}" has no quantity unit. Complete the material master data before saving.',
    },
    itemColumns: {
      brand: 'Brand',
      material: 'Material',
      spec: 'Spec',
      length: 'Length',
      quantity: 'Qty',
      weightTon: 'Weight(ton)',
      unitPrice: 'Unit Price',
      amount: 'Amount',
      warehouseName: 'Warehouse',
      sourceNo: 'Outbound No',
      materialSearchPlaceholder:
        'Search brand / category / material / spec / length',
      warehousePlaceholder: 'Select warehouse',
      settlementModePlaceholder: 'Select settlement mode',
    },
    units: {
      documentSheet: 'sheets',
      ton: 'ton',
      yuan: 'CNY',
    },
    nextModule: {
      createPurchaseInbound: 'Create Inbound',
      createSalesOutbound: 'Create Outbound',
      createFreightBill: 'Create Freight Bill',
    },
    saveResult: {
      success: 'Saved Successfully',
      pageSuccess: '{{title}} saved successfully',
      error: 'Save Failed',
      conflict: 'This data was updated by another operation',
      backToEdit: 'Back to Edit',
      reloadLatest: 'Reload Latest Data',
      createAnother: 'Create Another',
      close: 'Close',
    },
    statement: {
      supplier: 'Supplier',
      customer: 'Customer',
      freight: 'Freight',
      counterpartyNotFound: 'Counterparty information not found',
      multipleCounterparties:
        'Selected documents contain multiple counterparties. Please select documents from the same counterparty only.',
      dateMissing: 'Selected documents are missing date information',
      generateTitle: 'Generate {{type}} Statement',
      selectHint:
        'Please select documents from the list to generate a statement',
      extractError:
        'Unable to extract statement information from selected documents. Please check data integrity.',
      counterpartyUnit: 'Counterparty: ',
      period: 'Period: ',
      documentCount: 'Documents: ',
      documentCountUnit: '',
      generated: 'Statement generated',
      generateFailed: 'Generation failed, please try again later',
      generateButton: 'Generate Statement',
      gotIt: 'Got it',
    },
    weightCell: {
      materialCode: 'Material Code',

      materialType: 'Material Type',
      materialTypePhysical: 'Physical Goods',
      materialTypeExpense: 'Additional Charge',
      brand: 'Brand',
      material: 'Material',
      spec: 'Spec',
      length: 'Length',
      pieceWeightTon: 'Piece Wt(ton)',
      quantity: 'Qty',
      weightTon: 'Weight(ton)',
      popoverTitle: 'Piece Weight Details',
      noLineItems: 'No line item data',
      loadDetailFailed: 'Failed to load details',
    },
    detail: {
      titleSuffix: 'Detail',
      recordDetail: 'Record Detail',
      noData: 'No Data',
      close: 'Close',
      print: 'Print',
      noDetailItems: 'No detail items',
    },
    attachment: {
      title: 'Attachments',
      upload: 'Upload',
      uploadHint:
        'Click to upload, or paste images/files directly in this area',
      noPreviewUrl: 'No preview URL available',
      noDownloadUrl: 'No download URL available',
      previewFailed: 'Preview failed',
      downloadFailed: 'Download failed',
      uploadBindSuccess: 'Uploaded and bound successfully',
      uploadFailed: 'Upload failed',
      uploadNoId: 'Upload succeeded but no attachment ID returned',
      uploadingProgress: 'Uploading {{fileName}}, {{percent}}% complete',
      unbindSuccess: 'Unbound successfully',
      deleteFailed: 'Delete failed',
      loadFailed: 'Failed to load attachments',
      noAttachments: 'No attachments',
      preview: 'Preview attachment',
      download: 'Download attachment',
      unbind: 'Unbind attachment',
      unbindConfirm:
        'Are you sure you want to unbind this attachment from the current document?',
      pdfPreview: 'PDF Preview',
    },
    filter: {
      keyword: 'Keyword',
      status: 'Status',
      billStatus: 'Bill Status',
      auditStatus: 'Audit Status',
      signStatus: 'Sign Status',
      customerName: 'Customer',
      supplierName: 'Supplier',
      carrierName: 'Carrier',
      orderNo: 'Document No',
      contractNo: 'Contract No',
      outboundNo: 'Outbound No',
      inboundNo: 'Inbound No',
      freightNo: 'Freight No',
      referenceStatus: 'Downstream reference',
      referencedByFreightBill: 'Referenced by freight bill',
      referencedBySalesOutbound: 'Referenced by sales outbound',
      referencedBySalesOrder: 'Referenced by sales order',
      referencedByPurchaseInbound: 'Referenced by purchase inbound',
      notReferenced: 'Unreferenced',
      all: 'All',
      today: 'Today',
      last7Days: 'Last 7 days',
      last30Days: 'Last 30 days',
      thisMonth: 'This month',
      selectPlaceholder: 'Select {{label}}',
      inputPlaceholder: 'Enter {{label}}',
      quickFilters: 'Quick Filters',
      conditions: 'Filters',
      documentOrCounterparty: 'Doc No. / Counterparty',
      counterpartyCode: 'Counterparty Code',
      counterparty: 'Counterparty',
      counterpartyType: 'Counterparty Type',
      counterpartyCustomer: 'Customer',
      counterpartySupplier: 'Supplier',
      relatedContractProject: 'Related Contract/Project',
      receiptVoucher: 'Receipt/Voucher',
      relatedFreightBill: 'Related Freight Bill',
      freightBill: 'Freight Bill',
      salesOrder: 'Sales Order',
      activeCount: '{{count}} active',
    },
    formField: {
      inputPlaceholder: 'Enter {{label}}',
      selectPlaceholder: 'Select {{label}}',
      inputRequired: 'Please enter {{label}}',
      selectRequired: 'Please select {{label}}',
      nonNegative: '{{label}} must be non-negative',
    },
    salesContract: {
      contractNo: 'Contract No',
      name: 'Contract Name',
      customer: 'Customer',
      project: 'Project',
      signDate: 'Sign Date',
      startDate: 'Start Date',
      endDate: 'End Date',
      totalAmount: 'Contract Amount',
      totalTonnage: 'Contract Tonnage',
      dateRangeInvalid: 'End date cannot be earlier than start date',
      contractNoAutoHint: 'Leave blank to generate automatically',
      effectiveCount: 'Effective',
      issue: 'Issue',
      archive: 'Archive',
      versionConflict:
        'Contract was modified by someone else; refresh and retry',
      void: 'Void',
      deleteConfirmTitle: 'Delete Sales Contract',
      deleteConfirmContent: 'Delete contract "{{contractNo}}"?',
      deleteSuccess: 'Deleted',
      statusChangeConfirmTitle: 'Change Contract Status',
      statusChangeConfirmContent:
        'Change contract "{{contractNo}}" status to "{{status}}"?',
      statusChangeSuccess: 'Status updated',
    },
    salesContractCheck: {
      title: 'Contract Limit Exceeded',
      checkFailed: 'Contract limit check failed; saving was not blocked',
      stillSave: 'Save Anyway',
      backToEdit: 'Back to Edit',
      contractAmount: 'Contract amount: {{value}}',
      usedAmount: 'Used amount: {{value}}',
      remainingAmount: 'Remaining amount: {{value}}',
      contractTonnage: 'Contract tonnage: {{value}}',
      usedTonnage: 'Used tonnage: {{value}}',
      remainingTonnage: 'Remaining tonnage: {{value}}',
      exceededAmount: 'Exceeded amount this time: {{value}}',
      exceededTonnage: 'Exceeded tonnage this time: {{value}}',
    },
    print: {
      noTemplate: 'No templates available',
      preview: 'Print Preview',
      directPrint: 'Direct Print',
      downloadPdf: 'Export PDF',
      print: 'Print',
      jobTitle: 'Print Job',
      currentModule: 'Current Module',
      selectedRecords: '{{count}} document(s) selected',
      currentSettlementCompany: 'Settlement company',
      selectTemplate: 'Print Template',
      printOptions: 'Print Options',
      exportXlsx: 'Export Print Excel',
      hideUnitPrice: 'Hide unit price',
      hideRemark: 'Hide document remark',
      enableBrandOverride: 'Replace print brand',
      enableItemSelection: 'Select print items',
      enableSplitPrint: 'Split print',
      splitPieceCountLabel: 'Pieces per copy',
      splitPieceCountPlaceholder: 'Enter a positive integer',
      splitPieceCountRequired: 'Enter pieces per copy first',
      itemSplit: 'Split',
      splitPartIndex: 'Copy',
      splitNotNeeded: 'Quantity does not exceed pieces per copy; no split',
      mergeEquivalentItems: 'Merge matching',
      splitEquivalentItems: 'Split items',
      brandOverrideTo: 'Replace with',
      brandOverridePlaceholder: 'Leave blank to keep original',
      noBrands: 'No brands found in selected documents',
      totalItemsCount: '{{count}} item(s) total',
      selectedItemsCount: '{{count}} item(s) selected',
      noPrintItems: 'No printable line items found',
      printItemsLoadFailed: 'Failed to load print items',
      dragRowAriaLabel: 'Drag print item row {{index}}',
      deliveryDate: 'Delivery Date',
      recordRemark: 'Document Remark',
      totalQuantity: 'Total Qty',
      totalWeight: 'Total Weight',
      closeConfirmTitle: 'Close Print Job?',
      closeConfirmContent:
        'Closing will clear the current item selection, order, and output markers. Continue?',
      continueJob: 'Continue Working',
      itemSequence: 'No.',
      outputted: 'Output',
      mergeRows: 'rows merged into 1 row',
      itemBrand: 'Brand',
      itemCategory: 'Category',
      itemMaterial: 'Material',
      itemSpec: 'Spec',
      itemLength: 'Length',
      itemQuantity: 'Qty',
      itemPieceWeight: 'Piece Wt.',
      itemWeight: 'Total Wt.',
      itemUnitPrice: 'Unit Price',
      itemAmount: 'Amount',
      project: 'Project',
      selectedRecordList: 'Selected Documents',
      onlySelectedIds:
        'Selected record IDs are retained. No current-page summary is available.',
      moreSelectedRows: '{{count}} more current-page record(s) hidden',
      unloadedSelectedRows:
        '{{count}} cross-page selected record(s) will still be printed',
    },
    itemsPanel: {
      defaultTitle: 'Line Items',
    },
    workspace: {
      closeAria: 'Close workspace',
      currentPageSummary: 'Current page summary',
      pagination: 'List pagination',
      paginationRange: '{{start}}-{{end}} / {{total}} total',
    },
    page: {
      configNotFound: 'Page configuration not found',
      moduleConfigNotFound: 'Module configuration not found',
    },
    itemsSection: {
      addItem: 'Add Item',
      goodsTab: 'Goods Items',
      expenseTab: 'Additional Charges',
      addExpense: 'Add Charge',
      expensePanelTitle: 'Charge Items',
      importItems: 'Import {{label}} Items',
      deleteSelected: 'Delete Selected',
      autoSortItems: 'Auto Sort',
      autoSortBySourceNo: 'By bill number',
      autoSortByBillTime: 'Sort by date',
      autoSortByBillTimeAscending: 'Sort by date (ascending)',
      autoSortByBillTimeDescending: 'Sort by date (descending)',
      emptyTextWithImport:
        'No items yet. Add manually or import from parent document.',
      emptyText: 'No items yet. Add manually.',
      parentDoc: 'Parent Document',
      selectParent: 'Select {{label}}',
      freightBillGroup: 'Source freight bill: {{sourceNo}}',
      freightBillGroupBillTime: 'Document date: {{billTime}}',
      freightBillGroupCustomer: 'Customer: {{customerName}}',
      freightBillGroupProject: 'Project: {{projectName}}',
      freightBillGroupTotal: 'Total: Qty {{quantity}}, Weight {{weight}} ton',
      freightBillGroupUnitPrice: 'Freight unit price: {{unitPrice}} CNY/ton',
      freightBillGroupTotalFreight: 'Freight total: {{totalFreight}} CNY',
      freightBillProjectGroup: 'Project: {{projectName}}',
      freightBillProjectGroupCustomer: 'Customer: {{customerName}}',
      freightBillProjectGroupTotal:
        'Subtotal: Qty {{quantity}}, Weight {{weight}} ton',
      customerStatementGroupNo: 'Group: ',
      customerStatementSalesOrderNo: 'Sales order: ',
      customerStatementDeliveryDate: 'Delivery date: ',
      customerStatementTotalQuantity: 'Total Qty: ',
      customerStatementTotalWeight: 'Total Weight: ',
      customerStatementTotalAmount: 'Total Amount: ',
    },
    expense: {
      chargeName: 'Charge Name',
      chargeNamePlaceholder: 'Search or enter new charge name',
      quickCreate: 'Create charge item: "{{name}}"',
      noChargeOptions: 'No matching charges. Type a name to create one',
      amount: 'Amount',
      unit: 'Unit',
      unitPlaceholder: 'time/item',
      remark: 'Remark',
      emptyHint: 'No additional charges yet. Click "Add Charge" to enter',
      removeSelected: 'Remove Selected Charges',
      selectAllAriaLabel: 'Select all charge rows',
      deleteRowAriaLabel: 'Delete charge row {{index}}',
      totalAmount: 'Total Charges',
      rowCount: 'Rows',
    },
    table: {
      noData: 'No data',
      sequence: 'No.',
    },
    editorFooter: {
      cancel: 'Cancel',
      save: 'Save',
      saveAndAudit: 'Save & Audit',
      saveAndAction: 'Save & {{action}}',
      statusActionConfirm:
        'This action will change the document status to {{targetStatus}} and lock further editing. Continue with {{action}}?',
      confirmStatusAction: 'Confirm {{action}}',
      confirmDeliveryVerification: 'Confirm Verification',
      confirmDeliveryVerificationContent:
        'After confirmation, the sales order will be completed and settled using the final delivery results. Confirm that the outbound delivery and actual weight have been verified.',
    },
    itemsSummary: {
      rowCount: 'Rows',
      quantity: 'Qty',
      weight: 'Weight',
      amount: 'Amount',
      expenseAmount: 'Expense Total',
    },
    editorForm: {
      documentInfo: 'Document Info',
    },
    parentSelector: {
      title: 'Select Parent Document',
      selectedCount: '{{count}} selected',
      cancel: 'Cancel',
      confirmImport: 'Confirm Import',
      selectedDocuments: 'Selected Documents',
      selectedDocumentsCount: '({{count}})',
      clearSelected: 'Clear Selection',
      removeAriaLabel: 'Remove {{name}}',
      noSelectionHint:
        'No documents selected yet. Check items to pin them here.',
      paginationTotal: 'Total {{count}}',
      viewDetail: 'View details',
      collapseDetail: 'Collapse details',
      expandDetail: 'Expand details',
      copied: 'Copied',
      detailLines: '{{count}} items',
      detailImportable: 'Importable qty {{count}}',
      selectedMultiSummary:
        'Selected {{orderCount}} orders ({{lineCount}} items, importable qty {{importableQuantity}})',
      selectedMultiSummarySimple:
        'Selected {{orderCount}} orders ({{lineCount}} items)',
      selectedSingleSummary: 'Selected {{count}} order: {{docNo}}',
      selectedEmptyHint: 'Nothing selected. Click a row or checkbox to select.',
      status: {
        draft: 'Draft',
        unaudited: 'Unaudited',
        audited: 'Audited',
        purchaseComplete: 'Purchase Complete',
        salesComplete: 'Sales Complete',
        inboundComplete: 'Inbound Complete',
      },
      column: {
        orderNo: 'Order No',
        supplierName: 'Supplier',
        buyerName: 'Buyer',
        orderDate: 'Order Date',
        totalWeight: 'Total Weight (ton)',
        totalAmount: 'Total Amount',
        status: 'Status',
        relatedPurchaseOrder: 'Related Purchase Order',
        customerName: 'Customer',
        projectName: 'Project',
        deliveryDate: 'Delivery Date',
        outboundNo: 'Outbound No',
        relatedOrder: 'Related Order',
        outboundDate: 'Outbound Date',
        inboundNo: 'Inbound No',
        inboundDate: 'Inbound Date',
        docNo: 'Document No',
      },
      filter: {
        keyword: 'Keyword',
        keywordPlaceholder: 'Enter document number or keyword',
      },
      summary: {
        supplierName: 'Supplier',
        buyerName: 'Buyer',
        orderDate: 'Order Date',
        relatedOrder: 'Related Order',
        inboundDate: 'Inbound Date',
        customerName: 'Customer',
        projectName: 'Project',
        deliveryDate: 'Delivery Date',
        outboundDate: 'Outbound Date',
        carrierName: 'Carrier',
        relatedOutbound: 'Related Outbound',
        billTime: 'Document Date',
      },
    },
    editorWorkspace: {
      currentUserFallback: 'Current User',
      autoGeneratedPlaceholder: 'Auto generated',
      masterDataCodeGenerationFailed:
        'Failed to generate the master data code. Close and reopen the create form.',
    },
    validation: {
      maxImportExceeded: 'Row {{row}}: import quantity cannot exceed {{max}}',
      weighRequired:
        'Row {{row}}: this category requires weigh-based inbound. Please change the settlement mode to weigh.',
      weighWeightRequired: 'Please enter the weigh weight for row {{row}}',
      lockedSalesWeightExceeded:
        'Row {{row}}: weigh weight is lower than locked sales weight {{locked}} ton. Please adjust the weigh weight, or reverse-audit the sales outbound/order before saving.',
      purchaseInboundSourceRequired:
        'Row {{row}}: source purchase order item is required',
      purchaseInboundWarehouseRequired:
        'Row {{row}}: source warehouse ID is required',
      purchaseOrderQuantityMinimum:
        'Row {{row}}: quantity must be at least one quantity unit',
      purchaseInboundQuantityPositive:
        'Row {{row}}: inbound quantity must be greater than zero',
      salesOutboundQuantityMinimum:
        'Row {{row}}: outbound quantity must be at least one quantity unit',
      purchaseInboundMixedSource:
        'A purchase inbound can reference only one purchase order',
      lineItemRequired: 'Please enter {{label}} for row {{row}}',
      fieldRequired: 'Please enter {{label}}',
      minOneItem: 'Please add at least one line item',
      parentRelationOccupied:
        '{{parentLabel}}{{parentNo}} is already linked to {{occupiedNo}}',
      errorSummarySuffix: 'and {{count}} other issue(s)',
    },
    columns: {
      materialCode: 'Material Code',
      brand: 'Brand',
      category: 'Category',
      material: 'Material',
      spec: 'Spec',
      length: 'Length',
      unit: 'Unit',
      quantityUnit: 'Qty Unit',
      quantity: 'Quantity',
      pieceWeightTon: 'Piece Weight(ton)',
      piecesPerBundle: 'Pcs/Bundle',
      weightTon: 'Weight(ton)',
      unitPrice: 'Unit Price',
      amount: 'Amount',
      totalAmount: 'Total Amount',
      totalWeight: 'Total Weight',
      status: 'Status',
      remark: 'Remark',
      settlementMode: 'Settlement Mode',
      warehouse: 'Warehouse',
      warehouseName: 'Warehouse Name',
      batchNo: 'Batch No',
      materialName: 'Material Name',
      orderNo: 'Document No',
      customerName: 'Customer',
      projectName: 'Project',
      purchaseOrderNo: 'PO No',
      salesOrderNo: 'SO No',
      inboundNo: 'Inbound No',
      outboundNo: 'Outbound No',
      outboundRemainingQuantity: 'Remaining Outbound Qty',
      weighWeight: 'Weigh Weight',
      weightAdjustmentAmount: 'Weight Adj Amount',
      weightAdjustmentTon: 'Weight Adj(ton)',
      settlementCompany: 'Settlement company',
      account: 'Fund account',
    },
    pages: {
      material: {
        materials: 'Materials',
        keyword: 'Keyword',
        materialPlaceholder: 'Search code / brand / category / spec / length',
        category: 'Category',
        material: 'Material',
        materialCode: 'Material Code',
        materialType: 'Material Type',
        materialTypePhysical: 'Physical Goods',
        materialTypeExpense: 'Additional Charge',
        brand: 'Brand',
        spec: 'Spec',
        length: 'Length',
        unit: 'Unit',
        qtyUnit: 'Qty Unit',
        pieceWeightTon: 'Piece Weight (t)',
        pcsPerBundle: 'Pieces / Bundle',
        unitPrice: 'Unit Price',
        remark: 'Remark',
        materialCount: 'Materials',
        calculated: 'Threaded',
        weighed: 'Weighed',
        materialDesc:
          'Material master data including brand, category, spec, weight, and price.',
        downloadTemplate: 'Download Template',
        templateDownloadFailed: 'Failed to download material template',
        importFailed: 'Failed to import materials',
        importSuccessSummary:
          'Import completed: {{totalRows}} total, {{successCount}} succeeded, {{createdCount}} created, {{updatedCount}} updated, {{skippedCount}} skipped, {{failedCount}} failed',
        importPartialFailure:
          'Import completed with {{failedCount}} failed rows, see import result for details',
        importResultTitle: 'Import Result',
        importOutcomeCreated: 'Created',
        importOutcomeUpdated: 'Updated',
        importOutcomeSkipped: 'Skipped',
        importOutcomeFailed: 'Failed',
        importFilterAll: 'All',
        importColumnRowNumber: 'Row',
        importColumnOutcome: 'Outcome',
        importColumnReason: 'Failure Reason',
        versionHistoryAction: 'Version History',
        versionHistoryTitle: 'Material Version History',
        versionHistorySelectSingle: 'Select a single material first',
        versionHistoryEmpty: 'No version history',
        versionHistoryLoadFailed: 'Failed to load version history',
        versionHistoryColumnTime: 'Changed At',
        versionHistoryColumnSource: 'Source',
        versionHistoryColumnType: 'Type',
        versionHistoryColumnBatch: 'Import Batch',
        versionHistoryColumnRemark: 'Remark',
        versionHistoryColumnDiff: 'Field Changes',
        versionHistoryNoChange: 'No field changes',
        versionHistoryFieldEmpty: 'Empty',
        historySourceManual: 'Manual',
        historySourceImport: 'Import',
        historySourceRollback: 'Rollback',
        historyTypeCreated: 'Created',
        historyTypeUpdated: 'Updated',
        historyTypeDeleted: 'Deleted',
        historyTypeRollback: 'Rollback',
        importPreviewTitle: 'Import Preview',
        importPreviewFailed: 'Failed to preview import',
        importPreviewSummary:
          'Preview: {{totalRows}} total, {{createdCount}} created, {{updatedCount}} updated, {{skippedCount}} skipped, {{failedCount}} failed',
        importPreviewConfirm: 'Confirm Import',
        importPreviewEmpty: 'No importable rows in preview',
        importPreviewColumnChanges: 'Field Changes',
        importPreviewNoChange: 'No changes',
        importBatchNo: 'Import Batch No.',
        rollbackAction: 'Rollback Batch',
        rollbackConfirmTitle: 'Rollback Import Batch',
        rollbackConfirmContent:
          'Rollback batch {{batchNo}}? Created materials will be soft-deleted and updated materials will be restored to their pre-import values.',
        rollbackSuccess: 'Batch rollback completed',
        rollbackFailed: 'Failed to rollback batch',
        rollbackResultTitle: 'Rollback Result',
        rollbackTotal: 'Batch Rows',
        rollbackCreatedRolledBack: 'Created Rolled Back',
        rollbackUpdatedRestored: 'Updated Restored',
        rollbackMissing: 'Missing',
      },
      materialCategories: {
        materialCategory: 'Material Category',
        categoryDesc:
          'Manage material categories and whether purchase weighing is required.',
        keyword: 'Keyword',
        categoryPlaceholder: 'Search category code / name',
        categoryCode: 'Category Code',
        categoryName: 'Category Name',
        sortOrder: 'Sort Order',
        purchaseWeigh: 'Purchase Weigh',
        status: 'Status',
        remark: 'Remark',
        required: 'Required',
        notRequired: 'Not Required',
        categoryCount: 'Categories',
        enabled: 'Enabled',
      },
      payment: {
        payment: 'Payments',
        createPayment: 'Create payment',
        businessType: 'Business type',
        paymentPurpose: 'Payment purpose',
        statementSettlement: 'Statement settlement',
        purchasePrepayment: 'Purchase prepayment',
        supplier: 'Supplier',
        carrier: 'Carrier',
        draft: 'Draft',
        paid: 'Paid',
        paymentDate: 'Payment date',
        paymentNo: 'Payment no.',
        counterparty: 'Counterparty',
        counterpartyCode: 'Counterparty code',
        payType: 'Payment method',
        amount: 'Amount',
        status: 'Status',
        relatedStatement: 'Related statement',
        sourcePurchaseOrderId: 'Source purchase order ID',
        purchaseOrderNo: 'Purchase order no.',
        supplierCode: 'Supplier code',
        supplierName: 'Supplier name',
        settlementCompanyId: 'Settlement company ID',
        settlementCompany: 'Settlement company',
        parentPurchaseOrder: 'Purchase order',
        selectPurchaseOrder: 'Select purchase order',
        operator: 'Operator',
        remark: 'Remark',
        bankTransfer: 'Bank transfer',
        bankAcceptance: 'Bank acceptance',
        cash: 'Cash',
        paymentDesc:
          'Payments support statement settlement and purchase prepayment. Purchase prepayments import authoritative supplier and settlement snapshots from purchase orders.',
      },
      receipt: {
        receipt: 'Receipts',
        createReceipt: 'Create receipt',
        draft: 'Draft',
        received: 'Received',
        receiptDate: 'Receipt date',
        receiptNo: 'Receipt no.',
        customer: 'Customer',
        customerCode: 'Customer code',
        project: 'Project',
        receiptMethod: 'Receipt method',
        amount: 'Amount',
        status: 'Status',
        relatedStatement: 'Related statement',
        operator: 'Operator',
        remark: 'Remark',
        bankTransfer: 'Bank transfer',
        bankAcceptance: 'Bank acceptance',
        acceptanceBill: 'Acceptance bill',
        cash: 'Cash',
        receiptDesc:
          'Receipts handle customer collections. Received records must fully allocate to customer statements.',
      },
      customerStatement: {
        customerStatement: 'Customer Statements',
        generateStatement: 'Generate statement',
        viewCustomerSummary: 'View customer summary',
        period: 'Period',
        statementNo: 'Statement no.',
        customer: 'Customer',
        customerCode: 'Customer code',
        project: 'Project',
        startDate: 'Start date',
        endDate: 'End date',
        salesAmount: 'Sales amount',
        receiptAmount: 'Receipt amount',
        closingBalance: 'Closing balance',
        direction: 'Direction',
        blueDirection: 'Blue',
        redDirection: 'Red',
        sourceSalesReturnNo: 'Source return',
        status: 'Status',
        remark: 'Remark',
        relatedSalesOrders: 'Related Sales Orders',
        parentImportLabel: 'Sales Order',
        parentImportButton: 'Import SO Items',
        validationSelectCustomerFirst:
          'Select a customer before choosing sales orders',
        validationSameCustomer:
          'Only sales orders of the same customer can be imported',
        validationSameProject:
          'Only sales orders of the same project can be imported',
        customerStatementDesc:
          'Customer statements by customer and project; confirm to lock for settlement.',
      },
      freightStatement: {
        freightStatement: 'Freight Statements',
        generateFreightStatement: 'Generate freight statement',
        viewFreightSummary: 'View freight summary',
        pendingAudit: 'Pending audit',
        audited: 'Audited',
        unsigned: 'Unsigned',
        signed: 'Signed',
        period: 'Period',
        statementNo: 'Statement no.',
        carrier: 'Carrier',
        carrierCode: 'Carrier code',
        startDate: 'Start date',
        endDate: 'End date',
        totalWeight: 'Total weight (t)',
        totalFreight: 'Total freight',
        paidAmount: 'Paid amount',
        unpaidAmount: 'Unpaid amount',
        auditStatus: 'Audit status',
        signStatus: 'Sign status',
        attachment: 'Attachment',
        remark: 'Remark',
        freightStatementDesc:
          'Manage audited freight statements, signing status, attachments, and payment totals by carrier.',
      },
      freightOperations: {
        freightBill: 'Freight Bills',
        createFreightBill: 'Create freight bill',
        freightBillPlaceholder: 'Enter freight bill no.',
        unaudited: 'Unaudited',
        audited: 'Audited',
        documentDate: 'Document date',
        freightBillNo: 'Freight bill no.',
        carrier: 'Carrier',
        vehiclePlate: 'Vehicle plate',
        totalWeight: 'Total weight (t)',
        unitPrice: 'Unit price',
        totalFreight: 'Total freight',
        customerName: 'Customer',
        projectName: 'Project',
        auditStatus: 'Audit status',
        remark: 'Remark',
        freightBillDesc:
          'Manage carriers, customers, projects, shipment details, and freight charges independently.',
      },
      carrier: {
        title: 'Carriers',
        description:
          'Manage carrier master data including contact info, vehicle details, and pricing modes.',
        placeholderKeyword: 'Carrier code / name / contact',
        colCarrierCode: 'Carrier Code',
        colCarrierName: 'Carrier Name',
        colContactName: 'Contact',
        colContactPhone: 'Phone',
        colVehicleType: 'Vehicle Type',
        colPriceMode: 'Price Mode',
        colDefaultSettlementCompany: 'Default Settlement Entity',
        colDefaultSettlementCompanyId: 'Default Settlement Entity ID',
        colVehiclePlate: 'Plate',
        colVehicleContact: 'Driver',
        colVehiclePhone: 'Driver Phone',
        colVehicleRemark: 'Remark',
        colVehiclePlate2: 'Plate 2',
        colVehicleContact2: 'Driver 2',
        colVehiclePhone2: 'Driver Phone 2',
        colVehicleRemark2: 'Remark 2',
        colVehiclePlate3: 'Plate 3',
        colVehicleContact3: 'Driver 3',
        colVehiclePhone3: 'Driver Phone 3',
        colVehicleRemark3: 'Remark 3',
        formVehiclePlate: 'Plate',
        formVehicleContact: 'Driver',
        formVehiclePhone: 'Driver Phone',
        formVehicleRemark: 'Remark',
        formVehiclePlate2: 'Plate 2',
        formVehicleContact2: 'Driver 2',
        formVehiclePhone2: 'Driver Phone 2',
        formVehicleRemark2: 'Remark 2',
        formVehiclePlate3: 'Plate 3',
        formVehicleContact3: 'Driver 3',
        formVehiclePhone3: 'Driver Phone 3',
        formVehicleRemark3: 'Remark 3',
      },
      customer: {
        title: 'Customers',
        description:
          'Manage customer master data, related projects, contacts, and settlement modes.',
        placeholderKeyword: 'Customer code / name / project',
        colCustomerCode: 'Customer Code',
        colCustomerName: 'Customer Name',
        colProjectName: 'Related Projects',
        colProjectNameAbbr: 'Project Abbr',
        colProjectAddress: 'Project Address',
        colContactName: 'Contact',
        colContactPhone: 'Phone',
        colCity: 'City',
        colSettlementMode: 'Settlement',
        colDefaultSettlementCompany: 'Default Settlement Entity',
        colDefaultSettlementCompanyId: 'Default Settlement Entity ID',
        manageProjects: 'Manage Projects',
        settlementCash: 'Cash',
        settlementMonthly: 'Monthly',
        settlementCredit: 'Credit',
      },
      project: {
        title: 'Projects',
        description:
          'Manage project master data, customers, and project settlement entities.',
        placeholderKeyword: 'Project code / name / abbreviation / customer',
        projectCode: 'Project Code',
        projectName: 'Project Name',
        projectNameAbbr: 'Project Abbreviation',
        customer: 'Customer',
        settlementCompany: 'Settlement Entity',
        projectManager: 'Project Manager',
        projectAddress: 'Project Address',
        priceFloatMode: 'Net price float',
        priceFloatAdd: 'Markup',
        priceFloatSubtract: 'Markdown',
        priceFloatValue: 'Float amount (CNY/ton)',
        priceFloatValuePlaceholder: 'e.g. 30',
        quoteSource: 'Price source',
        quoteSourceMysteel: 'Mysteel',
        quoteSourceSteelx: 'SteelX (Xiben)',
        quoteRegion: 'Region',
      },
      purchaseOrder: {
        title: 'Purchase Orders',
        description:
          'Manage purchase orders by supplier, order date, and item details.',
        placeholderOrderNo: 'Enter order number',
        filterOrderDate: 'Order Date',
        colOrderNo: 'Order No',
        colSupplier: 'Supplier',
        colSettlementCompany: 'Purchase Settlement Entity',
        colSettlementCompanyId: 'Purchase Settlement Entity ID',
        colBuyer: 'Buyer',
        colOrderDate: 'Order Date',
        referencedBySalesOrder: 'Referenced by sales order',
        referencedByPurchaseInbound: 'Referenced by purchase inbound',
        formTotalWeight: 'Total Weight (ton)',
        formTotalAmount: 'Total Amount',
      },
      salesOrder: {
        title: 'Sales Orders',
        description:
          'Manage sales orders by customer, project, delivery date, and item details.',
        placeholderOrderNo: 'Enter order number',
        placeholderProductKeyword: 'Material code / brand / spec',
        placeholderPurchaseOrderNo: 'Auto-generated',
        filterProductKeyword: 'Product Keyword',
        filterDeliveryDate: 'Delivery Date',
        filterProjectName: 'Project',
        colPurchaseOrderNo: 'PO Number',
        colOrderNo: 'Order No',
        colCustomerName: 'Customer',
        colProjectName: 'Project',
        colSalesName: 'Salesperson',
        colDeliveryDate: 'Delivery Date',
        referencedByFreightBill: 'Referenced by freight bill',
        referencedBySalesOutbound: 'Referenced by sales outbound',
        referencedBySalesReturn: 'Referenced by sales return',
        colDeliveredQuantity: 'Delivered Qty',
        colReturnedQuantity: 'Returned Qty',
        colDeliveredNetQuantity: 'Net Delivered Qty',
        documentFlow: {
          action: 'Document Flow',
          title: 'Document Flow: {{no}}',
          empty: 'No document flow data',
          emptySection: 'No documents',
          relationsTitle: 'Document Relations',
          relationsEmpty: 'No document relations',
          loadFailed: 'Failed to load document flow',
          selectSingle: 'Select one sales order to view document flow',
          relationArrow: 'to',
        },
        parentImportLabel: 'Purchase Order',
        parentImportButton: 'Import PO Items',
        originalPlannedWeight: 'Original planned {{weight}} t',
        netPriceFillAll: 'Fill from net price',
        netPriceTitle: 'Assign unit price from net price',
        netPriceHint:
          'Matches net prices by delivery date and period, applies the project markup/markdown, and fills unit prices. Rows without a net price keep their value — fill them manually.',
        netPriceDate: 'Net price date',
        netPricePeriod: 'Period',
        netPricePeriodAll: 'All periods',
        netPriceApply: 'Fetch and fill',
        netPricePickDate: 'Please pick a net price date',
        netPriceRule: 'Price rule',
        netPriceRulePick: 'Select a price rule',
        netPricePickRule:
          'This project has multiple price rules; please pick one',
        netPriceFilled: 'Filled unit price for {{count}} rows from net price',
        netPriceNoPrice:
          '{{items}} and {{count}} row(s) have no net price; fill them manually',
        netPriceFailed: 'Failed to fetch net prices, please retry',
        netPriceFloatHint: 'Project rule: net price {{mode}} {{value}} CNY/ton',
      },
      salesContract: {
        title: 'Sales Contracts',
        description:
          'Maintain sales contracts and cap sales order amount/tonnage per project; warn before saving when exceeded.',
        placeholderKeyword: 'Contract no / name / customer / project',
      },
      purchaseInbound: {
        title: 'Purchase Inbounds',
        description: 'Manage purchase inbound and receiving confirmation.',
        placeholderInboundNo: 'Enter inbound number',
        placeholderParentImport: 'Enter PO number',
        filterInboundDate: 'Inbound Date',
        colInboundNo: 'Inbound No',
        colPurchaseOrderNo: 'PO Number',
        colSupplier: 'Supplier',
        colInboundDate: 'Inbound Date',
        colTotalWeighWeight: 'Weigh Weight',
        colWeightAdjustment: 'Weight Adj',
        formOrderNo: 'Inbound No',
        formInboundDate: 'Inbound Date',
        formBuyer: 'Buyer',
        formTotalWeight: 'Total Weight (ton)',
        formTotalAmount: 'Total Amount',
        parentImportLabel: 'Purchase Order',
        parentImportButton: 'Import PO Items',
        validationSameSupplier:
          'Cannot merge purchase orders from different suppliers',
        validationSameSettlementCompany:
          'Cannot merge purchase orders with different settlement entities',
        validationPurchaseOrderNoTooLong:
          'Merged PO number cannot exceed {{max}} characters',
      },
      salesOutbound: {
        title: 'Sales Outbounds',
        description: 'Manage sales outbound and shipment confirmation.',
        placeholderOutboundNo: 'Enter outbound number',
        placeholderParentImport: 'Enter SO number',
        placeholderProductKeyword: 'Material code / brand / spec',
        filterOutboundDate: 'Outbound Date',
        filterProductKeyword: 'Product Keyword',
        filterProjectName: 'Project',
        colOutboundNo: 'Outbound No',
        colSalesOrderNo: 'SO Number',
        colCustomerName: 'Customer',
        colProjectName: 'Project',
        colOutboundDate: 'Outbound Date',
        parentImportLabel: 'Sales Order',
        parentImportButton: 'Import SO Items',
      },
      salesReturn: {
        title: 'Sales Returns',
        description:
          'Sales returns are reverse sales outbound documents. They return part or all of the original outbound items without modifying the sales order, sales outbound, or freight bill.',
        placeholderReturnNo: 'Enter return number',
        placeholderSalesOrderNo: 'Filled from sales outbound',
        filterReturnNo: 'Return No',
        filterReturnDate: 'Return Date',
        filterProjectName: 'Project',
        colReturnNo: 'Return No',
        colSalesOrderNo: 'SO Number',
        colCustomerName: 'Customer',
        colProjectName: 'Project',
        colReturnDate: 'Return Date',
        parentImportLabel: 'Sales Outbound',
        parentImportButton: 'Import SO Outbound Items',
        sourceImport: {
          action: 'Create Return from Outbound',
          title: 'Create Sales Return from Audited Outbound',
          outboundPlaceholder: 'Select an audited sales outbound',
          returnDateLabel: 'Return date',
          noOutbound: 'No audited sales outbound found',
          confirm: 'Create Return',
          colMaterialCode: 'Material Code',
          colBrand: 'Brand',
          colMaterial: 'Material',
          colSpec: 'Spec',
          colOutboundQuantity: 'Outbound Qty',
          colReturnedQuantity: 'Returned Qty',
          colReturnableQuantity: 'Returnable Qty',
          colReturnQuantity: 'Return Qty',
          loadOutboundFailed: 'Failed to load sales outbounds',
          loadCandidatesFailed: 'Failed to load return candidates',
          noReturnableItems: 'No returnable items for this outbound',
          quantityRequired:
            'Select at least one line and enter a quantity greater than 0',
          quantityAriaLabel: 'Return quantity for row {{index}}',
          quantityOutOfRange:
            'Return quantity cannot exceed returnable quantity (max {{max}})',
          createSuccess: 'Sales return created',
          createFailed: 'Failed to create sales return',
        },
      },
      supplier: {
        title: 'Suppliers',
        description:
          'Manage supplier master data including contacts and settlement info.',
        placeholderKeyword: 'Supplier code / name / contact',
        colSupplierCode: 'Supplier Code',
        colSupplierName: 'Supplier Name',
        colShortName: 'Supplier Short Name',
        colBrands: 'Operating Brands',
        colContactName: 'Contact',
        colContactPhone: 'Phone',
        colCity: 'City',
        colSettlementMode: 'Settlement',
        settlementCash: 'Cash',
        settlementMonthly: 'Monthly',
        settlementCredit: 'Credit',
      },
      operationLog: {
        operationLog: 'Operation Logs',
        export: 'Export',
        allLogs: 'All Logs',
        authentication: 'Authentication',
        keyword: 'Keyword',
        operationLogPlaceholder:
          'Log No / Operator / Business No / Request Path',
        module: 'Module',
        action: 'Action',
        result: 'Result',
        success: 'Success',
        failed: 'Failed',
        authType: 'Auth Type',
        operationTime: 'Operation Time',
        logNo: 'Log No',
        operator: 'Operator',
        loginName: 'Login Name',
        businessNo: 'Business No',
        requestMethod: 'Request Method',
        requestPath: 'Request Path',
        clientIp: 'Client IP',
        remark: 'Remark',
        logCount: 'Logs',
        successCount: 'Successful Logs',
        failedCount: 'Failed Logs',
        operationLogDesc:
          'Audit write operations across the system, including operator, module, action, target, request path and result status.',
      },
      operationLogOptions: {
        query: 'Query',
        view: 'View',
        create: 'Create',
        edit: 'Edit',
        delete: 'Delete',
        audit: 'Audit',
        reverseAudit: 'Reverse Audit',
        statusChange: 'Status Change',
        completePurchase: 'Complete Purchase',
        completeSales: 'Complete Sales',
        deliveryVerification: 'Delivery Verification',
        returnToAudited: 'Return to Audited',
        export: 'Export',
        print: 'Print',
        login: 'Login',
        loginFailed: 'Login Failed',
        logout: 'Logout',
        changePassword: 'Change Password',
        execute: 'Execute',
        save: 'Save',
        saveAndAudit: 'Save and Audit',
        saveAndConfirm: 'Save and Confirm',
        uploadJson: 'Upload JSON',
        exportBackup: 'Export Backup',
        importBackup: 'Import Backup',
        uploadAttachment: 'Upload Attachment',
        generateAttachmentUploadUrl: 'Generate Direct Upload URL',
        completeAttachmentDirectUpload: 'Complete Direct Upload',
        exportAttachmentRecoveryManifest: 'Export Recovery Manifest',
        common: 'Common',
        authentication: 'Authentication',
        currentAccount: 'Personal Account',
        database: 'Database Management',
        system: 'System',
        companyInfo: 'Settlement Entity Management',
        printTemplates: 'Print Templates',
        operationLog: 'Operation Logs',
        printModule: 'Printing',
        attachmentManagement: 'Attachment Management',
        business: 'Business',
        materials: 'Materials',
        supplier: 'Suppliers',
        customer: 'Customers',
        carrier: 'Carriers',
        warehouse: 'Warehouses',
        purchaseOrder: 'Purchase Orders',
        purchaseInbound: 'Purchase Inbound',
        salesOrder: 'Sales Orders',
        salesOutbound: 'Sales Outbound',
        salesReturn: 'Sales Returns',
        freightBill: 'Freight Bills',
        customerStatement: 'Customer Statements',
        freightStatement: 'Freight Statements',
        receipt: 'Receipts',
        payment: 'Payments',
      },
      systemCore: {
        keyword: 'Keyword',
        status: 'Status',
        remark: 'Remark',
        companyInfo: 'Settlement Entity Management',
        companyPlaceholder: 'Entity name / tax no / bank / account',
        companyName: 'Settlement Entity Name',
        taxNo: 'Tax No',
        companyInfoDesc:
          'Maintain settlement entity names, tax numbers, and bank accounts for customers, carriers, purchase orders, and finance pages.',
      },
      masterWarehouse: {
        warehouseMaster: 'Warehouses',
        keyword: 'Keyword',
        warehousePlaceholder: 'Search warehouse code / name',
        warehouseType: 'Warehouse Type',
        ownWarehouse: 'Own Warehouse',
        partnerWarehouse: 'Partner Warehouse',
        transitWarehouse: 'Transit Warehouse',
        thirdPartyWarehouse: 'Third-party Warehouse',
        warehouseCode: 'Warehouse Code',
        warehouseName: 'Warehouse Name',
        contact: 'Contact',
        phone: 'Phone',
        address: 'Address',
        status: 'Status',
        remark: 'Remark',
        warehouseDesc:
          'Manage warehouses including type, contact, and address.',
      },
    },
  },
  hooks: {
    batchActions: {
      pleaseSelectRecords: 'Please select records first',
      noBatchStatus:
        'Batch {{action}} status is not configured for this module',
      actionNotSupported:
        'Selected records are in a status that does not support {{action}}',
      batchAction: 'Batch {{action}}',
      batchActionConfirm:
        'Apply {{action}} to {{count}} selected record(s)?{{skippedPart}}',
      actionFailed: '{{action}} failed',
      actionCompletedWithFailures:
        '{{action}} completed: {{successCount}} succeeded, {{failedCount}} failed{{skippedPart}}{{errorPart}}',
      actionSuccess:
        '{{action}} succeeded for {{successCount}} record(s){{skippedPart}}',
      skippedPart:
        ' {{count}} record(s) will be skipped due to unsupported status.',
      skippedCount: ', {{count}} skipped',
      deleteNotSupported:
        'Selected records are in a status that does not support deletion',
      batchDelete: 'Batch Delete',
      batchDeleteConfirm:
        'Confirm deletion of {{count}} selected record(s)? This action cannot be undone.{{skippedPart}}',
      deleteFailed: 'Deletion failed',
      deleteCompletedWithFailures:
        'Deletion completed: {{successCount}} succeeded, {{failedCount}} failed{{skippedPart}}{{errorPart}}',
      deleteSuccess:
        'Deletion succeeded for {{successCount}} record(s){{skippedPart}}',
    },
    printActions: {
      selectPrintTemplate: 'Select Print Template',
      noPrintTemplateConfigured:
        'No print template configured for this document',
      noPrintableRecords: 'No printable records found in selection',
      singleRecordOnly: 'Printing supports one document at a time only',
      printScriptGenerationFailed: 'Print script generation failed',
      printServiceUnavailable:
        'Print service unavailable. Please check CLodop or print template configuration',
      noPrintContent: 'No print content generated',
      printFailed: 'Print failed',
      exportXlsxFailed: 'Print Excel export failed',
    },
    freightActions: {
      noFreightData: 'No freight statement data in the current list',
      freightSummaryTitle: 'Freight Reconciliation Summary',
      documentCount: 'Documents in list: {{count}}',
      totalWeight: 'Total weight (tons): ',
      totalFreight: 'Total freight: ',
      paidAmount: 'Paid amount: ',
      unpaidAmount: 'Unpaid amount: ',
    },
    customerActions: {
      noCustomerData: 'No customer statement data in the current list',
      customerSummaryTitle: 'Customer Reconciliation Summary',
      documentCount: 'Documents in list: {{count}}',
      salesAmount: 'Sales amount: ',
      receiptAmount: 'Receipt amount: ',
      closingAmount: 'Closing balance: ',
    },
    customerProjectActions: {
      title: 'Projects for {{name}}',
      create: 'Add Project',
      edit: 'Edit Project',
      delete: 'Delete Project',
      saveSuccess: 'Project saved',
      deleteSuccess: 'Project deleted',
      code: 'Project Code',
      name: 'Project Name',
      abbreviatedName: 'Project Abbreviation',
      address: 'Project Address',
      manager: 'Project Manager',
      status: 'Status',
      remark: 'Remark',
      operations: 'Actions',
      deleteConfirm: 'Delete this project?',
      selectedCustomerRequired: 'Select one customer record',
    },
    toolbarActions: {
      delete: 'Delete',
      printPreview: 'Print Preview',
      directPrint: 'Direct Print',
      noExtraLogic: '{{label}} has no additional handling logic.',
    },
    recordActions: {
      view: 'View',
      edit: 'Edit',
      attachment: 'Attachments',
      audit: 'Audit',
      reverseAudit: 'Reverse Audit',
      reopenDeliveryVerification: 'Reopen Verification',
      confirmDeliveryVerification: 'Confirm Verification',
    },
    requestError: {
      requestFailed: 'Request failed',
    },
    pagination: {
      total: '{{count}} total',
    },
    displaySupport: {
      yes: 'Yes',
      no: 'No',
    },
    excelExport: {
      exportSuccess: 'Export completed',
      exportFailed: 'Export failed',
    },
    openPages: {
      unnamedPage: 'Unnamed Page',
      workbench: 'Workbench',
    },
    gridColumns: {
      actions: 'Actions',
      detail: 'View details',
    },
    columnSettings: {
      syncRetryLater:
        'Column settings saved locally. Cloud sync will retry later.',
    },
    statement: {
      noCandidateDocuments:
        'No candidate documents available for generation with current filters',
    },
    recordHelpers: {
      currentUser: 'Current User',
    },
  },
  layouts: {
    userMenu: {
      personalSettings: 'Personal Settings',
      logout: 'Sign Out',
    },
    personalSettings: {
      title: 'Personal Settings',
      displayTab: 'Display',
      accountTab: 'Account Profile',
      passwordTab: 'Change Password',
    },
    settings: {
      systemFont: 'System Font',
      systemFontDefault: 'System Default',
      fontSize: 'Font Size',
      navLayout: 'Navigation Layout',
      themeMode: 'Theme Mode',
      resetDefault: 'Reset Default',
      saveDisplay: 'Save Display Settings',
      theme: {
        light: 'Light',
        dark: 'Dark',
        system: 'System',
      },
      layout: {
        sider: 'Side Navigation',
        top: 'Top Navigation',
        siderDesc:
          'Keeps the side menu, better suited for high-frequency data entry and multi-module switching.',
        topDesc:
          'Uses a top menu bar, consistent with the current Vue version layout.',
      },
    },
    sideNav: {
      root: 'Business Center',
      apiOnline: 'API Online',
      apiOffline: 'API Offline',
    },
    breadcrumb: {
      ariaLabel: 'Breadcrumb',
    },
    headerSearch: {
      placeholder: 'Search order, contract, or statement number',
      viewFlow: 'View document flow',
    },
    userInfo: {
      notLoggedIn: 'Not Logged In',
      currentAccount: 'Current Account',
    },
    routePage: {
      businessPageNotFound: 'Business page not found',
    },
    tabs: {
      refresh: 'Refresh page',
      close: 'Close tab',
      closeOthers: 'Close other tabs',
      closeRight: 'Close tabs to the right',
      closeAll: 'Close all tabs',
      pinDashboard: 'Dashboard is pinned',
    },
  },
  system: {
    company: {
      title: 'Settlement Entity Management',
      loadFailed: 'Failed to load settlement entities',
      saveSuccess: 'Settlement entity information saved',
      enterpriseMode: 'Entity Maintenance',
      singleEnterprise: 'Multiple Settlement Entities',
      subjectStatus: 'Subject Status',
      settlementBanks: 'Settlement Banks',
      supplementNote: 'Additional Notes',
      subjectList: 'Settlement Entities',
      addSubject: 'Add Entity',
      noSubjects: 'No settlement entities',
      deleteSubject: 'Delete Settlement Entity',
      deleteSubjectConfirm:
        'Delete this settlement entity? Entities referenced by customers, carriers, or purchase orders cannot be deleted.',
      deleteSubjectDirtyConfirm:
        'This settlement entity has unsaved changes. Deleting it will discard those changes, and entities referenced by business data still cannot be deleted. Continue?',
      activeSubjects: 'Active Entities',
      atLeastOneSettlementAccount: 'Settlement accounts are optional',
      inputAccountName:
        'Please enter account name for settlement account {{index}}',
      inputBankName: 'Please enter bank name for settlement account {{index}}',
      inputBankAccount:
        'Please enter bank account for settlement account {{index}}',
      duplicateBankAccount: 'Duplicate bank account: {{account}}',
      countUnit: '{{count}} items',
      settlementInfo: 'Settlement Information',
      addBank: 'Add Bank',
      noSettlementAccounts:
        'No settlement accounts maintained. Add one when needed.',
      accountName: 'Account Name',
      accountNamePlaceholder: 'e.g. Basic Account / Collection Account',
      usageType: 'Usage Type',
      usageGeneral: 'General',
      usageReceive: 'Receive',
      usagePay: 'Pay',
      bankName: 'Bank Name',
      bankNamePlaceholder: 'Enter bank name',
      bankAccount: 'Bank Account',
      bankAccountPlaceholder: 'Enter bank account',
      statusNormal: 'Normal',
      statusDisabled: 'Disabled',
      remarkPlaceholder: 'Account usage or description',
      subjectRemarkPlaceholder:
        'Subject header, settlement habits or financial notes',
    },
    printTemplate: {
      title: 'Print Templates',
      loadFailed: 'Failed to load print templates',
      description:
        'Manage print templates by document type and settlement company, with preview, copy, and file synchronization support.',
      newTemplate: 'New Template',
      editTemplate: 'Edit Template',
      deleteTemplate: 'Delete Print Template',
      preview: 'Preview',
      copy: 'Copy',
      searchPlaceholder: 'Search template name, code, type, or company',
      emptyList: 'No print templates',
      templateContent: 'Template Content',
      templateName: 'Template Name',
      templateCode: 'Template Code',
      billType: 'Bill Type',
      templateType: 'Template Type',
      engine: 'Engine',
      assetRef: 'Asset Ref',
      settlementCompany: 'Settlement Company',
      unassignedCompany: 'Unassigned Company',
      status: 'Status',
      statusActive: 'Active',
      statusDisabled: 'Disabled',
      syncMode: 'Sync Mode',
      syncModeManual: 'Manual',
      syncModeFile: 'File Managed',
      sourceRef: 'Source File',
      fileManagedEditHint:
        'File managed templates should be changed in the source file and synced after backend restart',
      uploadJson: 'Upload JSON',
      uploadJsonSuccess: 'Template JSON uploaded',
      uploadJsonFailed: 'Failed to upload template JSON',
      uploadPdfFormOnly: 'Only PDF form templates support JSON upload',
      uploadJsonFileOnly: 'Please upload a JSON file',
      uploadJsonSizeLimit: 'JSON file must be 1MB or smaller',
      uploadJsonHint:
        'File managed templates are managed by deployment files and cannot be uploaded. Copy it as a manual template first.',
      deleteContent: 'Delete template "{{name}}"?',
      inputTemplateContent: 'Please enter template content',
    },
    account: {
      title: 'Personal Account',
      profileSection: 'Account Profile',
      loginName: 'Login Name',
      userName: 'User Name',
      mobile: 'Mobile',
      remark: 'Remark',
      saveProfile: 'Save Profile',
      profileSaved: 'Profile saved',
      profileSaveFailed: 'Failed to save profile',
      passwordSection: 'Change Password',
      currentPassword: 'Current Password',
      newPassword: 'New Password',
      confirmPassword: 'Confirm New Password',
      changePassword: 'Update Password',
      passwordChanged: 'Password updated. Please log in again.',
      passwordChangeFailed: 'Failed to update password',
      passwordMismatch: 'The new passwords do not match',
      rolesSection: 'Roles',
      rolesHint:
        'Roles decide which features this account can access. Permissions are maintained by administrators under Role Management.',
      rolesLabel: 'Assigned Roles',
      rolesPlaceholder: 'Select roles',
      saveRoles: 'Save Roles',
      rolesSaved: 'Roles saved',
      rolesSaveFailed: 'Failed to save roles',
    },
    role: {
      title: 'Role Management',
      description:
        'Maintain roles and their permissions. Built-in roles cannot be deleted or have their code changed.',
      loadFailed: 'Failed to load roles',
      searchPlaceholder: 'Search role code or name',
      emptyList: 'No roles',
      newRole: 'New Role',
      wizardStepBasic: 'Basic Info',
      wizardStepPermissions: 'Permissions',
      wizardNext: 'Next',
      wizardPrevious: 'Previous',
      cloneRole: 'Clone Role',
      cloneNameSuffix: ' Copy',
      editRole: 'Edit Role',
      deleteRole: 'Delete Role',
      deleteContent: 'Delete role "{{name}}"? This cannot be undone.',
      code: 'Role Code',
      codePlaceholder: 'Enter role code',
      name: 'Role Name',
      namePlaceholder: 'Enter role name',
      descriptionLabel: 'Description',
      descriptionPlaceholder: 'Enter role description',
      status: 'Status',
      statusFilter: 'Filter by status',
      statusNormal: 'Normal',
      statusDisabled: 'Disabled',
      builtin: 'Built-in',
      builtinHint: 'Built-in roles cannot be deleted',
      builtinCodeReadonly: 'The code of a built-in role cannot be changed',
      permissionCount: 'Permissions',
      userCount: 'Users',
      permissions: 'Permission Matrix',
      permissionMatrixTitle: 'Permission Matrix - {{name}}',
      permissionsLoadFailed: 'Failed to load permissions',
      permissionSearchPlaceholder:
        'Search resources or permissions, e.g. "Sales Orders" or "Audit"',
      selectAllConfirmTitle: 'Grant all permissions?',
      selectAllConfirmContent:
        'This selects every permission in the catalog, including destructive ones (delete, rollback, rebuild). Continue?',
      permissionSelectedCount: '{{count}} permission(s) selected',
      selectAll: 'Select All',
      clearAll: 'Clear',
      wildcardNotice:
        'This role holds the global wildcard permission (*) as a super administrator. Maintain it separately in the permission catalog.',
      permissionsSaved: 'Permissions saved',
      permissionsSaveFailed: 'Failed to save permissions',
      enable: 'Enable',
      disable: 'Disable',
      enableConfirmTitle: 'Enable Role',
      disableConfirmTitle: 'Disable Role',
      statusConfirmContent: 'Change the status of role "{{name}}"?',
      statusUpdated: 'Role status updated',
      statusUpdateFailed: 'Failed to update role status',
      actions: {
        read: 'Read',
        create: 'Create',
        update: 'Update',
        delete: 'Delete',
        write: 'Write',
        audit: 'Audit',
        unaudit: 'Unaudit',
        complete: 'Complete',
        confirm: 'Confirm',
        print: 'Print',
        export: 'Export',
        import: 'Import',
        preview: 'Preview',
        backfill: 'Backfill',
        rollback: 'Rollback',
        rebuild: 'Rebuild',
        sync: 'Sync',
      },
      resources: {
        'sales-orders': 'Sales Orders',
        'sales-contracts': 'Sales Contracts',
        'sales-returns': 'Sales Returns',
        'sales-outbounds': 'Sales Outbounds',
        materials: 'Materials',
        'material-imports': 'Material Imports',
        'import-batches': 'Import Batches',
        inventory: 'Inventory',
        'customer-statements': 'Customer Statements',
        customers: 'Customers',
        suppliers: 'Suppliers',
        warehouses: 'Warehouses',
        carriers: 'Carriers',
        projects: 'Projects',
        'quote-sheets': 'Quote Sheets',
        'steel-quotes': 'Steel Quotes',
        'purchase-orders': 'Purchase Orders',
        'purchase-inbounds': 'Purchase Inbounds',
        'freight-bills': 'Freight Bills',
        finance: 'Finance',
        receipts: 'Receipts',
        payments: 'Payments',
        'ledger-adjustments': 'Ledger Adjustments',
        attachments: 'Attachments',
        'attachment-upload-sessions': 'Upload Sessions',
        'attachment-manifest-exports': 'Manifest Exports',
        'freight-statements': 'Freight Statements',
        'material-categories': 'Material Categories',
        'material-exports': 'Material Exports',
        'steel-quote-syncs': 'Quote Syncs',
        'steel-quote-backfills': 'Quote Backfills',
        'steel-quote-calendars': 'Quote Calendars',
        'material-price-matches': 'Price Matches',
        'cash-ledger': 'Cash Ledger',
        'print-templates': 'Print Templates',
        'print-exports': 'Print Exports',
        'print-previews': 'Print Previews',
        'company-settings': 'Settlement Entities',
        dashboard: 'Dashboard',
        'operation-logs': 'Operation Logs',
        'global-search': 'Global Search',
        meta: 'Metadata',
        'code-issuances': 'Code Issuances',
        'user-accounts': 'User Accounts',
        roles: 'Roles',
        permissions: 'Permissions',
        system: 'System',
      },
      fields: {
        amount: 'Amount',
        'unit-price': 'Unit Price',
        cost: 'Cost',
      },
    },
    companyHeader: {
      title: 'Settlement Entity Management',
      description:
        'Maintain settlement entity name, tax number, status, and bank accounts. Customers, carriers, and purchase orders can choose the corresponding settlement entity.',
    },
    userAccount: {
      title: 'User Accounts',
      description:
        'Maintain login accounts, enablement status, and role assignments. Passwords are reset by administrators.',
      loadFailed: 'Failed to load user accounts',
      searchPlaceholder: 'Search login name, user name, or mobile',
      emptyList: 'No user accounts',
      newUser: 'New User',
      wizardStepBasic: 'Basic Info',
      wizardStepRoles: 'Role Assignment',
      wizardNext: 'Next',
      wizardPrevious: 'Previous',
      effectivePermissions: 'Effective Permissions',
      effectivePermissionsEmpty: 'No role selected; no effective permissions',
      effectivePermissionsWildcard: 'Includes all permissions (*)',
      editUser: 'Edit User',
      deleteUser: 'Delete User',
      deleteContent:
        'Delete user "{{name}}"? The account will be disabled and cannot be restored.',
      deleteSelfConfirm:
        'This is the current login account. The backend will reject the deletion. Continue anyway?',
      deleteSelfForbidden: 'The current login account cannot be deleted',
      loginName: 'Login Name',
      loginNamePlaceholder: 'Enter login name',
      userName: 'User Name',
      userNamePlaceholder: 'Enter user name',
      password: 'Password',
      passwordPlaceholder: 'Enter password',
      passwordStrength:
        'Password must be at least 8 characters and contain both letters and digits',
      newPassword: 'New Password',
      newPasswordPlaceholder: 'Enter new password',
      confirmPassword: 'Confirm New Password',
      confirmPasswordPlaceholder: 'Enter the new password again',
      passwordMismatch: 'The new passwords do not match',
      mobile: 'Mobile',
      mobilePlaceholder: 'Enter mobile number',
      status: 'Status',
      statusFilter: 'Filter by status',
      statusNormal: 'Normal',
      statusDisabled: 'Disabled',
      lastLoginDate: 'Last Login',
      enable: 'Enable',
      disable: 'Disable',
      enableConfirmTitle: 'Enable User',
      disableConfirmTitle: 'Disable User',
      statusConfirmContent: 'Change the status of user "{{name}}"?',
      statusUpdated: 'User status updated',
      statusUpdateFailed: 'Failed to update user status',
      resetPassword: 'Reset Password',
      resetPasswordTitle: 'Reset Password - {{name}}',
      resetPasswordHint:
        'After reset, the account must log in again with the new password.',
      passwordResetSuccess: 'Password reset',
      passwordResetFailed: 'Failed to reset password',
      assignRoles: 'Assign Roles',
      assignRolesTitle: 'Assign Roles - {{name}}',
      rolesLabel: 'Assigned Roles',
      rolesPlaceholder: 'Select roles',
      rolesSaved: 'Roles saved',
      rolesSaveFailed: 'Failed to save roles',
      rolesUnavailable:
        'Unable to load the role list or account roles. The role read permission may be missing.',
    },
    companySubject: {
      sectionTitle: 'Entity Profile',
      companyName: 'Settlement Entity Name',
      companyNamePlaceholder: 'Enter settlement entity name',
      taxNo: 'Tax Number',
      taxNoPlaceholder: 'Enter tax number',
      status: 'Status',
      statusNormal: 'Normal',
      statusDisabled: 'Disabled',
      pendingCompany: 'Settlement entity pending configuration',
      pendingTaxNo: 'Tax number pending configuration',
      settlementBanks: 'Settlement Banks',
      unitSuffix: '',
    },
    printTemplateEditor: {
      editTitle: 'Edit Template',
      createTitle: 'New Template',
      basicInfo: 'Basic Info',
      availableFields: 'Available Fields',
      billType: 'Bill Type',
      templateName: 'Template Name',
      templateNamePlaceholder: 'Enter template name',
      templateCode: 'Template Code',
      templateCodePlaceholder: 'Leave empty to auto-generate',
      templateType: 'Template Type',
      templateTypeCoord: 'Coordinate Overlay',
      templateTypePdfForm: 'PDF Form',
      engine: 'Engine',
      engineLodop: 'Lodop Coordinate Overlay',
      enginePdfForm: 'PDF Form Fill',
      assetRef: 'PDF Asset (Optional)',
      assetRefPlaceholder: 'Leave empty for pure JSON templates',
      settlementCompany: 'Settlement Company',
      settlementCompanyPlaceholder: 'Select settlement company',
      versionNo: 'Version',
      status: 'Status',
      statusActive: 'Active',
      statusDisabled: 'Disabled',
      templateContent: 'Template Content',
      templateContentPlaceholder: 'Enter template content',
      templateHint:
        'COORD uses overlay script or shared layout JSON. PDF_FORM generates PDF from pure JSON and can optionally reference a PDF asset.',
      helpTitle: 'Print Template Guide',
      unifiedPrintApi:
        'Print APIs follow the v2.0 contract: GET /v2.0/print-templates lists templates, PUT /v2.0/print-templates/{id}/content uploads template JSON, GET /v2.0/print-previews/items loads print items, and POST /v2.0/print-exports generates print output. COORD returns overlay script and data, PDF_FORM returns PDF content. The frontend only previews and prints.',
      syntaxHint:
        'Use {{fieldName}} for fields, {{#each details}}...{{/each}} for detail rows, and {{#if fieldName}}...{{else}}...{{/if}} for conditional blocks.',
      commonFields: 'Header fields',
      detailFields: 'Detail fields',
      layoutFields: 'Backend-generated fields',
    },
    printTemplatePreview: {
      title: 'Template Preview',
      billType: 'Bill Type: ',
      emptyTemplate: '(Empty Template)',
    },
    printTemplateUtils: {
      copySuffix: '(Copy)',
    },
  },
  print: {
    defaultTitle: 'Print',
    clodopLicenseInjectFailed: 'CLodop license injection failed',
    clodopTemplatePrintFailed: 'CLodop template print failed',
    clodopPrintFailed: 'CLodop print failed',
    printTimeLabel: 'Print Time: ',
  },
  error: {
    retry: 'Retry',
    accessDenied: 'Access denied',
    serverBusy: 'Server is busy, please try again later',
    networkError: 'Network connection error, please check your network',
    requestTimeout: 'Request timed out, please try again later',
    serverResponseError: 'Server response error, please try again later',
    unknownError: 'An unknown error occurred, please click retry',
    serverError: {
      title: 'Cannot Connect to Server',
      subTitle:
        'The server is temporarily unavailable. Please check your network connection or try again later.',
    },
  },
  emptyState: {
    noData: {
      title: 'No Records Yet',
      description:
        'This list is empty. Create your first record to get started.',
      createFirstRecord: 'Create First Record',
    },
    noResult: {
      title: 'No Matching Records',
      description:
        'No records match the current filters. Try adjusting or clearing the filters.',
      resetFilters: 'Clear Filters',
    },
    noPermission: {
      title: 'No Access',
      description: 'You do not have permission to view this content.',
      hint: 'Contact your administrator to request access.',
    },
    error: {
      title: 'Failed to Load Data',
      description:
        'Something went wrong while loading data. Check your connection and try again.',
      retry: 'Retry',
    },
  },
  documentFlow: {
    title: 'Document Flow',
    titleWithNo: 'Document Flow: {{no}}',
    documentCount: '{{count}} document(s)',
    loadFailed: 'Failed to load document flow',
    empty: 'No flow available for this document number',
    nodeHint: 'Click a node to open the document',
    openDocument: 'Open {{no}}',
    amount: 'Amount',
    weight: 'Weight',
    types: {
      purchaseOrder: 'Purchase Order',
      purchaseInbound: 'Purchase Inbound',
      salesOrder: 'Sales Order',
      salesOutbound: 'Sales Outbound',
      salesReturn: 'Sales Return',
      freightBill: 'Freight Bill',
    },
    linkTypes: {
      inbound: 'Inbound',
      sales: 'Sales',
      outbound: 'Outbound',
      returns: 'Return',
      freight: 'Freight',
    },
  },
  priceCompare: {
    view: {
      title: 'Order Price Comparison',
      watermark: 'INTERNAL · Order Price Comparison',
      noBatch: 'No batch',
      removeSheetConfirmTitle: 'Delete this batch?',
      removeSheetConfirmContent:
        'All entries in this batch will also be deleted',
      editingByMe: 'You are editing this batch',
      editingByOther: '{{name}} is editing this batch',
      editingByOtherUnknown: 'Another user is editing this batch',
      requestTakeover: 'Request takeover',
      takeoverConfirmTitle: 'Take over editing?',
      takeoverConfirmContent:
        'Takeover acquires the edit right for this batch. If the current editor is still active it will conflict again; retry after the lock expires.',
      takeoverFailed: 'The other user is still editing. Try again later.',
    },
    picker: {
      project: 'Project',
      addProjectBatch: 'Add project batch',
      batch: 'Batch',
      newBatch: 'New Batch',
      deleteBatch: 'Delete Batch',
      keepOneBatch: 'Keep at least one batch',
      deleteCurrentBatch: 'Delete current batch',
    },
    sheet: {
      dateFormat: 'YYYY-MM-DD',
      dragRow: 'Drag to reorder rows',
      dragBrand: 'Drag to reorder brand columns',
      columns: {
        remark: 'Remark',
        brand: 'Brand',
        category: 'Category',
        variety: 'Material / Spec / Length',
        ton: 'Order Tonnage',
        net: 'Market Price',
        spot: 'Spot Price',
        diff: 'Difference',
        supplierShort: 'Abbr.',
        purchaseOrderNo: 'Order No.',
        purchaseOrderSupplier: 'Supplier',
        purchaseOrderOrdered: 'Ordered',
        purchaseOrderIssued: 'Issued',
        purchaseOrderRemaining: 'Remaining',
        purchaseOrderStatus: 'Status',
      },
      selectProduct: 'Select product',
      switchLength: 'Switch to {{length}}',
      lockRow: 'Lock row',
      unlockRow: 'Unlock row',
      lockRowHint:
        'Lock to link a purchase order; unlocking clears the linked purchase order',
      purchaseOrderLockFirst: 'Lock this row before linking a purchase order',
      tonPositive: 'Order tonnage must be a positive number',
      spotOutOfRange: 'Spot price is out of range (0 - {{max}})',
      spotBetter: 'Spot price is better',
      netBetter: 'Market price is better',
      addRow: '+ Add Row',
      tonTotal: 'Total',
      columnSettings: 'Columns',
      addSeparator: '+ Add Separator',
      separator: 'Separator',
      removeSelectedTitle: 'Delete {{selected}} selected row(s)?',
      unspecifiedProject: 'Unspecified project',
      orderDate: 'Order Date',
      remark: 'Remark',
      lock: 'Locked. Unlock to edit.',
      unlock: 'Unlock',
      clear: 'Clear',
      refPrice: 'Reference Price',
      refDateTooltip:
        'All rows use this date and period as the market price reference',
      fallbackLegend: 'HRB400E reference price',
      fallbackTooltip:
        'No HRB400 quote for this brand/spec; using the HRB400E reference price',
      bestDiff: 'Best Difference',
      refreshPrice: 'Refresh Prices',
      config: 'Settings',
      lockRef: 'Lock Reference',
      unlockRef: 'Unlock Reference',
      lockRefTooltip:
        'Lock the reference date/period to prevent refresh or accidental edits; market prices still update on refresh',
      unlockRefTooltip: 'Unlock to change the reference date/period',
      lockSpecQuantity: 'Lock Spec & Quantity',
      unlockSpecQuantity: 'Unlock Spec & Quantity',
      lockSpecQuantityTooltip:
        'Lock the product spec and quantity inputs to prevent accidental edits; independent of reference lock',
      unlockSpecQuantityTooltip: 'Unlock to edit product spec and quantity',
      specQuantityLockedHint:
        'Spec & quantity locked; adding, deleting, or reordering rows is disabled until unlocked',
      supplier: 'Supplier',
      fillSupplier: 'Bulk fill supplier',
      fillSupplierPick: 'Select supplier',
      fillSupplierBrand: 'Select brand column',
      fillSupplierColumn:
        'Fill the selected supplier into all product rows of the "{{brand}}" column',
      fillSupplierSelected:
        'Fill the selected supplier into the {{count}} selected rows',
      fillSupplierNoChangedRows:
        'None of the target rows had a spot price changed this session; nothing was modified (only rows with changed prices are updated)',
      purchaseOrderLabel: 'Linked purchase order',
      purchaseOrderPlaceholder: 'Link PO',
      purchaseOrderOptionTitle:
        '{{orderNo}} · {{supplier}} ({{status}}) | ordered {{ordered}}t | remaining {{remaining}}t',
      purchaseOrderTonnageHint: 'Issued {{issued}} / left {{remaining}}',
      purchaseOrderOverLimit: '(over)',
      purchaseOrderSavedBasis:
        'Based on saved sheets only; unsaved edits are not counted',
      purchaseOrderMissing: '{{orderNo}} (deleted)',
      purchaseOrderMissingHint: 'Linked order was deleted; please re-select',
      purchaseOrderLoadFailed:
        'Failed to load purchase-order tonnage; issued/remaining is unavailable (entry and saving are unaffected)',
      purchaseOrderIssuedShort: 'Issued {{issued}}',
      purchaseOrderDetail: 'Purchase order details',
      purchaseOrderUnlinked: 'No linked purchase order',
      purchaseOrderOverLimitLong: 'Entry tonnage exceeds the order remaining',
      purchaseOrderPickerOpen: 'Select purchase order',
      purchaseOrderPickerTitle: 'Select purchase order line',
      purchaseOrderPickerSearch: 'Search order no., supplier or spec',
      purchaseOrderPickerEmpty: 'No purchase orders available',
      purchaseOrderPickerClear: 'Clear link',
      purchaseOrderPickerClearHint: 'Unlink this row from the purchase order',
      purchaseOrderPickerHint:
        'Click a row to link it; issued tonnage is based on saved sheets.',
    },
    config: {
      title: 'Project Settings',
      tabs: {
        basic: 'Basic',
        brands: 'Brands & Categories',
        products: 'Available Products',
      },
      premiumLabel: '12m premium (CNY/ton, rebar only)',
      fallbackLabel:
        'Use the HRB400E reference price when HRB400 has no quote (marked with E)',
      brandsTitle: 'Participating Brands',
      brandsHint:
        'Brands and categories in comparison (disabled categories are hidden)',
      designatedBrands: 'Designated Brands',
      designatedBrandsHint:
        'Designated brands (display only, excluded from automatic filtering)',
      designatedBadge: 'Designated',
      selectedCount: 'Selected {{selected}}/{{total}}',
      selectAll: 'Select All',
      clear: 'Clear',
      selectedBrands: 'Selected brands ({{selected}})',
      noBrandSelected: 'No brand selected yet',
      freight: 'Freight (CNY/ton)',
      category: 'Category',
      unselectedBrands: 'Unselected brands (click to add)',
      searchBrand: 'Search brand',
      none: 'None',
      productsTitle: 'Available Products',
      productsHint:
        'Products selectable when ordering (all by default; uncheck to remove)',
      material: 'Material',
      all: 'All',
      searchSpec: 'Search spec',
      noMatchedProduct: 'No matching product',
      discardTitle: 'Discard unsaved changes?',
      abandon: 'Discard',
      continueEditing: 'Keep Editing',
    },
    pricing: {
      loginRequired: 'Please sign in before reading prices',
      refreshedLatest: 'Latest prices loaded',
      refreshedWithRef: 'Prices loaded for {{ref}}',
      refreshFailed: 'Failed to load prices: {{message}}',
      retryLater: 'Please try again later',
    },
    tour: {
      selectTitle: 'Select project/batch',
      selectDesc:
        'Switch project and batch at the top; each batch is one quote',
      entryTitle: 'Entry and Comparison',
      entryDesc:
        'Drag to reorder rows; enter spot prices and use Enter or arrow keys to switch rows',
    },
  },
  pages: {
    material: 'Materials',
    'material-categories': 'Material Categories',
    supplier: 'Suppliers',
    customer: 'Customers',
    project: 'Projects',
    carrier: 'Carriers',
    warehouse: 'Warehouses',
    'company-setting': 'Settlement Entities',
    'customer-statement': 'Customer Statements',
    'freight-statement': 'Freight Statements',
    'finance-overview': 'Finance Overview',
    receipt: 'Receipts',
    payment: 'Payments',
    'cash-ledger': 'Cash Ledger',
    'price-compare': 'Price Compare',
    'market-sync': 'Market Sync',
    'purchase-order': 'Purchase Orders',
    'purchase-inbound': 'Purchase Inbound',
    'sales-order': 'Sales Orders',
    'sales-contract': 'Sales Contracts',
    'sales-outbound': 'Sales Outbound',
    'sales-return': 'Sales Returns',
    inventory: 'Inventory',
    'freight-bill': 'Freight Bills',
    'print-template': 'Print Templates',
    'operation-log': 'Operation Logs',
    'role-management': 'Roles',
    'user-accounts': 'User Accounts',
    account: 'My Account',
    dashboard: 'Dashboard',
  },
  moduleFilters: {
    商品信息: 'Product',
    订单编号: 'Order No.',
    供应商名称: 'Supplier',
    结算主体: 'Settlement Entity',
    单据状态: 'Document Status',
    订单日期: 'Order Date',
    入库单号: 'Inbound No.',
    入库日期: 'Inbound Date',
    送货日期: 'Delivery Date',
    客户名称: 'Customer',
    项目名称: 'Project',
    物流商: 'Carrier',
    对账期间: 'Statement Period',
    出库单号: 'Outbound No.',
    出库日期: 'Outbound Date',
    退货单号: 'Return No.',
    退货日期: 'Return Date',
    物流单号: 'Freight Bill No.',
    单据日期: 'Document Date',
    方向: 'Direction',
    审核状态: 'Audit Status',
  },
  marketSync: {
    title: 'Market Sync',
    desc: '30-day coverage monitor; missing periods highlighted; click a cell to view its details',
    dataSource: 'Source',
    singleDay: 'Date',
    backfill: 'Backfill',
    days: 'days',
    backfillMissing: 'Backfill missing',
    region: 'Region',
    allPeriods: 'All periods',
    backfillMissingHint:
      'Only backfill articles missing in the last 30 days (existing ones are skipped)',
    sync: 'Sync',
    syncMissing: 'Missing',
    weekend: 'Off',
    matrixTitle: 'Coverage Matrix (last 30 days)',
    matrixDate: 'Date',
    matrixNoData: 'No quote data for this period; click to sync it',
    matrixRows: '{{rows}} rows',
    detailTitle: 'Quote Details',
    notSelected: 'Not selected',
    up: 'Up',
    down: 'Down',
    search: 'Search',
    reset: 'Reset',
    export: 'Export',
    goCompare: 'Compare',
    goCompareHint: 'Open price compare with this date/period',
    total: '{{total}} total',
    selectPeriodHint: 'Please select a date and period',
    columnFactory: 'Brand/Mill',
    columnBreed: 'Product',
    columnMaterial: 'Grade',
    columnSpec: 'Spec',
    columnPrice: 'Price (CNY/t)',
    columnChange: 'Change',
    columnRemark: 'Remark',
    period: 'Period',
    coverage: '30-day coverage {{covered}}/{{weekdays}} days',
    missingSlots: 'Missing periods {{count}}',
    backfilling: 'Backfilling {{from}} ~ {{to}}',
    backfillDone:
      'Last backfill {{from}} ~ {{to}}: {{synced}} ok / {{failed}} failed / {{rows}} rows',
    todayPeriod: 'Today {{period}} {{mark}}',
    backfillProgress: '{{done}}/{{total}} days',
    backfillFailures: 'Backfill failed for {{count}} days:',
    syncDone: 'Sync complete: {{date}} {{periods}}, {{rows}} rows{{exists}}',
    syncDoneExists: ' (already exists)',
    syncMissingPeriods: 'No quotes for these periods today: {{periods}}',
    syncFailed: 'Sync failed',
    backfillAccepted:
      'Backfill accepted: {{from}} ~ {{to}} (runs in background, auto-refreshes when done)',
    backfillFailed: 'Backfill failed',
    retryLater: 'Please retry later',
    syncedRows: 'Synced {{date}} {{periods}}, {{rows}} rows',
    exportFileName: 'quote-details_{{date}}_{{period}}.csv',
    exported: 'Exported {{rows}} rows',
    exportFailed: 'Export failed, please retry later',
  },
  financeDetail: {
    loadOverviewFailed: 'Failed to load finance overview',
    counterpartyTypeShort: 'Counterparty Type',
    ledgerDetail: 'Ledger Details',
    counterpartyCustomer: 'Customer',
    counterpartySupplier: 'Supplier',
    counterpartyCarrier: 'Carrier',
    flowReceipt: 'Receipt',
    flowPayment: 'Payment',
    flowPaymentReversal: 'Payment reversal',
    flowReceiptReversal: 'Receipt reversal',
    purposeCustomerStatementSettlement: 'Customer statement settlement',
    purposeSupplierPrepaymentRefund: 'Supplier prepayment refund',
    purposeSupplierOtherReceipt: 'Supplier other receipt',
    purposeStatementSettlement: 'Statement settlement',
    purposePurchasePrepayment: 'Purchase prepayment',
    purposeSupplierPayment: 'Supplier payment',
    openingBalance: 'Opening Balance',
    periodIncome: 'Period Income',
    periodExpense: 'Period Expense',
    closingBalance: 'Closing Balance',
    colBusinessDate: 'Business Date',
    colFlowType: 'Flow Type',
    colDocumentNo: 'Doc No.',
    document: 'Document',
    colCounterpartyType: 'Counterparty Type',
    colCounterpartyName: 'Counterparty',
    colPurpose: 'Purpose',
    colIncome: 'Income',
    colExpense: 'Expense',
    colRunningBalance: 'Running Balance',
    colOperator: 'Operator',
    colRemark: 'Remark',
    settlementCompany: 'Settlement Entity',
    selectSettlementCompany: 'Please select a settlement entity',
    keyword: 'Keyword',
    keywordPlaceholder: 'Doc no., counterparty, purpose, operator or remark',
    allCounterpartyTypes: 'All counterparty types',
    counterparty: 'Counterparty',
    allCounterparties: 'All counterparties',
    selectTypeFirst: 'Select a type first',
    allFlowTypes: 'All flow types',
    loadFailed: 'Failed to load cash ledger',
    retryLater: 'Please retry later',
    empty: 'No cash ledger entries',
    total: '{{count}} total',
    exported: 'Cash ledger exported',
    exportFailed: 'Failed to export cash ledger',
    refreshFailed: 'Failed to refresh cash ledger',
    receivable: 'Receivable',
    payable: 'Payable',
    received: 'Received',
    unreceived: 'Unreceived',
    prepaid: 'Prepaid',
    paid: 'Paid',
    unpaid: 'Unpaid',
    prepayment: 'Prepayment',
    direction: 'Finance Direction',
    balanceRange: 'Balance Range',
    all: 'All',
    hasBalance: 'Has balance',
    dueDate: 'As of Date',
    counterpartyCode: 'Counterparty Code',
    receivableYuan: 'Receivable (CNY)',
    payableYuan: 'Payable (CNY)',
    receivedYuan: 'Received (CNY)',
    paidYuan: 'Paid (CNY)',
    unreceivedYuan: 'Unreceived (CNY)',
    unpaidYuan: 'Unpaid (CNY)',
    prepaidYuan: 'Prepaid (CNY)',
    prepaymentYuan: 'Prepayment (CNY)',
    action: 'Action',
    goReceipt: 'Collect',
    goPayment: 'Pay',
    counterpartyPlaceholder: 'Name, pinyin or code',
  },
  receiptReconcile: {
    pendingStatements: 'Customer Statements to Write Off',
    unsettledPeriods: '{{name}} · {{count}} unsettled period(s)',
    fifoAllocate: 'Auto-allocate by period (FIFO)',
    clearAllocations: 'Clear allocations',
    totalUnsettled: 'Total (unsettled)',
    overOutstanding: 'Exceeds outstanding by {{amount}}',
    overAllocatedHint:
      'Allocated total {{total}} exceeds received amount {{amount}}; please adjust.',
    customerOverview: 'Customer Reconciliation Overview',
    settlementPreview: 'Live Write-off Simulation',
    amountReceivedYuan: 'Received Amount (CNY)',
    rmbPrefix: 'RMB ',
    voucherAndRemark: 'Voucher & Remark',
    uploadHint: 'Click or drag to upload bank e-receipts (max 5)',
    operator: 'Operator',
    shortcutSubmit: 'Press {{keys}} to submit',
    saveDraft: 'Save as Draft',
    confirmReceipt: 'Confirm Receipt & Write-off',
    flowTitle: 'Cash Ledger',
    ledgerEmpty: 'No audited cash ledger entries',
    workbench: 'Receipt Write-off Workbench',
    draft: 'Draft',
    detailTitle: 'Statement Details',
    period: 'Statement Period',
    dueDate: 'Payment Due Date',
    totalAmount: 'Statement Total',
    outstanding: 'Unsettled Payable',
    slipNo: 'Weigh Slip No.',
    productSpec: 'Product / Spec',
    netWeightTon: 'Net Weight (t)',
    unitPrice: 'Unit Price',
    subtotal: 'Subtotal',
    deductionNote: 'Deduction Note',
    ourSettlementCompany: 'Our Settlement Entity',
    counterpartyType: 'Counterparty Type',
    counterpartyCustomer: 'Customer',
    receiptDate: 'Receipt Date',
    receiptMethod: 'Receipt Method',
    bankAccount: 'Bank Account',
    remark: 'Remark',
    remarkPlaceholder: 'e.g. acceptance bill discount, payment on behalf',
    uploadEReceipt: 'Upload E-Receipt',
    prepaidDepositOption: 'Transfer full amount to prepaid deposit',
    performanceDepositOption: 'Record amount as performance deposit',
    prepaidDepositDesc:
      'The received {{amount}} CNY will be fully transferred to {{name}}’s prepaid deposit account, deductible on shipment.',
    performanceDepositDesc:
      'The received {{amount}} CNY will be held as {{name}}’s performance deposit until cooperation ends or contract conditions are met.',
    creditTerm: 'Credit Term',
    reconciledUnsettled: 'Reconciled Unsettled Total',
    overdueAmount: 'Overdue Amount',
    inTransitUnreconciled: 'In-Transit Unreconciled',
    receivedAmount: 'Received Amount',
    offsetStatementTotal: 'Offset Statement Total',
    overpaymentToPrepaid: 'Overpayment (to prepaid)',
    remainingDebt: 'Remaining Customer Debt after Write-off',
    uploadedEReceipts: '{{count}} e-receipt(s) uploaded',
    suggestUploadEReceipt: 'Upload a bank e-receipt as write-off evidence',
    statementNo: 'Statement No.',
    status: 'Status',
    offsetThisTime: 'Offset This Time',
    overAllocated:
      'Allocated total {{total}} exceeds received amount {{amount}}',
    writeOffStatement: 'Write off customer statement',
    saveAsPrepaid: 'Save directly as prepaid deposit',
    performanceDeposit: 'Performance deposit',
    missingCounterparty: 'Missing counterparty information',
    ledgerTitle: '{{name}} · Ledger Details',
    ledgerDetail: 'Ledger Details',
    loadLedgerFailed: 'Failed to load ledger details',
    date: 'Date',
    flowType: 'Flow Type',
    documentNo: 'Doc No.',
    customer: 'Customer',
    document: 'Document',
    income: 'Income',
    expense: 'Expense',
  },
  documentReference: {
    copyAllDocs: 'Copy all doc numbers',
    copyDoc: 'Copy doc number',
    copy: 'Copy {{no}}',
    copyAll: 'Copy all',
    copiedAll: 'Copied all doc numbers',
    copiedOne: 'Doc number copied',
    copyFailed: 'Copy failed; please select the numbers manually',
    related: 'Related {{label}}',
    amount: 'Amount: {{amount}}',
  },
  priceCompareStore: {
    saveSheetHeaderFailed: 'Failed to save quote sheet header',
    deleteItemFailed: 'Failed to delete line item',
    saveItemFailed: 'Failed to save line item',
    scopeConfig: 'Project config',
    scopeItem: 'A line item',
    scopeHeader: 'Header',
    conflictContent:
      'The server content was updated by another device. Choose how to proceed. Reload discards local changes; Overwrite replaces server content with yours.',
    conflictOverwrite: 'Overwrite with mine',
    conflictReload: 'Reload (discard my changes)',
    conflictRetry: 'Still being edited, please retry later',
    saveSheetFailed: 'Failed to save quote sheet',
    saveConfigFailed: 'Failed to save quote config',
    serverUpdated: 'The server has updates; refresh after saving',
    conflictTitle: 'Document version changed',
    lockConflict:
      'The document is being edited by someone else. Please retry later or request takeover.',
    reloadSheetFailed: 'Failed to reload the quote sheet, please retry later',
    reloadConfigFailed: 'Failed to reload the quote config, please retry later',
    loadSheetsFailed: 'Failed to load quote sheets, please retry later',
    deleteSheetFailed: 'Failed to delete the quote sheet, please retry later',
    takeoverTitle: 'Request Batch Takeover',
    takeoverContent:
      'Takeover will forcibly acquire edit rights for this batch as the current user; the other party’s in-progress edits will be overwritten and the action is logged. Confirm takeover?',
    takeoverOk: 'Force Takeover',
    takeoverFailedShort: 'Takeover failed, please retry later',
  },
  moduleSelector: {
    spaceEnterHint: 'Space to select, Enter to choose',
  },
  moduleEditor: {
    pricePendingTitle: 'Price Pending Reminder',
    pricePendingContent:
      '{{count}} line item(s) have a unit price of 0 and will be saved as "price pending". Continue?',
    continueSave: 'Continue',
    backToEdit: 'Back to edit',
  },
  navigation: {
    master: 'Master Data',
    market: 'Price Compare',
    purchase: 'Purchase',
    sales: 'Sales',
    inventory: 'Inventory',
    freight: 'Freight',
    statements: 'Statements',
    finance: 'Finance',
    system: 'System',
  },
}
