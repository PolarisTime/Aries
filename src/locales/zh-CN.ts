export const zhCN = {
  priceRule: {
    title: '价格规定',
    hint: '配置多条网价浮动方式，交付核定时可选择其一（单选，不叠加）。',
    name: '名称',
    namePlaceholder: '如 含税价',
    adjust: '方式',
    add: '加价',
    subtract: '减价',
    amount: '金额（元/吨）',
    amountPlaceholder: '如 30',
    remark: '备注',
    remarkPlaceholder: '说明（可选）',
    addRow: '新增规定',
    remove: '删除该规定',
    save: '保存价格规定',
    saved: '价格规定已保存',
    nameRequired: '请填写规定名称',
    nameDuplicate: '规定名称不能重复',
  },
  appUpdate: {
    title: '系统版本已更新',
    description: '检测到系统已发布新版本，请清除缓存并刷新后继续使用。',
    unsavedWarning: '刷新将丢失尚未保存的内容。',
    refreshButton: '清除缓存并强制刷新',
  },
  errorBoundary: {
    retry: '重试',
    accessDenied: '当前页面拒绝访问',
    serverBusy: '服务器繁忙，请稍后重试',
    networkError: '网络连接异常',
    timeout: '请求超时',
    serverError: '服务器响应异常，请稍后重试',
    networkHint: '请检查网络连接后重试',
    forbiddenHint: '请联系管理员开通相关权限',
    runtimeHint: '请刷新页面重试；若问题持续请联系技术支持',
    runtimeTitle: '页面渲染出错',
    errorSummary: '错误摘要',
    copyFailed: '复制失败，请手动复制',
  },
  result: {
    '403': {
      subTitle: '当前页面拒绝访问',
    },
    '404': {
      subTitle: '抱歉，您访问的页面不存在',
    },
    '500': {
      subTitle: '抱歉，服务器出错了',
    },
    success: {
      title: '操作成功',
    },
    error: {
      title: '操作失败',
      subTitle: '请稍后重试',
    },
    info: {
      title: '提示',
    },
    warning: {
      title: '警告',
    },
    homeButton: '返回首页',
    backButton: '返回上页',
  },
  toolbar: {
    searchPlaceholder: '搜索...',
    refresh: '刷新',
    create: '新建',
  },
  validation: {
    default: '字段校验失败',
    required: '请填写{label}',
    enum: '{label}的取值不在允许范围内',
    whitespace: '{label}不能为空',
    date: {
      format: '{label}格式不正确',
      parse: '{label}无法识别为日期',
      invalid: '{label}不是有效日期',
    },
    types: {
      string: '{label}格式不正确',
      method: '{label}格式不正确',
      array: '请选择{label}',
      object: '{label}格式不正确',
      number: '{label}必须为数字',
      date: '{label}不是有效日期',
      boolean: '{label}格式不正确',
      integer: '{label}必须为整数',
      float: '{label}必须为数字',
      regexp: '{label}格式不正确',
      email: '{label}不是有效邮箱',
      url: '{label}不是有效链接',
      hex: '{label}格式不正确',
    },
    string: {
      len: '{label}长度必须为{len}个字符',
      min: '{label}至少{min}个字符',
      max: '{label}最多{max}个字符',
      range: '{label}长度必须在{min}-{max}个字符之间',
    },
    number: {
      len: '{label}必须等于{len}',
      min: '{label}不能小于{min}',
      max: '{label}不能大于{max}',
      range: '{label}必须在{min}-{max}之间',
    },
    array: {
      len: '{label}必须选择{len}项',
      min: '{label}至少选择{min}项',
      max: '{label}最多选择{max}项',
      range: '{label}需选择{min}-{max}项',
    },
    pattern: {
      mismatch: '{label}格式不正确',
    },
  },
  dashboard: {
    greeting: {
      welcome: '欢迎回来，{{name}}',
    },
    actions: {
      forceRefresh: '强制刷新',
    },
    metrics: {
      purchaseAudit: '待审核采购',
      outboundTask: '待出库任务',
      statementConfirm: '待确认对账',
      statementAmount: '待确认金额 {{amount}}',
      monthOutbound: '本月出库',
      monthInbound: '本月入库',
      monthReceipt: '本月回款',
      monthUnit: '本月累计单量',
      awaitingInbound: '待入库采购',
      awaitingInboundHint: '已审核未入库（未发货）',
    },
    recent: {
      title: '最近单据',
      hint: '按业务日期倒序，点击查看详情',
      empty: '暂无单据',
      loadFailed: '最近单据加载失败，请稍后重试。',
    },
    modules: {
      title: '全部模块',
      byFlow: '按业务流程查看',
      create: '新建',
    },
    todo: {
      title: '待办工作台',
      tabs: {
        all: '全部待办',
        purchaseAudit: '采购审批',
        salesDelivery: '销售发货',
        financeReconcile: '财务对账',
      },
      columns: {
        docNo: '单据编号',
        bizType: '业务类型',
        relatedDocument: '关联单据',
        counterparty: '往来单位',
        businessDate: '业务日期',
        action: '操作',
      },
      goProcess: '去处理',
      empty: '暂无待办',
      loadFailed: '待办加载失败，请稍后重试。',
    },
    account: {
      title: '账户概况',
    },
    flow: {
      material: {
        title: '商品资料',
        hint: '维护品名、规格、品牌',
      },
      supplier: {
        title: '供应商资料',
        hint: '采购往来主体建档',
      },
      customer: {
        title: '客户资料',
        hint: '销售往来主体建档',
      },
      warehouse: {
        title: '仓库资料',
        hint: '维护仓库与库位能力',
      },
      purchaseOrder: {
        title: '采购订单',
        hint: '生成采购执行单',
      },
      purchaseInbound: {
        title: '采购入库',
        hint: '确认入库数量重量',
      },
      project: {
        title: '客户项目',
        hint: '维护客户项目档案',
      },
      payment: {
        title: '付款单',
        hint: '完成付款登记',
      },
      salesOrder: {
        title: '销售订单',
        hint: '下达客户订单',
      },
      salesOutbound: {
        title: '销售出库',
        hint: '确认发货与出库',
      },
      customerStatement: {
        title: '客户对账单',
        hint: '汇总销售应收',
      },
      receipt: {
        title: '收款单',
        hint: '完成收款登记',
      },
      carrier: {
        title: '物流方资料',
        hint: '维护承运主体',
      },
      freightBill: {
        title: '物流单',
        hint: '记录运输过程',
      },
      freightStatement: {
        title: '物流对账单',
        hint: '汇总运费结算',
      },
      loadingDescription: '正在加载业务流程说明...',
    },
    info: {
      userName: '当前用户',
      loginName: '登录账号',
      companyName: '所属公司',
      lastLogin: '最近登录',
      accountInfo: '账户信息',
      activeSessions: '活跃会话',
    },
    sections: {
      overview: '系统概览',
      businessFlow: '业务流程',
    },
    alerts: {
      title: '当前重点',
      loadFailed: '工作台摘要加载失败，请稍后重试。',
    },
    fields: {
      serverTime: '服务时间',
    },
    values: {
      unconfigured: '未配置',
    },
  },
  common: {
    confirm: '确认',
    cancel: '取消',
    save: '保存',
    delete: '删除',
    edit: '编辑',
    create: '新增',
    search: '搜索',
    reset: '重置',
    export: '导出',
    import: '导入',
    refresh: '刷新',
    submit: '提交',
    back: '返回',
    close: '关闭',
    ok: '确定',
    yes: '是',
    no: '否',
    more: '更多',
    expand: '展开',
    collapse: '收起',
    actions: '操作',
    status: '状态',
    remark: '备注',
    operation: '操作',
    createdAt: '创建时间',
    updatedAt: '更新时间',
    keyword: '关键词',
    detail: '详情',
    loading: '加载中...',
    loadingIndicatorAnnouncing: '正在加载数据…',
    noData: '暂无数据',
    total: '共 {{count}} 条',
    selected: '已选择 {{count}} 项',
    clearSelection: '清除选择',
    pleaseSelect: '请选择',
    pleaseInput: '请输入',
    confirmLogout: '确认退出',
    logout: '退出登录',
    saveSuccess: '保存成功',
    addSuccess: '新增成功',
    editSuccess: '编辑成功',
    deleteSuccess: '删除成功',
    deleteConfirm: '确定删除？',
    unsavedChangesTitle: '存在未保存的修改',
    unsavedChangesContent: '当前修改尚未保存，继续操作将丢失这些修改。',
    discardChanges: '放弃修改',
    saveInProgressTitle: '正在保存',
    saveInProgressContent: '当前单据正在保存，请等待保存完成后再离开。',
    columnSettings: '列设置',
    resetColumnWidths: '重置列宽',
    saveAndAudit: '保存并审核',
    auditConfirm: '审核后单据状态将变更为已审核，且不可再编辑。确定继续？',
    confirmLogoutContent: '确定要退出登录吗？',
    displaySettingsSaved: '显示设置已保存',
    brandSubtitle: '钢贸业务中台',
    productCopyright: '© {{year}} Leo',
    frontendVersion:
      '前端 v{{version}} · 编译时间 {{buildTime}} · 构建 SHA-1 {{gitCommit}}',
    backendVersion:
      '后端 v{{version}} · 编译时间 {{buildTime}} · 构建 SHA-1 {{gitCommit}}',
    versionUnknown: '--',
    confirmAudit: '确定审核',
    saveFailed: '保存失败',
    saveFailedRetry: '保存失败，请稍后重试',
    query: '查询',
    masterData: '基础建档',
    purchaseChain: '采购链路',
    salesChain: '销售链路',
    masterDataDesc: '先维护业务主数据，后续单据可直接关联带出。',
    purchaseChainDesc: '从合同与订单开始，经过入库、对账，最终完成付款。',
    salesChainDesc: '从销售合同与订单开始，经过出库、对账，最终形成收款闭环。',
    prevPage: '上一页',
    nextPage: '下一页',
    importParentFailed: '导入上级单据失败',
    logisticsChain: '物流协同',
    logisticsChainDesc: '物流相关单据独立流转，并继续支持物流对账。',
    pleaseSelectWith: '请先选择{{label}}',
    importParentSuccess:
      '已导入 {{parentCount}} 张上级单据，共 {{itemCount}} 条明细',
    importParentSuccessSimple: '已导入 {{itemCount}} 条明细',
    disabled: '已禁用',
    batchDelete: '批量删除',
  },
  finance: {
    filters: {
      advanced: '高级筛选',
    },
    overview: {
      title: '财务概览',
      description: '汇总应收、应付及往来方余额，支持按结算主体和截止日期查询。',
      refreshAria: '刷新财务概览',
      emptyBalances: '暂无往来余额',
      selectSettlementCompany: '请选择结算主体',
    },
    cashLedger: {
      title: '资金流水',
      description: '查询结算主体的收付款流水及期间余额变化。',
      exportFiltered: '导出当前筛选结果',
      refreshAria: '刷新资金流水',
    },
  },
  inventory: {
    title: '库存查询',
    description: '按物料与仓库查询库存余额及出入库流水。',
    refreshAria: '刷新库存数据',
    loadFailed: '加载库存数据失败',
    loadFailedHint: '请稍后重试',
    tabs: {
      balances: '库存余额',
      transactions: '库存流水',
    },
    filters: {
      keyword: '关键字',
      balanceKeywordPlaceholder: '物料编码 / 品牌 / 材质 / 规格',
      transactionKeywordPlaceholder: '事务号 / 物料 / 来源单据号',
      warehouse: '仓库',
      warehousePlaceholder: '全部仓库',
      transactionType: '事务类型',
      transactionTypePlaceholder: '全部事务类型',
      occurredAt: '发生日期',
    },
    columns: {
      materialCode: '物料编码',
      brand: '品牌',
      material: '材质',
      spec: '规格',
      length: '长度',
      unit: '单位',
      warehouse: '仓库',
      batchNo: '批次',
      quantity: '数量',
      avgUnitCost: '平均成本',
      amount: '金额',
      transactionNo: '事务号',
      transactionType: '类型',
      direction: '方向',
      unitCost: '单位成本',
      sourceDocumentNo: '来源单据号',
      occurredAt: '发生日期',
    },
    direction: {
      in: '入',
      out: '出',
    },
    transactionType: {
      PURCHASE_INBOUND: '采购入库',
      PURCHASE_RETURN: '采购退货出库',
      SALES_OUTBOUND: '销售出库',
      SALES_RETURN: '销售退货入库',
      INBOUND: '入库',
      OUTBOUND: '出库',
      ADJUSTMENT_IN: '盘盈入库',
      ADJUSTMENT_OUT: '盘亏出库',
      TRANSFER_IN: '调拨入库',
      TRANSFER_OUT: '调拨出库',
      INITIAL_BALANCE: '期初结存',
    },
    empty: {
      balances: '暂无库存余额',
      transactions: '暂无库存流水',
    },
  },
  auth: {
    login: '登录',
    logout: '退出登录',
    loginName: '用户名',
    password: '密码',
    remember: '记住我',
    loginSuccess: '登录成功',
    loginFailed: '登录失败',
    logoutSuccess: '退出成功',
    sessionExpired: '登录状态已失效，请重新登录',
    initialSetup: '系统初始化',
    loginview: {
      heroSubtitle: '统一采购、销售、库存、财务的一体化业务中台',
      backendOnline: '后端服务正常',
      backendOffline: '后端服务离线',
      setupReady: '系统已就绪',
      setupPending: '待初始化配置',
    },
    loginform: {
      step: '账号密码验证',
      title: '登录系统',
      description: '输入账号密码后进入系统。',
      loginNameLabel: '登录名',
      loginNameRequired: '请输入用户名',
      loginNamePlaceholder: '请输入用户名',
      passwordLabel: '登录密码',
      passwordRequired: '请输入密码',
      passwordPlaceholder: '请输入密码',
      remember: '记住登录状态',
      submit: '登录系统',
    },
    initialsetup: {
      checking: '正在检查初始化状态...',
      completedTitle: '系统已完成初始化',
      guideTitle: '唯一账号初始化',
      defaultAccountUserName: '系统用户',
      setupTokenLabel: '初始化凭证',
      setupTokenPlaceholder: '请输入部署环境中的初始化凭证',
      setupTokenRequired: '请输入初始化凭证',
      setupTokenInvalid: '初始化凭证格式无效',
      alreadyCompletedRedirect: '系统已完成初始化，即将跳转登录页',
      loadStatusFailed: '获取初始化状态失败',
      operationFailed: '操作失败',
      passwordMismatch: '两次密码输入不一致',
      accountCreateSuccess: '账号创建成功',
      account: {
        loginNameLabel: '登录名',
        loginNameRequired: '请输入登录名',
        loginNamePlaceholder: '登录名',
        passwordLabel: '密码',
        passwordRequired: '密码至少8位',
        passwordPlaceholder: '至少8位',
        confirmPasswordLabel: '确认密码',
        confirmPasswordRequired: '请确认密码',
        confirmPasswordPlaceholder: '再次输入密码',
        userNameLabel: '姓名',
        userNamePlaceholder: '系统用户',
        submit: '完成初始化',
      },
    },
    validation: {
      required: '必填项不能为空',
    },
    error: {
      network: '网络连接失败，请检查服务是否可用',
      timeout: '请求超时，请稍后重试',
      sessionExpired: '登录状态已失效，请重新登录',
      forbidden: '拒绝访问',
      notFound: '请求的接口不存在',
      serverError: '服务暂时不可用，请稍后重试',
      generic: '请求失败，请稍后重试',
      loginFailed: '登录失败',
      missingTokenOrUser: '登录响应缺少 token 或用户信息',
      sessionRestoreFailed: 'Session 恢复失败',
    },
  },
  table: {
    create: '新增',
    edit: '编辑',
    delete: '删除',
    batchDelete: '批量删除',
    deleteConfirm: '确认删除选中记录？',
    deleteSuccess: '删除成功',
    saveSuccess: '保存成功',
    createSuccess: '新增成功',
    updateSuccess: '更新成功',
    operationSuccess: '操作成功',
    operationFailed: '操作失败',
    readOnly: '当前模块为只读模式',
  },
  attachment: {
    title: '附件管理',
    upload: '上传附件',
    preview: '预览',
    download: '下载',
    delete: '删除附件',
    uploadSuccess: '附件上传成功',
    notSupported: '该附件暂不支持预览',
  },
  api: {
    requestFailed: '请求失败',
    loadFailed: '加载失败',
    saveFailed: '保存失败',
    deleteFailed: '删除失败',
    updateFailed: '更新失败',
    createFailed: '创建失败',
    exportFailed: '导出失败',
    importFailed: '导入失败',
    validationFailed: '数据校验失败',
    networkError: '网络错误，请检查网络连接',
    timeout: '请求超时，请稍后重试',
    unauthorized: '未授权，请重新登录',
    forbidden: '拒绝访问',
    notFound: '请求的资源不存在',
    serverError: '服务器错误，请稍后重试',
    moduleNotConfigured: '未配置模块接口',
    authServiceUnavailable: '认证服务不可用',
    backendServiceUnavailable: '后端服务不可用',
    responseValidationFailed: '响应数据校验失败',
    pageDataValidationFailed: '分页数据校验失败',
    loadAccountColumnSettingsFailed: '加载账号列设置失败',
    saveAccountColumnSettingsFailed: '保存账号列设置失败',
    resultIncomplete: '结果不完整：已达客户端过滤上限，请缩小筛选范围。',
    loadCompanyInfoFailed: '加载结算主体信息失败',
    saveCompanyInfoFailed: '保存结算主体信息失败',
    loadCurrentAccountFailed: '加载当前账号失败',
    saveCurrentAccountFailed: '保存当前账号失败',
    loadSearchResultsFailed: '加载搜索结果失败',
    getInitStatusFailed: '获取初始化状态失败',
    accountInitFailed: '账号初始化失败',
    importResponseValidationFailed: '导入响应数据校验失败',
    queryStatementCandidatesFailed: '查询对账候选单据失败',
    loginExpiring: '登录即将过期',
    loginExpiringDescription:
      '您的登录状态将在明天过期，请及时保存工作并重新登录',
    refreshLoginStatusFailed: '刷新登录状态失败',
    loginStatusExpired: '登录状态已失效，请重新登录',
    errorMessages: {
      requiredEmpty: '必填项不能为空',
      networkError: '网络连接失败，请检查服务是否可用',
      timeout: '请求超时，请稍后重试',
      sessionExpired: '登录状态已失效，请重新登录',
      forbidden: '拒绝访问',
      notFound: '请求的接口不存在',
      requestFailed: '请求失败，请稍后重试',
      missingRequiredField: '缺少必填字段：{{fieldName}}',
      serviceUnavailable: '服务暂时不可用，请稍后重试',
    },
  },
  modules: {
    actions: {
      create: '新增',
      export: '导出',
    },
    purchasePickupList: {
      action: '查看提货清单（{{count}}）',
      title: '采购订单提货清单（{{count}}张）',
      tooManyOrders: '单次最多查看 50 张采购订单',
      loadFailed: '提货清单加载失败',
      orderCount: '订单数',
      supplierCount: '供应商数',
      itemCount: '明细数',
      totalQuantity: '总件数',
      totalWeight: '总重量（吨）',
      itemName: '品名',
      pickupQuantity: '数量',
      pieceWeight: '件重（吨）',
      pickupWeight: '重量（吨）',
      groupItemCount: '{{count}}条明细',
      groupTotalQuantity: '总件数',
      groupTotalWeight: '总重量',
      groupByWarehouse: '按仓库一键分组',
      unassignedWarehouse: '未指定仓库',
      addGroup: '添加分组',
      groupLabel: '分组 {{index}}',
      groupRemarkPlaceholder: '添加分组备注',
      lockGroup: '锁定分组 {{index}}',
      unlockGroup: '解除锁定分组 {{index}}',
      removeGroup: '移除分组 {{index}}',
      emptyGroup: '暂无明细',
      dragGroup: '拖动分组 {{index}}',
      dragRow: '拖动第 {{index}} 行',
      restoreDefault: '恢复默认顺序',
      splitItem: '拆分数量',
      mergeItem: '合并拆分',
      splitPartLabel: '第 {{index}}/{{total}} 份',
      removeSplitPart: '移除第 {{index}} 行',
      splitRecorded: '拆分数量仅用于本次查看与分组，不会写回采购订单。',
      splitTitle: '拆分数量',
      splitConfirm: '拆分',
      splitPieceCountLabel: '每份件数',
      splitPieceCountPlaceholder: '请输入每份件数',
      splitTotal: '当前共 {{total}} 件',
      splitPreview: '拆分预览',
      splitPartShort: '第 {{index}} 份：{{quantity}} 件',
      splitPartTotal: '共 {{count}} 份（每件约 {{pieceWeight}} 吨）',
      splitInvalid: '每份件数需为 1 至 {{max}} 之间的整数',
    },
    overview: {
      recordCount: '记录数',
      totalWeight: '总重量',
      totalAmount: '总金额',
      documentCount: '单据数量',
      masterDataCount: '主数据数',
      normalCount: '正常数',
      currentAmount: '本期金额',
      settledAmount: '已结算金额',
      statementCount: '对账单数',
      balance: '余额',
    },
    status: {
      draft: '草稿',
      unaudited: '未审核',
      audited: '已审核',
      reviewed: '审核',
      issued: '签发',
      executing: '执行中',
      signed: '已签署',
      unsigned: '未签署',
      archived: '已归档',
      voided: '作废',
      completed: '已完成',
      completedPurchase: '完成采购',
      completedSales: '完成销售',
      deliveryVerification: '交付核定',
      completedInbound: '完成入库',
      confirmed: '已确认',
      pendingConfirm: '待确认',
      pendingAudit: '待审核',
      pendingApproval: '待审批',
      approved: '已审批',
      paid: '已付款',
      received: '已收款',
      partialInbound: '部分入库',
      partialOutbound: '部分出库',
      partialSettled: '部分结算',
      normal: '正常',
      disabled: '已禁用',
      success: '成功',
      failed: '失败',
      deleted: '已删除',
    },
    statusActions: {
      audit: '审核',
      reverseAudit: '反审核',
      confirm: '确认',
      reverseConfirm: '反确认',
      approve: '核准',
      reverseApprove: '反核准',
      reopenDeliveryVerification: '重新核定',
    },
    editor: {
      edit: '编辑',
      create: '新建',
      title: '{{mode}} — {{title}}',
      missingQuantityUnit:
        '商品「{{material}}」未维护数量单位，请先补全商品主数据后再保存',
    },
    itemColumns: {
      brand: '品牌',
      material: '材质',
      spec: '规格',
      length: '长度',
      quantity: '数量',
      weightTon: '总重(吨)',
      unitPrice: '单价',
      amount: '金额',
      warehouseName: '码头',
      sourceNo: '出库单号',
      materialSearchPlaceholder: '搜索品牌 / 类别 / 材质 / 规格 / 长度',
      warehousePlaceholder: '选择码头',
      settlementModePlaceholder: '选择结算方式',
    },
    units: {
      documentSheet: '张',
      ton: '吨',
      yuan: '元',
    },
    nextModule: {
      createPurchaseInbound: '创建采购入库',
      createSalesOutbound: '创建销售出库',
      createFreightBill: '创建物流单',
    },
    saveResult: {
      success: '保存成功',
      pageSuccess: '{{title}}保存成功',
      error: '保存失败',
      conflict: '数据已被其他操作更新',
      backToEdit: '返回编辑',
      reloadLatest: '重新加载最新数据',
      createAnother: '再做一张',
      close: '关闭',
    },
    statement: {
      supplier: '供应商',
      customer: '客户',
      freight: '物流',
      counterpartyNotFound: '未找到对方单位信息',
      multipleCounterparties:
        '选中的单据包含多个对方单位，请只勾选同一单位的单据',
      dateMissing: '选中的单据缺少日期信息',
      generateTitle: '生成{{type}}对账单',
      selectHint: '请先在列表中勾选需要生成对账单的单据',
      extractError: '无法从选中的单据中提取对账信息，请检查数据完整性',
      counterpartyUnit: '对方单位：',
      period: '对账期间：',
      documentCount: '单据数量：',
      documentCountUnit: '笔',
      generated: '对账单已生成',
      generateFailed: '生成失败，请稍后重试',
      generateButton: '生成对账单',
      gotIt: '知道了',
    },
    weightCell: {
      materialCode: '商品编码',
      materialType: '商品类型',
      materialTypePhysical: '实体商品',
      materialTypeExpense: '附加费用',
      brand: '品牌',
      material: '材质',
      spec: '规格',
      length: '长度',
      pieceWeightTon: '件重(吨)',
      quantity: '数量',
      weightTon: '总重(吨)',
      popoverTitle: '每件重量明细',
      noLineItems: '当前单据无明细数据',
      loadDetailFailed: '加载明细失败',
    },
    detail: {
      titleSuffix: '详情',
      recordDetail: '记录详情',
      noData: '暂无数据',
      close: '关闭',
      print: '打印',
      noDetailItems: '暂无明细数据',
    },
    attachment: {
      title: '附件管理',
      upload: '上传附件',
      uploadHint: '支持点击上传，也支持在此区域内直接粘贴图片或文件',
      noPreviewUrl: '当前附件暂无预览地址',
      noDownloadUrl: '当前附件暂无下载地址',
      previewFailed: '预览失败',
      downloadFailed: '下载失败',
      uploadBindSuccess: '上传并绑定成功',
      uploadFailed: '上传失败',
      uploadNoId: '上传成功但未返回附件标识',
      uploadingProgress: '正在上传 {{fileName}}，已完成 {{percent}}%',
      unbindSuccess: '解除绑定成功',
      deleteFailed: '删除失败',
      loadFailed: '附件加载失败',
      noAttachments: '暂无附件',
      preview: '预览附件',
      download: '下载附件',
      unbind: '解除绑定',
      unbindConfirm: '确定解除该附件与当前单据的绑定吗？',
      pdfPreview: 'PDF 预览',
    },
    filter: {
      keyword: '关键字',
      status: '状态',
      billStatus: '单据状态',
      auditStatus: '审核状态',
      signStatus: '签署状态',
      customerName: '客户名称',
      supplierName: '供应商名称',
      carrierName: '物流商',
      orderNo: '单据编号',
      contractNo: '合同编号',
      outboundNo: '出库单号',
      inboundNo: '入库单号',
      freightNo: '物流单号',
      referenceStatus: '下游引用',
      referencedByFreightBill: '被物流单引用',
      referencedBySalesOutbound: '被销售出库引用',
      referencedBySalesOrder: '被销售订单引用',
      referencedByPurchaseInbound: '被采购入库引用',
      notReferenced: '未被引用',
      all: '全部',
      today: '今天',
      last7Days: '近7天',
      last30Days: '近30天',
      thisMonth: '本月',
      selectPlaceholder: '请选择{{label}}',
      inputPlaceholder: '请输入{{label}}',
      quickFilters: '快捷筛选',
      conditions: '筛选条件',
      documentOrCounterparty: '单号/往来方',
      counterpartyCode: '往来方编码',
      counterparty: '往来方',
      counterpartyType: '往来类型',
      counterpartyCustomer: '客户',
      counterpartySupplier: '供应商',
      relatedContractProject: '关联合同/项目',
      receiptVoucher: '回单/凭证',
      relatedFreightBill: '关联物流单',
      freightBill: '物流单',
      salesOrder: '销售订单',
      activeCount: '已启用 {{count}} 项',
    },
    formField: {
      inputPlaceholder: '请输入{{label}}',
      selectPlaceholder: '请选择{{label}}',
      inputRequired: '请输入{{label}}',
      selectRequired: '请选择{{label}}',
      nonNegative: '{{label}}必须为非负数',
    },
    salesContract: {
      contractNo: '合同编号',
      name: '合同名称',
      customer: '客户',
      project: '项目',
      signDate: '签订日期',
      startDate: '开始日期',
      endDate: '结束日期',
      totalAmount: '合同总金额',
      totalTonnage: '合同总吨位',
      dateRangeInvalid: '结束日期不得早于开始日期',
      contractNoAutoHint: '留空则由系统自动生成',
      effectiveCount: '有效合同',
      issue: '签发',
      archive: '归档',
      versionConflict: '合同已被他人修改，请刷新后重试',
      void: '作废',
      deleteConfirmTitle: '删除销售合同',
      deleteConfirmContent: '确定删除合同「{{contractNo}}」吗？',
      deleteSuccess: '删除成功',
      statusChangeConfirmTitle: '变更合同状态',
      statusChangeConfirmContent:
        '确定将合同「{{contractNo}}」状态变更为「{{status}}」吗？',
      statusChangeSuccess: '状态变更成功',
    },
    salesContractCheck: {
      title: '合同额度超限提示',
      checkFailed: '合同额度校验未成功，已跳过校验继续保存',
      stillSave: '仍然保存',
      backToEdit: '返回修改',
      contractAmount: '合同金额：{{value}}',
      usedAmount: '已用金额：{{value}}',
      remainingAmount: '剩余金额：{{value}}',
      contractTonnage: '合同吨位：{{value}}',
      usedTonnage: '已用吨位：{{value}}',
      remainingTonnage: '剩余吨位：{{value}}',
      exceededAmount: '本次超出金额：{{value}}',
      exceededTonnage: '本次超出吨位：{{value}}',
    },
    print: {
      noTemplate: '无可用模板',
      preview: '打印预览',
      directPrint: '直接打印',
      downloadPdf: '导出 PDF',
      print: '打印',
      jobTitle: '打印作业',
      currentModule: '当前模块',
      selectedRecords: '已选 {{count}} 条单据',
      currentSettlementCompany: '结算主体',
      selectTemplate: '打印模板',
      printOptions: '打印选项',
      exportXlsx: '导出套打 Excel',
      hideUnitPrice: '不打印单价',
      hideRemark: '不打印单据备注',
      enableBrandOverride: '替换打印品牌',
      enableItemSelection: '选择打印明细',
      enableSplitPrint: '拆分打印',
      splitPieceCountLabel: '每份件数',
      splitPieceCountPlaceholder: '请输入正整数',
      splitPieceCountRequired: '请先填写每份件数',
      itemSplit: '拆分',
      splitPartIndex: '份次',
      splitNotNeeded: '件数未超过每份件数，无需拆分',
      mergeEquivalentItems: '合并同款',
      splitEquivalentItems: '拆分明细',
      brandOverrideTo: '替换为',
      brandOverridePlaceholder: '留空保持原品牌',
      noBrands: '选中单据未找到品牌',
      totalItemsCount: '共 {{count}} 行明细',
      selectedItemsCount: '已选 {{count}} 项',
      noPrintItems: '选中单据暂无可展示明细',
      printItemsLoadFailed: '打印明细加载失败',
      dragRowAriaLabel: '拖动第 {{index}} 行打印明细',
      deliveryDate: '交货日期',
      recordRemark: '单据备注',
      totalQuantity: '合计件数',
      totalWeight: '合计重量',
      closeConfirmTitle: '确认关闭打印作业',
      closeConfirmContent:
        '关闭后，本次打印作业中的明细选择、排序和已输出标记将清除。确定关闭吗？',
      continueJob: '继续操作',
      itemSequence: '序号',
      outputted: '已输出',
      mergeRows: '行合并为 1 行',
      itemBrand: '品牌',
      itemCategory: '类别',
      itemMaterial: '材质',
      itemSpec: '规格',
      itemLength: '长度',
      itemQuantity: '件数',
      itemPieceWeight: '件重',
      itemWeight: '总重',
      itemUnitPrice: '单价',
      itemAmount: '金额',
      project: '项目',
      selectedRecordList: '选中单据',
      onlySelectedIds: '已保留选中记录标识，当前页暂无可展示摘要',
      moreSelectedRows: '另有 {{count}} 条当前页记录未展开显示',
      unloadedSelectedRows: '另有 {{count}} 条跨分页选中记录仅参与打印',
    },
    itemsPanel: {
      defaultTitle: '明细列表',
    },
    workspace: {
      closeAria: '关闭工作区',
      currentPageSummary: '当前页汇总',
      pagination: '列表分页',
      paginationRange: '第 {{start}}-{{end}} 条 / 共 {{total}} 条',
    },
    page: {
      configNotFound: '页面配置未找到',
      moduleConfigNotFound: '模块配置未找到',
    },
    itemsSection: {
      addItem: '新增明细',
      goodsTab: '货物明细',
      expenseTab: '附加费用',
      addExpense: '添加费用',
      expensePanelTitle: '费用明细',
      importItems: '导入{{label}}明细',
      deleteSelected: '删除选中',
      autoSortItems: '自动排序',
      autoSortBySourceNo: '按单号顺序',
      autoSortByBillTime: '按日期排序',
      autoSortByBillTimeAscending: '按日期排序（升序）',
      autoSortByBillTimeDescending: '按日期排序（降序）',
      emptyTextWithImport: '当前没有明细，可手动新增或从上级单据导入',
      emptyText: '当前没有明细，可手动新增',
      parentDoc: '上级单据',
      selectParent: '选择{{label}}',
      freightBillGroup: '来源物流单：{{sourceNo}}',
      freightBillGroupBillTime: '单据日期：{{billTime}}',
      freightBillGroupCustomer: '客户名称：{{customerName}}',
      freightBillGroupProject: '项目名称：{{projectName}}',
      freightBillGroupTotal: '合计：数量 {{quantity}}，重量 {{weight}} 吨',
      freightBillGroupUnitPrice: '运费单价：{{unitPrice}} 元/吨',
      freightBillGroupTotalFreight: '运费总价：{{totalFreight}} 元',
      freightBillProjectGroup: '项目：{{projectName}}',
      freightBillProjectGroupCustomer: '客户：{{customerName}}',
      freightBillProjectGroupTotal:
        '小计：数量 {{quantity}}，重量 {{weight}} 吨',
      customerStatementGroupNo: '分组序号：',
      customerStatementSalesOrderNo: '销售单号：',
      customerStatementDeliveryDate: '交货日期：',
      customerStatementTotalQuantity: '合计件数：',
      customerStatementTotalWeight: '合计重量：',
      customerStatementTotalAmount: '合计金额：',
    },
    expense: {
      chargeName: '费用名称',
      chargeNamePlaceholder: '搜索或输入新费用名称',
      quickCreate: '创建费用项: "{{name}}"',
      noChargeOptions: '暂无匹配的费用项，可输入名称快速创建',
      amount: '金额 (元)',
      unit: '单位',
      unitPlaceholder: '次/项',
      remark: '备注说明',
      emptyHint: '暂无附加费用，点击"添加费用"录入',
      removeSelected: '删除选中费用',
      selectAllAriaLabel: '全选费用行',
      deleteRowAriaLabel: '删除第 {{index}} 行费用',
      totalAmount: '费用合计金额',
      rowCount: '行数',
    },
    table: {
      noData: '暂无数据',
      sequence: '序号',
    },
    editorFooter: {
      cancel: '取消',
      save: '保存',
      saveAndAudit: '保存并审核',
      saveAndAction: '保存并{{action}}',
      statusActionConfirm:
        '{{action}}后单据状态将变更为{{targetStatus}}，且不可再编辑。确定继续？',
      confirmStatusAction: '确定{{action}}',
      confirmDeliveryVerification: '确认核定',
      confirmDeliveryVerificationContent:
        '确认核定后销售订单将进入完成销售状态，并按最终交付结果进入结算。请确认销售出库和实际重量已经核定。',
    },
    itemsSummary: {
      rowCount: '行数',
      quantity: '数量',
      weight: '重量',
      amount: '金额',
      expenseAmount: '附加费用合计',
    },
    editorForm: {
      documentInfo: '单据信息',
    },
    parentSelector: {
      title: '选择父单据',
      selectedCount: '已选择 {{count}} 条',
      cancel: '取消',
      confirmImport: '确认导入',
      selectedDocuments: '已选单据',
      selectedDocumentsCount: '（{{count}} 条）',
      clearSelected: '清空已选',
      removeAriaLabel: '移除 {{name}}',
      noSelectionHint: '暂未选择单据，勾选后会固定显示在这里。',
      paginationTotal: '共 {{count}} 条',
      viewDetail: '查看明细',
      collapseDetail: '收起明细',
      expandDetail: '展开明细',
      copied: '已复制',
      detailLines: '明细 {{count}} 条',
      detailImportable: '可导数量 {{count}}',
      selectedMultiSummary:
        '已选 {{orderCount}} 个订单（含 {{lineCount}} 条明细，可导数量 {{importableQuantity}}）',
      selectedMultiSummarySimple:
        '已选 {{orderCount}} 个订单（含 {{lineCount}} 条明细）',
      selectedSingleSummary: '已选 {{count}} 个订单：{{docNo}}',
      selectedEmptyHint: '未选择，点击行或勾选框进行选择',
      status: {
        draft: '草稿',
        unaudited: '未审核',
        audited: '已审核',
        purchaseComplete: '完成采购',
        salesComplete: '完成销售',
        inboundComplete: '完成入库',
      },
      column: {
        orderNo: '订单编号',
        supplierName: '供应商',
        buyerName: '采购员',
        orderDate: '订单日期',
        totalWeight: '总重量（吨）',
        totalAmount: '总金额',
        status: '状态',
        relatedPurchaseOrder: '关联采购订单',
        customerName: '客户名称',
        projectName: '项目名称',
        deliveryDate: '送货日期',
        outboundNo: '出库单号',
        relatedOrder: '关联订单',
        outboundDate: '出库日期',
        inboundNo: '入库单号',
        inboundDate: '入库日期',
        docNo: '单据号',
      },
      filter: {
        keyword: '关键字',
        keywordPlaceholder: '输入单据号或关键词',
      },
      summary: {
        supplierName: '供应商',
        buyerName: '采购员',
        orderDate: '订单日期',
        relatedOrder: '关联订单',
        inboundDate: '入库日期',
        customerName: '客户名称',
        projectName: '项目名称',
        deliveryDate: '送货日期',
        outboundDate: '出库日期',
        carrierName: '物流商',
        relatedOutbound: '关联出库单',
        billTime: '单据日期',
      },
    },
    editorWorkspace: {
      currentUserFallback: '当前用户',
      autoGeneratedPlaceholder: '系统自动生成',
      masterDataCodeGenerationFailed:
        '基础资料编码生成失败，请关闭后重新打开新建页面',
    },
    validation: {
      maxImportExceeded: '第{{row}}行可关联数量不能超过{{max}}件',
      weighRequired:
        '第{{row}}行商品类别需按过磅入库，请将本行结算方式改为过磅',
      weighWeightRequired: '请填写第{{row}}行过磅重量',
      lockedSalesWeightExceeded:
        '第{{row}}行过磅重量小于已锁定销售重量{{locked}}吨，请修改过磅重量，或先反审核销售出库/销售订单后再保存',
      purchaseInboundSourceRequired: '第{{row}}行缺少来源采购订单明细',
      purchaseInboundWarehouseRequired: '第{{row}}行缺少来源仓库ID',
      purchaseOrderQuantityMinimum: '第{{row}}行数量必须至少为1个数量单位',
      purchaseInboundQuantityPositive: '第{{row}}行入库数量必须大于0',
      salesOutboundQuantityMinimum: '第{{row}}行出库数量必须至少为1个数量单位',
      purchaseInboundMixedSource: '一张采购入库单只能关联一张采购订单',
      lineItemRequired: '请填写第{{row}}行{{label}}',
      fieldRequired: '请填写{{label}}',
      minOneItem: '请至少填写一条明细',
      parentRelationOccupied:
        '{{parentLabel}}{{parentNo}}已被{{occupiedNo}}关联',
      errorSummarySuffix: '等共{{count}}个问题',
    },
    columns: {
      materialCode: '商品编码',
      brand: '品牌',
      category: '类别',
      material: '材质',
      spec: '规格',
      length: '长度',
      unit: '单位',
      quantityUnit: '数量单位',
      quantity: '数量',
      pieceWeightTon: '件重/吨',
      piecesPerBundle: '每件支数',
      weightTon: '重量(吨)',
      unitPrice: '单价',
      amount: '金额',
      totalAmount: '总金额',
      totalWeight: '总重量',
      status: '状态',
      remark: '备注',
      settlementMode: '结算方式',
      warehouse: '仓库',
      warehouseName: '仓库名称',
      batchNo: '批号',
      materialName: '商品名称',
      orderNo: '单据编号',
      customerName: '客户名称',
      projectName: '项目名称',
      purchaseOrderNo: '采购订单号',
      salesOrderNo: '销售订单号',
      inboundNo: '入库单号',
      outboundNo: '出库单号',
      outboundRemainingQuantity: '剩余可出库数量',
      weighWeight: '过磅重量',
      weightAdjustmentAmount: '重量调整金额',
      weightAdjustmentTon: '重量调整(吨)',
      settlementCompany: '结算主体',
      account: '资金账户',
    },
    pages: {
      material: {
        materials: '商品资料',
        keyword: '关键字',
        materialPlaceholder: '商品编码 / 品牌 / 规格',
        category: '类别',
        material: '材质',
        materialCode: '商品编码',
        materialType: '商品类型',
        materialTypePhysical: '实体商品',
        materialTypeExpense: '附加费用',
        brand: '品牌',
        spec: '规格',
        length: '长度',
        unit: '单位',
        qtyUnit: '数量单位',
        pieceWeightTon: '件重/吨',
        pcsPerBundle: '每件支数',
        unitPrice: '单价',
        remark: '备注',
        materialCount: '商品数',
        calculated: '理算品类',
        weighed: '过磅品类',
        materialDesc:
          '根据既定商品字段维护主数据页面，支持按编码、品牌、规格和材质统一管理，便于直接对接真实商品接口。',
        downloadTemplate: '下载模板',
        templateDownloadFailed: '商品资料模板下载失败',
        importFailed: '商品资料导入失败',
        importSuccessSummary:
          '导入完成：总行数 {{totalRows}}，成功 {{successCount}}，新增 {{createdCount}}，更新 {{updatedCount}}，跳过 {{skippedCount}}，失败 {{failedCount}}',
        importPartialFailure:
          '导入完成，但有 {{failedCount}} 行失败，详情见导入结果',
        importResultTitle: '导入结果',
        importOutcomeCreated: '新增',
        importOutcomeUpdated: '更新',
        importOutcomeSkipped: '跳过',
        importOutcomeFailed: '失败',
        importFilterAll: '全部',
        importColumnRowNumber: '行号',
        importColumnOutcome: '结果',
        importColumnReason: '失败原因',
        versionHistoryAction: '版本历史',
        versionHistoryTitle: '商品版本历史',
        versionHistorySelectSingle: '请先选择单条商品',
        versionHistoryEmpty: '暂无版本历史',
        versionHistoryLoadFailed: '版本历史加载失败',
        versionHistoryColumnTime: '变更时间',
        versionHistoryColumnSource: '来源',
        versionHistoryColumnType: '类型',
        versionHistoryColumnBatch: '导入批次',
        versionHistoryColumnRemark: '备注',
        versionHistoryColumnDiff: '字段差异',
        versionHistoryNoChange: '无字段变更',
        versionHistoryFieldEmpty: '空',
        historySourceManual: '手工',
        historySourceImport: '导入',
        historySourceRollback: '回滚',
        historyTypeCreated: '新建',
        historyTypeUpdated: '更新',
        historyTypeDeleted: '删除',
        historyTypeRollback: '回滚',
        importPreviewTitle: '导入预览',
        importPreviewFailed: '导入预览失败',
        importPreviewSummary:
          '预览结果：共 {{totalRows}} 行，新建 {{createdCount}}，更新 {{updatedCount}}，跳过 {{skippedCount}}，失败 {{failedCount}}',
        importPreviewConfirm: '确认导入',
        importPreviewEmpty: '预览无可导入数据',
        importPreviewColumnChanges: '字段差异',
        importPreviewNoChange: '无变化',
        importBatchNo: '导入批次号',
        rollbackAction: '回滚该批次',
        rollbackConfirmTitle: '回滚导入批次',
        rollbackConfirmContent:
          '确定回滚批次 {{batchNo}}？该批次新建的商品将被软删除，更新的商品将还原为导入前内容。',
        rollbackSuccess: '批次回滚完成',
        rollbackFailed: '批次回滚失败',
        rollbackResultTitle: '回滚结果',
        rollbackTotal: '批次行数',
        rollbackCreatedRolledBack: '新建软删',
        rollbackUpdatedRestored: '更新还原',
        rollbackMissing: '缺失',
      },
      materialCategories: {
        materialCategory: '商品类别',
        categoryDesc: '管理商品类别字典，支持按编码和名称增删改查。',
        keyword: '关键字',
        categoryPlaceholder: '类别编码 / 类别名称',
        categoryCode: '类别编码',
        categoryName: '类别名称',
        sortOrder: '排序',
        purchaseWeigh: '采购过磅',
        status: '状态',
        remark: '备注',
        required: '需要',
        notRequired: '不需要',
        categoryCount: '类别数',
        enabled: '启用',
      },
      payment: {
        payment: '付款单',
        createPayment: '新增付款单',
        businessType: '业务类型',
        paymentPurpose: '付款用途',
        statementSettlement: '对账结算',
        purchasePrepayment: '采购预付款',
        supplier: '供应商',
        carrier: '物流商',
        draft: '草稿',
        paid: '已付款',
        paymentDate: '付款日期',
        paymentNo: '付款单号',
        counterparty: '往来单位',
        counterpartyCode: '往来单位编码',
        payType: '付款方式',
        amount: '金额',
        status: '状态',
        relatedStatement: '关联对账单',
        sourcePurchaseOrderId: '来源采购订单ID',
        purchaseOrderNo: '采购订单号',
        supplierCode: '供应商编码',
        supplierName: '供应商名称',
        settlementCompanyId: '结算主体ID',
        settlementCompany: '结算主体',
        parentPurchaseOrder: '采购订单',
        selectPurchaseOrder: '选择采购订单',
        operator: '经办人',
        remark: '备注',
        bankTransfer: '银行转账',
        bankAcceptance: '银行承兑',
        cash: '现金',
        paymentDesc:
          '付款单支持对账结算与采购预付款；采购预付款从采购订单导入权威供应商和结算快照。',
      },
      receipt: {
        receipt: '收款单',
        createReceipt: '新增收款单',
        draft: '草稿',
        received: '已收款',
        receiptDate: '收款日期',
        receiptNo: '收款单号',
        customer: '客户',
        customerCode: '客户编码',
        project: '项目',
        receiptMethod: '收款方式',
        amount: '金额',
        status: '状态',
        relatedStatement: '关联对账单',
        operator: '经办人',
        remark: '备注',
        bankTransfer: '银行转账',
        bankAcceptance: '银行承兑',
        acceptanceBill: '承兑汇票',
        cash: '现金',
        receiptDesc:
          '收款单用于承接客户回款，已收款必须关联并完整核销客户对账单。',
      },
      customerStatement: {
        customerStatement: '客户对账单',
        generateStatement: '生成对账单',
        viewCustomerSummary: '查看客户对账汇总',
        period: '账期',
        statementNo: '对账单号',
        customer: '客户',
        customerCode: '客户编码',
        project: '项目',
        startDate: '开始日期',
        endDate: '结束日期',
        salesAmount: '销售金额',
        receiptAmount: '收款金额',
        closingBalance: '期末余额',
        direction: '方向',
        blueDirection: '蓝字',
        redDirection: '红字',
        sourceSalesReturnNo: '来源退货单',
        status: '状态',
        remark: '备注',
        relatedSalesOrders: '关联销售订单',
        parentImportLabel: '销售订单',
        parentImportButton: '选择销售订单生成明细',
        validationSelectCustomerFirst: '请先选择客户，再选择销售订单',
        validationSameCustomer: '只能选择同一客户的销售订单生成客户对账单',
        validationSameProject: '只能选择同一项目的销售订单生成客户对账单',
        customerStatementDesc:
          '客户对账单按已完成销售的销售订单明细口径组织，生成后保留来源订单行，便于逐项核对销售数量、单价和金额。',
      },
      freightStatement: {
        freightStatement: '物流对账单',
        generateFreightStatement: '生成物流对账单',
        viewFreightSummary: '查看运费对账汇总',
        pendingAudit: '待审核',
        audited: '已审核',
        unsigned: '未签署',
        signed: '已签署',
        period: '账期',
        statementNo: '对账单号',
        carrier: '物流商',
        carrierCode: '物流商编码',
        startDate: '开始日期',
        endDate: '结束日期',
        totalWeight: '总重量（吨）',
        totalFreight: '总运费',
        paidAmount: '已付金额',
        unpaidAmount: '未付金额',
        auditStatus: '审核状态',
        signStatus: '签署状态',
        attachment: '附件',
        remark: '备注',
        freightStatementDesc:
          '物流对账单和运费对账汇总拆开处理，这里展示正式物流对账单列表，用于审核、签署、附件和付款累计。',
      },
      freightOperations: {
        freightBill: '物流单',
        createFreightBill: '新增物流单',
        freightBillPlaceholder: '输入物流单号',
        unaudited: '未审核',
        audited: '已审核',
        documentDate: '单据日期',
        freightBillNo: '物流单号',
        carrier: '物流商',
        vehiclePlate: '车号',
        totalWeight: '总重量（吨）',
        unitPrice: '单价',
        totalFreight: '总运费',
        customerName: '客户名称',
        projectName: '项目名称',
        auditStatus: '审核状态',
        remark: '备注',
        freightBillDesc:
          '物流单独立维护物流商、客户、项目、运输明细，并继续流向物流对账。',
      },
      carrier: {
        title: '物流方资料',
        description:
          '物流方资料管理承运主体信息，包括联系方式、车辆信息和计价模式，支持多车辆录入。',
        placeholderKeyword: '物流商编码 / 名称 / 联系人',
        colCarrierCode: '物流商编码',
        colCarrierName: '物流商名称',
        colContactName: '联系人',
        colContactPhone: '联系电话',
        colVehicleType: '车辆类型',
        colPriceMode: '计价模式',
        colDefaultSettlementCompany: '默认结算主体',
        colDefaultSettlementCompanyId: '默认结算主体ID',
        colVehiclePlate: '车牌号',
        colVehicleContact: '驾驶员',
        colVehiclePhone: '驾驶员电话',
        colVehicleRemark: '备注',
        colVehiclePlate2: '车牌号 2',
        colVehicleContact2: '驾驶员 2',
        colVehiclePhone2: '驾驶员电话 2',
        colVehicleRemark2: '备注 2',
        colVehiclePlate3: '车牌号 3',
        colVehicleContact3: '驾驶员 3',
        colVehiclePhone3: '驾驶员电话 3',
        colVehicleRemark3: '备注 3',
        formVehiclePlate: '车牌号',
        formVehicleContact: '驾驶员',
        formVehiclePhone: '驾驶员电话',
        formVehicleRemark: '备注',
        formVehiclePlate2: '车牌号 2',
        formVehicleContact2: '驾驶员 2',
        formVehiclePhone2: '驾驶员电话 2',
        formVehicleRemark2: '备注 2',
        formVehiclePlate3: '车牌号 3',
        formVehicleContact3: '驾驶员 3',
        formVehiclePhone3: '驾驶员电话 3',
        formVehicleRemark3: '备注 3',
      },
      customer: {
        title: '客户资料',
        description: '统一管理客户主数据及其关联项目、联系方式和结算模式。',
        placeholderKeyword: '客户编码 / 名称 / 项目名称',
        colCustomerCode: '客户编码',
        colCustomerName: '客户名称',
        colProjectName: '关联项目',
        colProjectNameAbbr: '项目简称',
        colProjectAddress: '项目地址',
        colContactName: '联系人',
        colContactPhone: '联系电话',
        colCity: '城市',
        colSettlementMode: '结算方式',
        colDefaultSettlementCompany: '默认结算主体',
        colDefaultSettlementCompanyId: '默认结算主体ID',
        manageProjects: '管理关联项目',
        settlementCash: '现结',
        settlementMonthly: '月结',
        settlementCredit: '授信',
      },
      project: {
        title: '项目资料',
        description: '独立管理项目资料、所属客户和项目结算主体。',
        placeholderKeyword: '项目编码 / 名称 / 简称 / 客户编码',
        projectCode: '项目编码',
        projectName: '项目名称',
        projectNameAbbr: '项目简称',
        customer: '客户',
        settlementCompany: '结算主体',
        projectManager: '项目负责人',
        projectAddress: '项目地址',
        priceFloatMode: '网价浮动',
        priceFloatAdd: '加价',
        priceFloatSubtract: '减价',
        priceFloatValue: '浮动幅度（元/吨）',
        priceFloatValuePlaceholder: '如 30',
        quoteSource: '取价数据源',
        quoteSourceMysteel: 'Mysteel(网价)',
        quoteSourceSteelx: '西本新干线',
        quoteRegion: '取价地区',
      },
      purchaseOrder: {
        title: '采购订单',
        description:
          '采购订单按供应商、订单日期和商品明细管理，并自动汇总重量和金额。',
        placeholderOrderNo: '输入采购订单号',
        filterOrderDate: '订单日期',
        colOrderNo: '订单编号',
        colSupplier: '供应商',
        colSettlementCompany: '结算主体',
        colSettlementCompanyId: '采购结算主体ID',
        colBuyer: '采购员',
        colOrderDate: '订单日期',
        referencedBySalesOrder: '已被销售订单引用',
        referencedByPurchaseInbound: '已被采购入库引用',
        formTotalWeight: '总重量（吨）',
        formTotalAmount: '总金额',
      },
      salesOrder: {
        title: '销售订单',
        description:
          '销售订单围绕客户、项目、交货日期和商品明细管理，支持从采购订单导入并自动汇总重量和金额。',
        placeholderOrderNo: '输入销售订单号',
        placeholderProductKeyword: '商品编码 / 品牌 / 规格',
        placeholderPurchaseOrderNo: '系统自动生成',
        filterProductKeyword: '商品关键字',
        filterDeliveryDate: '交货日期',
        filterProjectName: '项目名称',
        colPurchaseOrderNo: '采购订单号',
        colOrderNo: '订单编号',
        colCustomerName: '客户名称',
        colProjectName: '项目名称',
        colSalesName: '销售员',
        colDeliveryDate: '交货日期',
        referencedByFreightBill: '已被物流单引用',
        referencedBySalesOutbound: '已被销售出库引用',
        referencedBySalesReturn: '已被销售退货单引用',
        colDeliveredQuantity: '已交付数量',
        colReturnedQuantity: '已退货数量',
        colDeliveredNetQuantity: '净交付数量',
        documentFlow: {
          action: '单据流',
          title: '单据流：{{no}}',
          empty: '暂无单据流数据',
          emptySection: '暂无单据',
          relationsTitle: '单据关系',
          relationsEmpty: '暂无单据关系',
          loadFailed: '单据流加载失败',
          selectSingle: '请选择一条销售订单查看单据流',
          relationArrow: '至',
        },
        parentImportLabel: '采购订单',
        parentImportButton: '导入采购订单明细',
        originalPlannedWeight: '原始计划 {{weight}} 吨',
        netPriceFillAll: '整单取网价',
        netPriceTitle: '按网价赋单价',
        netPriceHint:
          '按送货日期与时段匹配网价，叠加项目浮动的加价/减价后填入单价；无网价的行保持原值，请手工填写。',
        netPriceDate: '网价日期',
        netPricePeriod: '时段',
        netPricePeriodAll: '全部时段',
        netPriceApply: '取价并填入',
        netPricePickDate: '请选择网价日期',
        netPriceRule: '价格规定',
        netPriceRulePick: '请选择价格规定',
        netPricePickRule: '项目配置了多条价格规定，请先选择一条',
        netPriceFilled: '已按网价填入 {{count}} 行单价',
        netPriceNoPrice: '{{items}} 等 {{count}} 行无网价，请手工填写',
        netPriceFailed: '取网价失败，请稍后重试',
        netPriceFloatHint: '项目约定：网价{{mode}} {{value}} 元/吨',
      },
      salesContract: {
        title: '销售合同',
        description:
          '维护销售合同并按项目核定销售订单的金额与吨位额度，超限时在保存前提示。',
        placeholderKeyword: '合同编号 / 合同名称 / 客户 / 项目',
      },
      purchaseInbound: {
        title: '采购入库',
        description:
          '采购入库管理采购到货和入库确认，支持从采购订单导入并自动汇总重量和金额。',
        placeholderInboundNo: '输入入库单号',
        placeholderParentImport: '输入采购订单号',
        filterInboundDate: '入库日期',
        colInboundNo: '入库单号',
        colPurchaseOrderNo: '采购订单号',
        colSupplier: '供应商',
        colInboundDate: '入库日期',
        colTotalWeighWeight: '过磅重量',
        colWeightAdjustment: '重量调整',
        formOrderNo: '入库单号',
        formInboundDate: '入库日期',
        formBuyer: '采购员',
        formTotalWeight: '总重量（吨）',
        formTotalAmount: '总金额',
        parentImportLabel: '采购订单',
        parentImportButton: '导入采购订单明细',
        validationSameSupplier:
          '来源采购订单存在不同供应商，不能合并生成采购入库单',
        validationSameSettlementCompany:
          '来源采购订单存在不同结算主体，不能合并生成采购入库单',
        validationPurchaseOrderNoTooLong:
          '合并后的采购订单号长度不能超过 {{max}} 个字符',
      },
      salesOutbound: {
        title: '销售出库',
        description:
          '销售出库管理发货和出库确认，支持从销售订单导入并自动汇总重量和金额。',
        placeholderOutboundNo: '输入出库单号',
        placeholderParentImport: '输入销售订单号',
        placeholderProductKeyword: '商品编码 / 品牌 / 规格',
        filterOutboundDate: '出库日期',
        filterProductKeyword: '商品关键字',
        filterProjectName: '项目名称',
        colOutboundNo: '出库单号',
        colSalesOrderNo: '销售订单号',
        colCustomerName: '客户名称',
        colProjectName: '项目名称',
        colOutboundDate: '出库日期',
        parentImportLabel: '销售订单',
        parentImportButton: '导入销售订单明细',
      },
      salesReturn: {
        title: '销售退货单',
        description:
          '销售退货单是销售出库的反向单据，按原销售出库明细办理部分或整单退货，不修改原销售订单、销售出库与物流单。',
        placeholderReturnNo: '输入退货单号',
        placeholderSalesOrderNo: '从销售出库带入',
        filterReturnNo: '退货单号',
        filterReturnDate: '退货日期',
        filterProjectName: '项目名称',
        colReturnNo: '退货单号',
        colSalesOrderNo: '销售订单号',
        colCustomerName: '客户名称',
        colProjectName: '项目名称',
        colReturnDate: '退货日期',
        parentImportLabel: '销售出库',
        parentImportButton: '导入销售出库明细',
        sourceImport: {
          action: '从销售出库创建退货',
          title: '从已审核销售出库创建退货单',
          outboundPlaceholder: '选择已审核的销售出库单',
          returnDateLabel: '退货日期',
          noOutbound: '未找到已审核的销售出库单',
          confirm: '创建退货单',
          colMaterialCode: '商品编码',
          colBrand: '品牌',
          colMaterial: '材质',
          colSpec: '规格',
          colOutboundQuantity: '出库数量',
          colReturnedQuantity: '已退数量',
          colReturnableQuantity: '可退数量',
          colReturnQuantity: '本次退货数量',
          loadOutboundFailed: '销售出库列表加载失败',
          loadCandidatesFailed: '退货来源候选加载失败',
          noReturnableItems: '该出库单没有可退明细',
          quantityRequired: '请至少选择一行并填写大于 0 的退货数量',
          quantityAriaLabel: '第 {{index}} 行退货数量',
          quantityOutOfRange: '退货数量不能超过可退数量（最多 {{max}}）',
          createSuccess: '退货单已创建',
          createFailed: '退货单创建失败',
        },
      },
      supplier: {
        title: '供应商资料',
        description:
          '供应商资料管理供应商主数据，包括联系方式和结算信息，支持按供应商编码和名称统一管理。',
        placeholderKeyword: '供应商编码 / 名称 / 联系人',
        colSupplierCode: '供应商编码',
        colSupplierName: '供应商名称',
        colShortName: '供应商简称',
        colBrands: '经营品牌',
        colContactName: '联系人',
        colContactPhone: '联系电话',
        colCity: '城市',
        colSettlementMode: '结算方式',
        settlementCash: '现结',
        settlementMonthly: '月结',
        settlementCredit: '授信',
      },
      operationLog: {
        operationLog: '操作日志',
        export: '导出',
        allLogs: '全部日志',
        authentication: '身份认证',
        keyword: '关键字',
        operationLogPlaceholder: '日志编号 / 操作人 / 业务单号 / 请求路径',
        module: '模块',
        action: '动作',
        result: '结果',
        success: '成功',
        failed: '失败',
        authType: '认证方式',
        operationTime: '操作时间',
        logNo: '日志编号',
        operator: '操作人',
        loginName: '登录账号',
        businessNo: '业务单号',
        requestMethod: '请求方式',
        requestPath: '请求路径',
        clientIp: '客户端IP',
        remark: '备注',
        logCount: '日志数',
        successCount: '成功数',
        failedCount: '失败数',
        operationLogDesc:
          '操作日志用于审计整个系统的写操作，记录操作人、模块、动作、对象、请求路径和结果状态，便于问题追踪与合规检查。',
      },
      operationLogOptions: {
        query: '查询',
        view: '查看',
        create: '新增',
        edit: '编辑',
        delete: '删除',
        audit: '审核',
        reverseAudit: '反审核',
        statusChange: '状态变更',
        completePurchase: '完成采购',
        completeSales: '完成销售',
        deliveryVerification: '交付核定',
        returnToAudited: '退回已审核',
        export: '导出',
        print: '打印',
        login: '登录',
        loginFailed: '登录失败',
        logout: '退出登录',
        changePassword: '修改密码',
        execute: '执行',
        save: '保存',
        saveAndAudit: '保存并审核',
        saveAndConfirm: '保存并确认',
        uploadJson: '上传 JSON',
        exportBackup: '导出备份',
        importBackup: '导入备份',
        uploadAttachment: '上传附件',
        generateAttachmentUploadUrl: '生成附件直传地址',
        completeAttachmentDirectUpload: '完成附件直传',
        exportAttachmentRecoveryManifest: '导出附件恢复清单',
        common: '常用',
        authentication: '身份认证',
        currentAccount: '个人账号',
        database: '数据库管理',
        system: '系统',
        companyInfo: '结算主体管理',
        printTemplates: '打印模板',
        operationLog: '操作日志',
        printModule: '打印',
        attachmentManagement: '附件管理',
        business: '业务',
        materials: '商品资料',
        supplier: '供应商',
        customer: '客户',
        carrier: '物流商',
        warehouse: '仓库',
        purchaseOrder: '采购订单',
        purchaseInbound: '采购入库',
        salesOrder: '销售订单',
        salesOutbound: '销售出库',
        salesReturn: '销售退货单',
        freightBill: '物流单',
        customerStatement: '客户对账单',
        freightStatement: '物流对账单',
        receipt: '收款单',
        payment: '付款单',
      },
      systemCore: {
        keyword: '关键字',
        status: '状态',
        remark: '备注',
        companyInfo: '结算主体管理',
        companyPlaceholder: '结算主体名称 / 税号 / 银行 / 账号',
        companyName: '结算主体名称',
        taxNo: '税号',
        companyInfoDesc:
          '结算主体管理集中维护主体名称、税号与多个结算银行信息，供客户、物流商、采购订单和财务页面统一引用。',
      },
      masterWarehouse: {
        warehouseMaster: '仓库资料',
        keyword: '关键字',
        warehousePlaceholder: '仓库编码 / 名称 / 联系人',
        warehouseType: '仓库类型',
        ownWarehouse: '自营仓',
        partnerWarehouse: '合作仓',
        transitWarehouse: '中转仓',
        thirdPartyWarehouse: '第三方仓',
        warehouseCode: '仓库编码',
        warehouseName: '仓库名称',
        contact: '联系人',
        phone: '联系电话',
        address: '仓库地址',
        status: '状态',
        remark: '备注',
        warehouseDesc:
          '仓库主数据统一维护仓库编码、仓库名称、仓库类型、联系人和启用状态，供采购入库、销售出库和物流单页面复用。',
      },
    },
  },
  hooks: {
    batchActions: {
      pleaseSelectRecords: '请先选择记录',
      noBatchStatus: '当前模块未配置批量{{action}}状态',
      actionNotSupported: '勾选单据当前状态不支持{{action}}',
      batchAction: '批量{{action}}',
      batchActionConfirm:
        '确定对选中的 {{count}} 条记录执行{{action}}吗？{{skippedPart}}',
      actionFailed: '{{action}}失败',
      actionCompletedWithFailures:
        '{{action}}完成：成功 {{successCount}} 条，失败 {{failedCount}} 条{{skippedPart}}{{errorPart}}',
      actionSuccess: '{{action}}成功 {{successCount}} 条{{skippedPart}}',
      skippedPart: '另有 {{count}} 条因状态不支持将跳过。',
      skippedCount: '，跳过 {{count}} 条',
      deleteNotSupported: '勾选单据当前状态不支持删除',
      batchDelete: '批量删除',
      batchDeleteConfirm:
        '确定删除选中的 {{count}} 条记录吗？此操作不可恢复。{{skippedPart}}',
      deleteFailed: '删除失败',
      deleteCompletedWithFailures:
        '删除完成：成功 {{successCount}} 条，失败 {{failedCount}} 条{{skippedPart}}{{errorPart}}',
      deleteSuccess: '删除成功 {{successCount}} 条{{skippedPart}}',
    },
    printActions: {
      selectPrintTemplate: '选择打印模板',
      noPrintTemplateConfigured: '当前单据未配置打印模板',
      noPrintableRecords: '未找到可打印的选中记录',
      singleRecordOnly: '打印仅支持单据单打，请只选择一条单据',
      printScriptGenerationFailed: '打印脚本生成失败',
      printServiceUnavailable: '打印服务不可用，请检查 CLodop 或打印模板配置',
      noPrintContent: '未生成打印内容',
      printFailed: '打印失败',
      exportXlsxFailed: '套打 Excel 导出失败',
    },
    freightActions: {
      noFreightData: '当前列表暂无物流对账单数据',
      freightSummaryTitle: '运费对账汇总',
      documentCount: '当前列表单据数：{{count}}',
      totalWeight: '总重量（吨）：',
      totalFreight: '总运费：',
      paidAmount: '已付金额：',
      unpaidAmount: '未付金额：',
    },
    customerActions: {
      noCustomerData: '当前列表暂无客户对账单数据',
      customerSummaryTitle: '客户对账汇总',
      documentCount: '当前列表单据数：{{count}}',
      salesAmount: '销售金额：',
      receiptAmount: '收款金额：',
      closingAmount: '期末余额：',
    },
    customerProjectActions: {
      title: '{{name}}的关联项目',
      create: '新增项目',
      edit: '编辑项目',
      delete: '删除项目',
      saveSuccess: '项目保存成功',
      deleteSuccess: '项目删除成功',
      code: '项目编码',
      name: '项目名称',
      abbreviatedName: '项目简称',
      address: '项目地址',
      manager: '项目负责人',
      status: '状态',
      remark: '备注',
      operations: '操作',
      deleteConfirm: '确认删除该项目？',
      selectedCustomerRequired: '请选择一条客户资料',
    },
    toolbarActions: {
      delete: '删除',
      printPreview: '打印预览',
      directPrint: '直接打印',
      noExtraLogic: '{{label}} 当前没有额外处理逻辑。',
    },
    recordActions: {
      view: '查看',
      edit: '编辑',
      attachment: '附件',
      audit: '审核',
      reverseAudit: '反审核',
      reopenDeliveryVerification: '重新核定',
      confirmDeliveryVerification: '确认核定',
    },
    requestError: {
      requestFailed: '请求失败',
    },
    pagination: {
      total: '共 {{count}} 条',
    },
    displaySupport: {
      yes: '是',
      no: '否',
    },
    excelExport: {
      exportSuccess: '导出完成',
      exportFailed: '导出失败',
    },
    openPages: {
      unnamedPage: '未命名页面',
      workbench: '工作台',
    },
    gridColumns: {
      actions: '操作',
      detail: '查看明细',
    },
    columnSettings: {
      syncRetryLater: '列设置已保存到本地，云同步稍后重试',
    },
    statement: {
      noCandidateDocuments: '当前筛选条件下没有可生成的候选单据',
    },
    recordHelpers: {
      currentUser: '当前用户',
    },
  },
  layouts: {
    userMenu: {
      personalSettings: '个人设置',
      logout: '退出登录',
    },
    personalSettings: {
      title: '个人设置',
      displayTab: '外观',
      accountTab: '账户资料',
      passwordTab: '修改密码',
    },
    settings: {
      systemFont: '系统字体',
      systemFontDefault: '系统自带',
      fontSize: '字体大小',
      navLayout: '导航布局',
      themeMode: '主题模式',
      resetDefault: '恢复默认',
      saveDisplay: '保存显示设置',
      theme: {
        light: '亮色',
        dark: '暗色',
        system: '跟随系统',
      },
      layout: {
        sider: '左侧导航',
        top: '顶部导航',
        siderDesc: '保留侧边菜单，更适合高频表格录入和多模块切换。',
        topDesc: '采用顶部菜单栏，整体风格与当前 Vue 版保持一致。',
      },
    },
    sideNav: {
      root: '业务中心',
      apiOnline: 'API 正常',
      apiOffline: 'API 离线',
    },
    breadcrumb: {
      ariaLabel: '当前位置',
    },
    headerSearch: {
      placeholder: '搜索单号、合同号、对账单号',
      viewFlow: '查看单据流向',
    },
    userInfo: {
      notLoggedIn: '未登录',
      currentAccount: '当前账号',
    },
    routePage: {
      businessPageNotFound: '未找到对应业务页面',
    },
    tabs: {
      refresh: '刷新页面',
      close: '关闭标签页',
      closeOthers: '关闭其他',
      closeRight: '关闭右侧',
      closeAll: '全部关闭',
      pinDashboard: '工作台固定展示',
    },
  },
  system: {
    company: {
      title: '结算主体管理',
      loadFailed: '结算主体加载失败',
      saveSuccess: '结算主体信息已保存',
      enterpriseMode: '主体维护',
      singleEnterprise: '支持多结算主体',
      subjectStatus: '主体状态',
      settlementBanks: '结算银行',
      supplementNote: '补充说明',
      subjectList: '结算主体',
      addSubject: '新增主体',
      noSubjects: '暂无结算主体',
      deleteSubject: '删除结算主体',
      deleteSubjectConfirm:
        '确定删除该结算主体吗？已被客户、物流商或采购订单引用的主体不可删除。',
      deleteSubjectDirtyConfirm:
        '当前结算主体包含未保存的修改。删除后这些修改将丢失；已被业务数据引用的主体仍无法删除。确定继续吗？',
      activeSubjects: '正常主体',
      atLeastOneSettlementAccount: '结算账户为可选信息',
      inputAccountName: '请输入第 {{index}} 个结算账户的账户名称',
      inputBankName: '请输入第 {{index}} 个结算账户的开户银行',
      inputBankAccount: '请输入第 {{index}} 个结算账户的银行账号',
      duplicateBankAccount: '银行账号重复：{{account}}',
      countUnit: '{{count}} 个',
      settlementInfo: '结算信息',
      addBank: '新增银行',
      noSettlementAccounts: '未维护结算账户，可按需新增',
      accountName: '账户名称',
      accountNamePlaceholder: '如：基本户 / 收款户',
      usageType: '用途',
      usageGeneral: '通用',
      usageReceive: '收款',
      usagePay: '付款',
      bankName: '开户银行',
      bankNamePlaceholder: '输入开户银行',
      bankAccount: '银行账号',
      bankAccountPlaceholder: '输入银行账号',
      statusNormal: '正常',
      statusDisabled: '禁用',
      remarkPlaceholder: '补充账户用途或说明',
      subjectRemarkPlaceholder: '补充主体抬头、结算习惯或财务说明',
    },
    printTemplate: {
      title: '打印模板',
      loadFailed: '打印模板加载失败',
      description:
        '按单据类型和结算主体维护打印模板，支持预览、复制及文件同步。',
      newTemplate: '新建模板',
      editTemplate: '编辑模板',
      deleteTemplate: '删除打印模板',
      preview: '预览',
      copy: '复制',
      searchPlaceholder: '搜索模板名称、编码、类型、结算主体',
      emptyList: '暂无打印模板',
      templateContent: '模板内容',
      templateName: '模板名称',
      templateCode: '模板编码',
      billType: '单据类型',
      templateType: '模板类型',
      engine: '渲染引擎',
      assetRef: '资源引用',
      settlementCompany: '关联结算主体',
      unassignedCompany: '未关联结算主体',
      status: '状态',
      statusActive: '启用',
      statusDisabled: '停用',
      syncMode: '同步方式',
      syncModeManual: '手动维护',
      syncModeFile: '文件托管',
      sourceRef: '源文件',
      fileManagedEditHint: '文件托管模板请修改源文件后重启同步',
      uploadJson: '上传 JSON',
      uploadJsonSuccess: '模板 JSON 上传成功',
      uploadJsonFailed: '模板 JSON 上传失败',
      uploadPdfFormOnly: '仅 PDF 表单模板支持上传 JSON',
      uploadJsonFileOnly: '请上传 JSON 文件',
      uploadJsonSizeLimit: 'JSON 文件不能超过 1MB',
      uploadJsonHint:
        '文件托管模板由部署文件管理，不支持上传，请先复制为手动模板',
      deleteContent: '确定删除模板「{{name}}」吗？',
      inputTemplateContent: '请输入模板内容',
    },
    account: {
      title: '个人账号',
      profileSection: '账户资料',
      loginName: '登录账号',
      userName: '用户姓名',
      mobile: '手机号',
      remark: '备注',
      saveProfile: '保存资料',
      profileSaved: '资料已保存',
      profileSaveFailed: '保存资料失败',
      passwordSection: '修改密码',
      currentPassword: '当前密码',
      newPassword: '新密码',
      confirmPassword: '确认新密码',
      changePassword: '更新密码',
      passwordChanged: '密码已更新，请重新登录',
      passwordChangeFailed: '更新密码失败',
      passwordMismatch: '两次输入的新密码不一致',
      rolesSection: '角色',
      rolesHint:
        '角色决定账号可访问的功能范围，具体权限由管理员在角色管理中维护。',
      rolesLabel: '所属角色',
      rolesPlaceholder: '请选择角色',
      saveRoles: '保存角色',
      rolesSaved: '角色已保存',
      rolesSaveFailed: '保存角色失败',
    },
    role: {
      title: '角色管理',
      description: '维护角色及其权限，内置角色不可删除或修改编码。',
      loadFailed: '角色加载失败',
      searchPlaceholder: '搜索角色编码或名称',
      emptyList: '暂无角色',
      newRole: '新建角色',
      wizardStepBasic: '基本信息',
      wizardStepPermissions: '权限配置',
      wizardNext: '下一步',
      wizardPrevious: '上一步',
      cloneRole: '克隆角色',
      cloneNameSuffix: ' 副本',
      editRole: '编辑角色',
      deleteRole: '删除角色',
      deleteContent: '确定删除角色「{{name}}」吗？删除后不可恢复。',
      code: '角色编码',
      codePlaceholder: '请输入角色编码',
      name: '角色名称',
      namePlaceholder: '请输入角色名称',
      descriptionLabel: '描述',
      descriptionPlaceholder: '请输入角色描述',
      status: '状态',
      statusFilter: '按状态筛选',
      statusNormal: '正常',
      statusDisabled: '禁用',
      builtin: '内置',
      builtinHint: '内置角色不可删除',
      builtinCodeReadonly: '内置角色的编码不可修改',
      permissionCount: '权限数',
      userCount: '用户数',
      permissions: '权限矩阵',
      permissionMatrixTitle: '权限矩阵 - {{name}}',
      permissionsLoadFailed: '权限数据加载失败',
      permissionSearchPlaceholder: '搜索资源或权限，如「销售订单」「审核」',
      selectAllConfirmTitle: '授予全部权限？',
      selectAllConfirmContent:
        '将勾选权限目录中的全部权限（含删除、回滚、重建等破坏性操作）。确定继续？',
      permissionSelectedCount: '已选 {{count}} 项权限',
      selectAll: '全选',
      clearAll: '清空',
      wildcardNotice:
        '该角色拥有全局通配权限（*），属于超级管理员角色，请在权限目录中单独维护。',
      permissionsSaved: '权限已保存',
      permissionsSaveFailed: '保存权限失败',
      enable: '启用',
      disable: '禁用',
      enableConfirmTitle: '启用角色',
      disableConfirmTitle: '禁用角色',
      statusConfirmContent: '确定更改角色「{{name}}」的状态吗？',
      statusUpdated: '角色状态已更新',
      statusUpdateFailed: '更新角色状态失败',
      actions: {
        read: '查看',
        create: '新增',
        update: '编辑',
        delete: '删除',
        write: '写入',
        audit: '审核',
        unaudit: '反审核',
        complete: '完成',
        confirm: '确认',
        print: '打印',
        export: '导出',
        import: '导入',
        preview: '预览',
        backfill: '补数',
        rollback: '回滚',
        rebuild: '重建',
        sync: '同步',
      },
      resources: {
        'sales-orders': '销售订单',
        'sales-contracts': '销售合同',
        'sales-returns': '销售退货单',
        'sales-outbounds': '销售出库',
        materials: '商品资料',
        'material-imports': '商品导入',
        'import-batches': '导入批次',
        inventory: '库存',
        'customer-statements': '客户对账单',
        customers: '客户资料',
        suppliers: '供应商资料',
        warehouses: '仓库资料',
        carriers: '物流方资料',
        projects: '项目资料',
        'quote-sheets': '报单比价',
        'steel-quotes': '钢材行情',
        'purchase-orders': '采购订单',
        'purchase-inbounds': '采购入库',
        'freight-bills': '物流单',
        finance: '财务',
        receipts: '收款单',
        payments: '付款单',
        'ledger-adjustments': '台账调整单',
        attachments: '附件',
        'attachment-upload-sessions': '附件直传会话',
        'attachment-manifest-exports': '附件恢复清单导出',
        'freight-statements': '物流对账单',
        'material-categories': '商品类别',
        'material-exports': '商品导出',
        'steel-quote-syncs': '行情同步',
        'steel-quote-backfills': '行情补数',
        'steel-quote-calendars': '行情日历',
        'material-price-matches': '商品行情匹配',
        'cash-ledger': '资金流水',
        'print-templates': '打印模板',
        'print-exports': '打印导出',
        'print-previews': '打印预览',
        'company-settings': '结算主体管理',
        dashboard: '工作台',
        'operation-logs': '操作日志',
        'global-search': '全局搜索',
        meta: '元数据',
        'code-issuances': '编码发放',
        'user-accounts': '用户账号',
        roles: '角色管理',
        permissions: '权限目录',
        system: '系统',
      },
      fields: {
        amount: '金额',
        'unit-price': '单价',
        cost: '成本',
      },
    },
    companyHeader: {
      title: '结算主体管理',
      description:
        '本页维护结算主体的名称、税号、状态和结算银行信息；客户、物流商和采购订单可选择对应结算主体。',
    },
    userAccount: {
      title: '用户账号',
      description: '维护登录账号、启用状态与角色分配；密码由管理员重置。',
      loadFailed: '用户账号加载失败',
      searchPlaceholder: '搜索登录账号、姓名或手机号',
      emptyList: '暂无用户账号',
      newUser: '新建用户',
      wizardStepBasic: '基本信息',
      wizardStepRoles: '角色分配',
      wizardNext: '下一步',
      wizardPrevious: '上一步',
      effectivePermissions: '有效权限预览',
      effectivePermissionsEmpty: '未选择角色，暂无有效权限',
      effectivePermissionsWildcard: '包含全部权限（*）',
      editUser: '编辑用户',
      deleteUser: '删除用户',
      deleteContent:
        '确定删除用户「{{name}}」吗？删除后账号将被停用且不可恢复。',
      deleteSelfConfirm: '这是当前登录账号，后端会拒绝删除。确定继续尝试吗？',
      deleteSelfForbidden: '不能删除当前登录账号',
      loginName: '登录账号',
      loginNamePlaceholder: '请输入登录账号',
      userName: '用户姓名',
      userNamePlaceholder: '请输入用户姓名',
      password: '密码',
      passwordPlaceholder: '请输入密码',
      passwordStrength: '密码需至少8位且同时包含字母和数字',
      newPassword: '新密码',
      newPasswordPlaceholder: '请输入新密码',
      confirmPassword: '确认新密码',
      confirmPasswordPlaceholder: '请再次输入新密码',
      passwordMismatch: '两次输入的新密码不一致',
      mobile: '手机号',
      mobilePlaceholder: '请输入手机号',
      status: '状态',
      statusFilter: '按状态筛选',
      statusNormal: '正常',
      statusDisabled: '禁用',
      lastLoginDate: '最近登录',
      enable: '启用',
      disable: '停用',
      enableConfirmTitle: '启用用户',
      disableConfirmTitle: '停用用户',
      statusConfirmContent: '确定更改用户「{{name}}」的状态吗？',
      statusUpdated: '用户状态已更新',
      statusUpdateFailed: '更新用户状态失败',
      resetPassword: '重置密码',
      resetPasswordTitle: '重置密码 - {{name}}',
      resetPasswordHint: '重置后该账号需使用新密码重新登录。',
      passwordResetSuccess: '密码已重置',
      passwordResetFailed: '重置密码失败',
      assignRoles: '分配角色',
      assignRolesTitle: '分配角色 - {{name}}',
      rolesLabel: '所属角色',
      rolesPlaceholder: '请选择角色',
      rolesSaved: '角色已保存',
      rolesSaveFailed: '保存角色失败',
      rolesUnavailable: '无法加载角色列表或账号角色，可能缺少角色查看权限。',
    },
    companySubject: {
      sectionTitle: '主体资料',
      companyName: '结算主体名称',
      companyNamePlaceholder: '输入结算主体名称',
      taxNo: '税号',
      taxNoPlaceholder: '输入税号',
      status: '状态',
      statusNormal: '正常',
      statusDisabled: '禁用',
      pendingCompany: '结算主体待配置',
      pendingTaxNo: '税号待配置',
      settlementBanks: '结算银行',
      unitSuffix: '个',
    },
    printTemplateEditor: {
      editTitle: '编辑模板',
      createTitle: '新建模板',
      basicInfo: '基础信息',
      availableFields: '可用字段',
      billType: '单据类型',
      templateName: '模板名称',
      templateNamePlaceholder: '请输入模板名称',
      templateCode: '模板编码',
      templateCodePlaceholder: '留空时系统自动生成',
      templateType: '模板类型',
      templateTypeCoord: '坐标套打',
      templateTypePdfForm: 'PDF 表单',
      engine: '渲染引擎',
      engineLodop: 'Lodop 坐标套打',
      enginePdfForm: 'PDF 表单填充',
      assetRef: 'PDF 底版资源（可选）',
      assetRefPlaceholder: '纯 JSON 模板可留空',
      settlementCompany: '关联结算主体',
      settlementCompanyPlaceholder: '请选择结算主体',
      versionNo: '版本号',
      status: '状态',
      statusActive: '启用',
      statusDisabled: '停用',
      templateContent: '模板内容',
      templateContentPlaceholder: '请输入模板内容',
      templateHint:
        'COORD 使用套打脚本或通用布局 JSON；PDF_FORM 使用纯 JSON 生成 PDF，可选引用 PDF 底版资源。',
      helpTitle: '打印模板说明',
      unifiedPrintApi:
        '打印接口统一使用 v2.0 契约：GET /v2.0/print-templates 查询模板，PUT /v2.0/print-templates/{id}/content 上传模板 JSON，GET /v2.0/print-previews/items 获取打印明细，POST /v2.0/print-exports 生成打印输出。COORD 返回套打脚本和数据，PDF_FORM 返回 PDF 内容；前端只负责预览和打印。',
      syntaxHint:
        '字段使用 {{fieldName}}，明细使用 {{#each details}}...{{/each}}，条件块使用 {{#if fieldName}}...{{else}}...{{/if}}。',
      commonFields: '主表字段',
      detailFields: '明细字段',
      layoutFields: '后端生成字段',
    },
    printTemplatePreview: {
      title: '模板预览',
      billType: '单据类型：',
      emptyTemplate: '（空模板）',
    },
    printTemplateUtils: {
      copySuffix: '（副本）',
    },
  },
  print: {
    defaultTitle: '打印',
    clodopLicenseInjectFailed: 'CLodop 注册信息注入失败',
    clodopTemplatePrintFailed: 'CLodop 模板打印失败',
    clodopPrintFailed: 'CLodop 打印失败',
    printTimeLabel: '打印时间：',
  },
  error: {
    retry: '重试',
    accessDenied: '当前页面拒绝访问',
    serverBusy: '服务器繁忙，请稍后重试',
    networkError: '网络连接异常，请检查网络后重试',
    requestTimeout: '请求超时，请稍后重试',
    serverResponseError: '服务器响应异常，请稍后重试',
    unknownError: '发生了未知错误，请点击重试按钮',
    serverError: {
      title: '无法连接到服务器',
      subTitle: '服务器暂时不可用，请检查网络连接或稍后重试',
    },
  },
  emptyState: {
    noData: {
      title: '还没有任何数据',
      description: '当前列表暂无记录，创建第一条记录开始使用',
      createFirstRecord: '创建第一条记录',
    },
    noResult: {
      title: '没有匹配的记录',
      description: '当前筛选条件下未找到记录，可调整或清空筛选条件后重试',
      resetFilters: '清空筛选',
    },
    noPermission: {
      title: '暂无访问权限',
      description: '您没有查看此内容的权限',
      hint: '如需访问，请联系管理员开通相应权限',
    },
    error: {
      title: '数据加载失败',
      description: '加载数据时出现问题，请检查网络后重试',
      retry: '重试',
    },
  },
  documentFlow: {
    title: '单据流向',
    titleWithNo: '单据流向：{{no}}',
    documentCount: '共 {{count}} 张单据',
    loadFailed: '单据流向加载失败',
    empty: '该单号暂无可展示的流向',
    nodeHint: '点击节点打开对应单据',
    openDocument: '打开 {{no}}',
    amount: '金额',
    weight: '重量',
    types: {
      purchaseOrder: '采购单',
      purchaseInbound: '采购入库单',
      salesOrder: '销售单',
      salesOutbound: '销售出库单',
      salesReturn: '销售退货单',
      freightBill: '物流单',
    },
    linkTypes: {
      inbound: '入库',
      sales: '销售',
      outbound: '出库',
      returns: '退货',
      freight: '物流',
    },
  },
  priceCompare: {
    view: {
      title: '报单比价',
      watermark: '内部资料 · 报单比价',
      noBatch: '暂无批次',
      removeSheetConfirmTitle: '删除该批次？',
      removeSheetConfirmContent: '该批次的全部录入内容将一并删除',
      editingByMe: '你正在编辑该批次',
      editingByOther: '{{name}} 正在编辑该批次',
      editingByOtherUnknown: '他人正在编辑该批次',
      requestTakeover: '申请接管',
      takeoverConfirmTitle: '确认接管编辑？',
      takeoverConfirmContent:
        '接管将取得该批次编辑权；若对方仍在编辑会再次冲突，请等待其签出过期后重试。',
      takeoverFailed: '对方仍在编辑该批次，请稍后再试。',
    },
    picker: {
      project: '项目',
      addProjectBatch: '新增项目批次',
      batch: '批次',
      newBatch: '新批次',
      deleteBatch: '删除批次',
      keepOneBatch: '至少保留一个批次',
      deleteCurrentBatch: '删除当前批次',
    },
    sheet: {
      dateFormat: 'YYYY年M月D日',
      dragRow: '拖动调整行顺序',
      dragBrand: '拖动调整品牌列顺序',
      columns: {
        remark: '备注',
        brand: '品牌',
        category: '类别',
        variety: '材质 / 规格 / 长度',
        ton: '报单吨位',
        net: '网价',
        spot: '现货',
        diff: '差价',
        supplierShort: '简称',
        purchaseOrderNo: '采购订单号',
        purchaseOrderSupplier: '供应商',
        purchaseOrderOrdered: '订货吨位',
        purchaseOrderIssued: '已开吨位',
        purchaseOrderRemaining: '剩余吨位',
        purchaseOrderStatus: '状态',
      },
      tonTotal: '合计',
      columnSettings: '列显示',
      selectProduct: '选择商品',
      switchLength: '切换为{{length}}',
      lockRow: '锁定该行',
      unlockRow: '解锁该行',
      lockRowHint: '锁定后可关联采购订单；解锁会清除已关联的采购订单',
      purchaseOrderLockFirst: '请先锁定该行再关联采购订单',
      tonPositive: '报单吨位需为正数',
      spotOutOfRange: '现货价超出合理范围（0 - {{max}}）',
      spotBetter: '现货更划算',
      netBetter: '网价更优',
      addRow: '＋ 添加一行',
      addSeparator: '＋ 添加隔断',
      separator: '隔断',
      removeSelectedTitle: '删除选中的 {{selected}} 行？',
      unspecifiedProject: '未指定项目',
      orderDate: '报单日期',
      remark: '备注信息',
      lock: '已锁定，点击解锁后可编辑',
      unlock: '解锁',
      clear: '清除',
      refPrice: '参照网价',
      refDateTooltip: '全部行统一使用该日期与时段作为网价基准',
      fallbackLegend: 'HRB400E 参考价',
      fallbackTooltip: '该品牌此规格暂无 HRB400 网价，当前按 HRB400E 参考价',
      bestDiff: '差价最优',
      refreshPrice: '刷新价格',
      config: '配置',
      lockRef: '锁定参照',
      unlockRef: '解锁参照',
      lockRefTooltip: '锁定参照日期/时段，避免刷新或误改；网价仍随刷新更新',
      unlockRefTooltip: '解锁后可修改参照日期/时段',
      lockSpecQuantity: '锁定规格和数量',
      unlockSpecQuantity: '解锁规格和数量',
      lockSpecQuantityTooltip:
        '锁定商品规格与数量输入，避免误改；与锁定参照互不影响',
      unlockSpecQuantityTooltip: '解锁后可修改商品规格与数量',
      specQuantityLockedHint:
        '已锁定规格和数量，暂不可新增、删除或拖动行；解锁后可继续编辑',
      supplier: '供应商',
      fillSupplier: '批量填入供应商',
      fillSupplierPick: '选择供应商',
      fillSupplierBrand: '选择品牌列',
      fillSupplierColumn: '把所选供应商填入「{{brand}}」列全部商品行',
      fillSupplierSelected: '把所选供应商填入选中的 {{count}} 行',
      fillSupplierNoChangedRows:
        '所选范围内没有本次改过现货价的行，未做任何修改（仅更新价格变动过的行）',
      purchaseOrderLabel: '关联采购订单',
      purchaseOrderPlaceholder: '关联采购',
      purchaseOrderOptionTitle:
        '{{orderNo}} · {{supplier}}（{{status}}）｜订货 {{ordered}} 吨｜剩余 {{remaining}} 吨',
      purchaseOrderTonnageHint: '已开 {{issued}} / 剩 {{remaining}}',
      purchaseOrderOverLimit: '（超额）',
      purchaseOrderSavedBasis: '按已保存报单统计，未保存的改动不计入',
      purchaseOrderMissing: '{{orderNo}}（订单已删除）',
      purchaseOrderMissingHint: '关联订单已删除，请重新选择',
      purchaseOrderLoadFailed:
        '采购订单吨位加载失败，已开/剩余吨位暂不可用；不影响吨位填写与保存',
      purchaseOrderIssuedShort: '已开 {{issued}}',
      purchaseOrderDetail: '采购订单明细',
      purchaseOrderUnlinked: '未关联采购订单',
      purchaseOrderOverLimitLong: '报单吨位已超过订单剩余可开吨',
      purchaseOrderPickerOpen: '选择采购订单',
      purchaseOrderPickerTitle: '选择采购订单规格行',
      purchaseOrderPickerSearch: '搜索单号、供应商或规格',
      purchaseOrderPickerEmpty: '暂无可选采购订单',
      purchaseOrderPickerClear: '清除关联',
      purchaseOrderPickerClearHint: '解除本行与采购订单的关联',
      purchaseOrderPickerHint:
        '点击一行即选中并关联；已开吨位按已保存报单统计。',
    },
    config: {
      title: '项目配置',
      tabs: {
        basic: '基础',
        brands: '品牌与品种',
        products: '可选商品',
      },
      premiumLabel: '12米加价（元/吨，仅螺纹钢生效）',
      fallbackLabel: 'HRB400 无网价时参考 HRB400E 价（表格中以 E 标记）',
      brandsTitle: '参与品牌',
      brandsHint: '参与比价的品牌与品种（未启用的品种该品牌不显示价格）',
      designatedBrands: '指定品牌',
      designatedBrandsHint: '指定品牌（仅用于报单展示，不参与自动过滤）',
      designatedBadge: '指定',
      selectedCount: '已选 {{selected}}/{{total}}',
      selectAll: '全选',
      clear: '清空',
      selectedBrands: '已参与品牌（{{selected}}）',
      noBrandSelected: '尚未选择品牌',
      freight: '运费(元/吨)',
      category: '品种',
      unselectedBrands: '未参与品牌（点击加入）',
      searchBrand: '搜索品牌',
      none: '无',
      productsTitle: '可选商品',
      productsHint: '报单时可选的商品（默认全选；取消勾选即从下拉中移除）',
      material: '材质',
      all: '全部',
      searchSpec: '搜索规格',
      noMatchedProduct: '无匹配商品',
      discardTitle: '放弃未保存的修改？',
      abandon: '放弃',
      continueEditing: '继续编辑',
    },
    pricing: {
      loginRequired: '请先登录后再读取网价',
      refreshedLatest: '已读取最新网价',
      refreshedWithRef: '已读取 {{ref}} 网价',
      refreshFailed: '读取网价失败：{{message}}',
      retryLater: '请稍后重试',
    },
    tour: {
      selectTitle: '选择项目/批次',
      selectDesc: '在顶部切换项目与批次；每个批次对应一次报价',
      entryTitle: '录入与比价',
      entryDesc: '支持拖动排序行；录入现货价，按回车或方向键切换行',
    },
  },
  pages: {
    material: '商品资料',
    'material-categories': '商品类别',
    supplier: '供应商资料',
    customer: '客户资料',
    project: '项目资料',
    carrier: '物流方资料',
    warehouse: '仓库资料',
    'company-setting': '结算主体管理',
    'customer-statement': '客户对账单',
    'freight-statement': '物流对账单',
    'finance-overview': '财务概览',
    receipt: '收款单',
    payment: '付款单',
    'cash-ledger': '资金流水',
    'price-compare': '报单比价',
    'market-sync': '行情同步',
    'purchase-order': '采购订单',
    'purchase-inbound': '采购入库',
    'sales-order': '销售订单',
    'sales-contract': '销售合同',
    'sales-outbound': '销售出库',
    'sales-return': '销售退货单',
    inventory: '库存查询',
    'freight-bill': '物流单',
    'print-template': '打印模板',
    'operation-log': '操作日志',
    'role-management': '角色管理',
    'user-accounts': '用户账号',
    account: '个人账号',
    dashboard: '工作台',
  },
  moduleFilters: {
    商品信息: '商品信息',
    订单编号: '订单编号',
    供应商名称: '供应商名称',
    结算主体: '结算主体',
    单据状态: '单据状态',
    订单日期: '订单日期',
    入库单号: '入库单号',
    入库日期: '入库日期',
    送货日期: '送货日期',
    客户名称: '客户名称',
    项目名称: '项目名称',
    物流商: '物流商',
    对账期间: '对账期间',
    出库单号: '出库单号',
    出库日期: '出库日期',
    退货单号: '退货单号',
    退货日期: '退货日期',
    物流单号: '物流单号',
    单据日期: '单据日期',
    方向: '方向',
    审核状态: '审核状态',
  },
  marketSync: {
    title: '行情同步',
    desc: '近 30 天覆盖监控；缺时段高亮；点击单元格查看该时段明细',
    dataSource: '数据源',
    singleDay: '单日',
    backfill: '补数',
    days: '天',
    backfillMissing: '补缺失',
    region: '地区',
    allPeriods: '全部时段',
    backfillMissingHint: '只补最近30天缺失的文章(已入库的会跳过)',
    sync: '同步',
    syncMissing: '缺',
    weekend: '休',
    matrixTitle: '覆盖矩阵（近30天）',
    matrixDate: '日期',
    matrixNoData: '该时段无行情数据，点击同步该时段',
    matrixRows: '{{rows}}行',
    detailTitle: '行情明细',
    notSelected: '未选择',
    up: '涨',
    down: '跌',
    search: '查询',
    reset: '重置',
    export: '导出',
    goCompare: '去比价',
    goCompareHint: '打开报单比价并应用该日期/时段',
    total: '共 {{total}} 条',
    selectPeriodHint: '请选择日期时段',
    columnFactory: '品牌/钢厂',
    columnBreed: '品名',
    columnMaterial: '材质',
    columnSpec: '规格',
    columnPrice: '价格(元/吨)',
    columnChange: '涨跌',
    columnRemark: '备注',
    period: '时段',
    coverage: '近30天覆盖 {{covered}}/{{weekdays}} 天',
    missingSlots: '缺失时段 {{count}}',
    backfilling: '补数中 {{from}} ~ {{to}}',
    backfillDone:
      '上次补数 {{from}} ~ {{to}}：成功{{synced}}天/失败{{failed}}天/{{rows}}行',
    todayPeriod: '今日{{period}} {{mark}}',
    backfillProgress: '{{done}}/{{total}} 天',
    backfillFailures: '补数失败 {{count}} 天：',
    syncDone: '同步完成：{{date}} {{periods}}，{{rows}} 行{{exists}}',
    syncDoneExists: '（已存在）',
    syncMissingPeriods: '当天暂无以下时段行情：{{periods}}',
    syncFailed: '同步失败',
    backfillAccepted:
      '已受理补数：{{from}} ~ {{to}}（后台执行，完成后自动刷新）',
    backfillFailed: '补数失败',
    retryLater: '请稍后重试',
    syncedRows: '已同步 {{date}} {{periods}}，{{rows}} 行',
    exportFileName: '行情明细_{{date}}_{{period}}.csv',
    exported: '已导出 {{rows}} 行',
    exportFailed: '导出失败，请稍后重试',
  },
  financeDetail: {
    loadOverviewFailed: '加载财务概览失败',
    counterpartyTypeShort: '往来类型',
    ledgerDetail: '对账明细',
    counterpartyCustomer: '客户',
    counterpartySupplier: '供应商',
    counterpartyCarrier: '物流商',
    flowReceipt: '收款',
    flowPayment: '付款',
    flowPaymentReversal: '付款冲销',
    flowReceiptReversal: '收款冲销',
    purposeCustomerStatementSettlement: '客户结算收款',
    purposeSupplierPrepaymentRefund: '供应商预付款退款',
    purposeSupplierOtherReceipt: '供应商其他收款',
    purposeStatementSettlement: '对账结算',
    purposePurchasePrepayment: '采购预付款',
    purposeSupplierPayment: '供应商总额付款',
    openingBalance: '期初余额',
    periodIncome: '期间收入',
    periodExpense: '期间支出',
    closingBalance: '期末余额',
    colBusinessDate: '业务日期',
    colFlowType: '流水类型',
    colDocumentNo: '单号',
    document: '单据',
    colCounterpartyType: '往来方类型',
    colCounterpartyName: '往来方名称',
    colPurpose: '用途',
    colIncome: '收入',
    colExpense: '支出',
    colRunningBalance: '累计余额',
    colOperator: '经办人',
    colRemark: '备注',
    settlementCompany: '结算主体',
    selectSettlementCompany: '请选择结算主体',
    keyword: '关键字',
    keywordPlaceholder: '单号、往来方、用途、经办人或备注',
    allCounterpartyTypes: '全部往来方类型',
    counterparty: '往来方',
    allCounterparties: '全部往来方',
    selectTypeFirst: '先选择类型',
    allFlowTypes: '全部流水类型',
    loadFailed: '加载资金流水失败',
    retryLater: '请稍后重试',
    empty: '暂无资金流水',
    total: '共 {{count}} 条',
    exported: '资金流水已导出',
    exportFailed: '导出资金流水失败',
    refreshFailed: '刷新资金流水失败',
    receivable: '应收',
    payable: '应付',
    received: '已收',
    unreceived: '未收',
    prepaid: '预收',
    paid: '已付',
    unpaid: '未付',
    prepayment: '预付',
    direction: '财务方向',
    balanceRange: '余额范围',
    all: '全部',
    hasBalance: '有余额',
    dueDate: '截止日期',
    counterpartyCode: '往来方编码',
    receivableYuan: '应收 (元)',
    payableYuan: '应付 (元)',
    receivedYuan: '已收 (元)',
    paidYuan: '已付 (元)',
    unreceivedYuan: '未收 (元)',
    unpaidYuan: '未付 (元)',
    prepaidYuan: '预收 (元)',
    prepaymentYuan: '预付 (元)',
    action: '操作',
    goReceipt: '去收款',
    goPayment: '去付款',
    counterpartyPlaceholder: '名称、拼音或编码',
  },
  receiptReconcile: {
    pendingStatements: '待核销客户对账单',
    unsettledPeriods: '{{name}} · 共 {{count}} 期未结',
    fifoAllocate: '按账期优先自动分配（FIFO）',
    clearAllocations: '清空分配',
    totalUnsettled: '合计（未结清）',
    overOutstanding: '超出未结余额 {{amount}}',
    overAllocatedHint:
      '本次分配核销总额 {{total}} 已超出实收金额 {{amount}}，请调整分配金额',
    customerOverview: '客户对账全景',
    settlementPreview: '动态核销试算',
    amountReceivedYuan: '实收金额（元）',
    rmbPrefix: '人民币',
    voucherAndRemark: '凭证与备注',
    uploadHint: '点击或拖拽上传银行电子回单（最多 5 份）',
    operator: '经办人',
    shortcutSubmit: '快捷键 {{keys}} 提交',
    saveDraft: '存为草稿',
    confirmReceipt: '确认收款并核销',
    flowTitle: '资金流水',
    ledgerEmpty: '暂无已审核资金流水',
    workbench: '收款核销工作台',
    draft: '草稿',
    detailTitle: '对账单明细',
    period: '对账周期',
    dueDate: '付款到期日',
    totalAmount: '对账总额',
    outstanding: '未结清待付',
    slipNo: '磅单号',
    productSpec: '品名规格',
    netWeightTon: '净重(t)',
    unitPrice: '单价',
    subtotal: '小计',
    deductionNote: '扣减说明',
    ourSettlementCompany: '我方结算主体',
    counterpartyType: '往来类型',
    counterpartyCustomer: '往来客户',
    receiptDate: '收款日期',
    receiptMethod: '收款方式',
    bankAccount: '入账银行账户',
    remark: '备注说明',
    remarkPlaceholder: '如：承兑汇票贴现、代付说明等',
    uploadEReceipt: '电子回单上传',
    prepaidDepositOption: '款项全额转预收定金',
    performanceDepositOption: '款项计入履约保证金',
    prepaidDepositDesc:
      '实收 {{amount}} 元将全额转入「{{name}}」的预收定金账户，后续可在出货时冲抵。',
    performanceDepositDesc:
      '实收 {{amount}} 元将作为「{{name}}」的履约保证金暂存，待合作期满或合同约定条件满足后处理。',
    creditTerm: '合作账期规则',
    reconciledUnsettled: '已对账未结清总额',
    overdueAmount: '其中逾期金额',
    inTransitUnreconciled: '在途送货未对账',
    receivedAmount: '实收金额',
    offsetStatementTotal: '冲抵对账单总计',
    overpaymentToPrepaid: '多收差额（转预收）',
    remainingDebt: '核销后客户剩余欠款',
    uploadedEReceipts: '已上传电子回单 {{count}} 份',
    suggestUploadEReceipt: '建议上传银行电子回单作为核销凭证',
    statementNo: '对账单号',
    status: '状态',
    offsetThisTime: '本次冲抵核销',
    overAllocated: '本次分配核销总额 {{total}} 已超出实收金额 {{amount}}',
    writeOffStatement: '核销客户对账单',
    saveAsPrepaid: '直接存为预收定金',
    performanceDeposit: '履约保证金',
    missingCounterparty: '缺少往来方信息',
    ledgerTitle: '{{name}} · 对账明细',
    ledgerDetail: '对账明细',
    loadLedgerFailed: '加载对账明细失败',
    date: '日期',
    flowType: '流水类型',
    documentNo: '单号',
    customer: '客户',
    document: '单据',
    income: '收入',
    expense: '支出',
  },
  documentReference: {
    copyAllDocs: '复制全部单号',
    copyDoc: '复制单号',
    copy: '复制{{no}}',
    copyAll: '复制全部',
    copiedAll: '已复制全部单号',
    copiedOne: '已复制单号',
    copyFailed: '复制失败，请手动选择单号',
    related: '关联{{label}}',
    amount: '金额：{{amount}}',
  },
  priceCompareStore: {
    saveSheetHeaderFailed: '保存比价单表头失败',
    deleteItemFailed: '删除商品行失败',
    saveItemFailed: '保存商品行失败',
    scopeConfig: '项目配置',
    scopeItem: '某商品行',
    scopeHeader: '表头',
    conflictContent:
      '服务器上的内容已被其他设备更新，请选择处理方式。重新加载将丢弃本地改动，以我的覆盖将用当前内容覆盖服务器。',
    conflictOverwrite: '以我的覆盖',
    conflictReload: '重新加载（丢弃我的改动）',
    conflictRetry: '仍在被修改，请稍后重试',
    saveSheetFailed: '保存比价单失败',
    saveConfigFailed: '保存比价配置失败',
    serverUpdated: '服务器有更新，保存后请刷新',
    conflictTitle: '单据版本已变更',
    lockConflict: '单据已被他人签出编辑，请稍后重试或申请接管',
    reloadSheetFailed: '重新加载比价单失败，请稍后重试',
    reloadConfigFailed: '重新加载比价配置失败，请稍后重试',
    loadSheetsFailed: '加载比价单失败，请稍后重试',
    deleteSheetFailed: '删除比价单失败，请稍后重试',
    takeoverTitle: '申请接管批次',
    takeoverContent:
      '接管将以当前用户强制取得该批次编辑权，对方正在进行的编辑将被覆盖，服务端会记录接管操作。确认接管？',
    takeoverOk: '强制接管',
    takeoverFailedShort: '接管失败，请稍后再试',
  },
  moduleSelector: {
    spaceEnterHint: 'Space 选择，Enter 选择',
  },
  moduleEditor: {
    pricePendingTitle: '价格待定提醒',
    pricePendingContent:
      '当前 {{count}} 条明细单价为 0，将以「待定价」状态保存。确认继续吗？',
    continueSave: '继续保存',
    backToEdit: '返回修改',
  },
  navigation: {
    master: '基础数据',
    market: '比价',
    purchase: '采购',
    sales: '销售',
    inventory: '库存',
    freight: '物流',
    statements: '对账',
    finance: '财务',
    system: '设置',
  },
}
