...
export const operationsPageConfigs: Record<string, ModulePageConfig> = {
  ...
  'sales-orders': {
    ...
    detailFields: [
      ...
      { label: '本次收款', key: 'receiptAmount', type: 'amount', row: 2 },
      { label: '本次欠款', key: 'closingAmount', type: 'amount', row: 2 },
      { label: '备注', key: 'remark', row: 3, fullRow: true },
    ],
    formFields: [
      { key: 'customerName', label: '客户', type: 'select', required: true, options: getCustomerOptions, row: 1 },
      { key: 'orderNo', label: '订单编号', type: 'input', required: true, row: 1 },
      { key: 'purchaseOrderNo', label: '关联采购订单', type: 'input', disabled: true, placeholder: '通过采购订单导入，可追加多个单号', row: 1 },
      { key: 'salesName', label: '销售员', type: 'input', required: true, row: 1 },
      { key: 'projectName', label: '项目名称', type: 'select', required: true, options: getCustomerProjectOptions, row: 2 },
      { key: 'deliveryDate', label: '送货日期', type: 'date', required: true, row: 2 },
      { key: 'receiptAmount', label: '本次收款', type: 'number', disabled: true, precision: 2, placeholder: '--', row: 2 },
      { key: 'closingAmount', label: '本次欠款', type: 'number', disabled: true, precision: 2, placeholder: '--', row: 2 },
      { key: 'remark', label: '备注', type: 'input', row: 3, fullRow: true },
    ],
    saveFields: {
      scalar: ['customerName', 'orderNo', 'purchaseOrderNo', 'salesName', 'projectName', 'deliveryDate', 'status', 'remark'],
    },
    parentImport: {
      parentModuleKey: 'purchase-orders',
      label: '上级采购订单',
  ...
  'freight-bills': {
    ...
    columns: [
      { title: '审核状态', dataIndex: 'status', width: 100, type: 'status', align: 'center' },
      ...
      { title: '送达状态', dataIndex: 'deliveryStatus', width: 100, type: 'status', align: 'center' },
    ],
    detailFields: [
