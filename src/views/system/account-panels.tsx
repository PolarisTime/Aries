...
/**
 * 账户资料面板：登录账号只读，姓名/手机号/备注可维护。
 * 供「个人账号」路由页与右上角「个人设置」弹窗共用，数据经 React Query 缓存共享。
 */
export function AccountProfilePanel() {
  ...
  return (
    <Form form={profileForm} layout="vertical" className="account-form">
      <Row gutter={[24, 0]}>
        <Col span={12}>
          <Form.Item label={t('system.account.loginName')}>
            <Input value={accountQuery.data?.loginName ?? ''} disabled />
          </Form.Item>
        </Col>
        <Col span={12}>
          <Form.Item
...
export function AccountPasswordPanel() {
  ...
  return (
    <Form form={passwordForm} layout="vertical" className="account-form">
      <Row gutter={[24, 0]}>
        <Col span={8}>
          <Form.Item
