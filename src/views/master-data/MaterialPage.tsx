...
/**
 * 商品表单字段定义。附加费用类仍需填写“名称”、单位与单价，
 * 物理属性字段（品牌/规格/长度/数量单位/件重/支数）与类别均隐藏，
 * 类别由后端固定写入“附加费用”。
 */
export function buildMaterialFormFields(
  t: (key: string) => string,
  materialTypeOptions: MasterOption[],
): MasterFormFieldSpec[] {
  return [
    {
      key: 'materialType',
...
export function MaterialPage() {
  ...
  const spec = useMemo<MasterDataPageSpec>(() => {
    ...
    return {
      ...
      formFields: buildMaterialFormFields(t, materialTypeOptions),
      buildValues: (record) => ({
