...
interface Props {
  ...
  disabled: boolean
  loading: boolean
  onPrint: (preview: boolean, template?: PrintTemplateRecord) => void
}
  ...
  const menuItems: MenuProps['items'] = (() => {
    if (!templates.length) {
      return [
        {
          key: 'no-template',
          label: t('modules.print.noTemplate'),
          disabled: true,
        },
      ]
    }
    return templates.flatMap((tpl) => [
      {
        ...
        children: [
          {
            key: `preview:${tpl.id}`,
            label: t('modules.print.preview'),
            icon: <PrinterOutlined />,
          },
          {
            key: `direct:${tpl.id}`,
            label: t('modules.print.directPrint'),
            icon: <PrinterOutlined />,
          },
        ],
      },
