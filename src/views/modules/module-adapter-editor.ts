...
function calculateAveragePieceWeightTon(quantity: unknown, weightTon: unknown) {
  ...
  if (!Number.isFinite(numericQuantity) || numericQuantity <= 0 || !Number.isFinite(numericWeightTon)) {
    return 0
  }
  return toRoundedNumber((numericWeightTon / numericQuantity) + Number.EPSILON, 3)
}

function calculateWeightByPieceWeightTon(quantity: unknown, pieceWeightTon: unknown) {
...
function resolveTheoreticalWeightTon(item: ModuleLineItem) {
  ...
  if (isWeighSettlement) {
    ...
    if (Number.isFinite(sourcePieceWeightTon) && sourcePieceWeightTon > 0) {
      return calculateWeightByPieceWeightTon(item.quantity, sourcePieceWeightTon)
    }
    ...
    if (
      ...
      return toRoundedNumber(Number(item.weightTon || 0) - weightAdjustmentTon, 3)
    }
  }
  return calculateWeightByPieceWeightTon(item.quantity, item.pieceWeightTon)
}

function resolveSourceInboundResidualWeightTon(item: ModuleLineItem, defaultWeightTon: number) {
  if (!item.sourceInboundItemId && !item.sourcePurchaseOrderItemId) {
    return defaultWeightTon
  }
  const sourceWeighWeightTon = Number(item._sourceWeighWeightTon ?? item._sourceTotalWeightTon ?? item._maxImportWeightTon)
  ...
  const sourcePieceWeightTon = Number(item._sourcePieceWeightTon || item.pieceWeightTon || 0)
  const allocatedQuantity = Math.max(sourceTotalQuantity - maxImportQuantity, 0)
  const allocatedWeightTon = calculateWeightByPieceWeightTon(allocatedQuantity, sourcePieceWeightTon)
  return Math.max(0, toRoundedNumber(sourceWeighWeightTon - allocatedWeightTon, 3))
}
...
export function isEditorItemColumnEditableForModule(
  ...
  if (moduleKey === 'purchase-inbounds' && columnKey === 'batchNo') {
    return false
  }
...
export function recalculateEditorLineItem(item: ModuleLineItem, changedKey?: string) {
  ...
  const isChangedValueCleared = changedValue === undefined || changedValue === null || changedValue === ''
  const hasWeighWeightTon = isWeighSettlement
    && item.weighWeightTon !== undefined
    && item.weighWeightTon !== null
    && item.weighWeightTon !== ''

  if (isWeighSettlement && ['weightTon', 'weighWeightTon'].includes(changedKeyString) && isChangedValueCleared) {
    item.weightTon = undefined
    item.weighWeightTon = undefined
  ...
  if (isWeighSettlement && hasWeighWeightTon && ['weighWeightTon', 'quantity', 'settlementMode'].includes(String(changedKey))) {
  ...
  if (changedKey === 'weightTon' && isWeighSettlement) {
    const hasWeightTon = item.weightTon !== undefined && item.weightTon !== null && item.weightTon !== ''
    if (hasWeightTon) {
      item.weightTon = toRoundedNumber(item.weightTon, 3)
      item.weighWeightTon = item.weightTon
      item.pieceWeightTon = calculateAveragePieceWeightTon(item.quantity, item.weightTon)
    } else {
      item.weighWeightTon = undefined
      item.pieceWeightTon = 0
    }
