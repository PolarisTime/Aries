...
describe.skip('module-display-switch-config', () => {
  it('keeps purchase inbound weigh fields while hiding all amount fields in weight-only mode', () => {
    const config = applyWeightOnlyViewConfig(
      'purchase-inbounds',
      operationsPageConfigs['purchase-inbounds'],
    )
    ...
    expect(config.itemColumns?.map((column) => column.dataIndex)).toContain(
      'settlementMode',
    )
    expect(config.itemColumns?.map((column) => column.dataIndex)).toContain(
      'weighWeightTon',
    ...
    expect(config.itemColumns?.map((column) => column.dataIndex)).not.toContain(
      'unitPrice',
    )
    expect(config.itemColumns?.map((column) => column.dataIndex)).not.toContain(
      'amount',
    ...
    expect(
      config.buildOverview([{ id: '1', totalWeight: 1.234, totalAmount: 999 }]),
    ).toEqual([
      { label: '记录数', value: '1' },
  ...
  it('hides sales outbound amount fields without disturbing other item columns', () => {
    const config = applyWeightOnlyViewConfig(
      'sales-outbounds',
      operationsPageConfigs['sales-outbounds'],
    )

    expect(config.columns.map((column) => column.dataIndex)).not.toContain(
      'totalAmount',
    )
  ...
  it('returns the original config for modules without a weight-only variant', () => {
    const config = applyWeightOnlyViewConfig(
      'sales-orders',
      operationsPageConfigs['sales-orders'],
    )
