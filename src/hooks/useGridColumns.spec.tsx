...
const { useModuleDisplaySupportMock, tMock } = vi.hoisted(() => ({
  useModuleDisplaySupportMock: vi.fn().mockReturnValue({
    formatCellValue: vi.fn((value: unknown, type?: string) => String(value)),
  }),
...
vi.mock('@/components/StatusTag', () => ({
  StatusTag: ({ status }: { status: string }) => `<StatusTag>${status}</StatusTag>`,
}))

vi.mock('@/components/TableActions', () => ({
  TableActions: ({ items }: { items: any[] }) => `<TableActions>${items.length}</TableActions>`,
}))

import { useGridColumns, ACTION_COLUMN_WIDTH } from './useGridColumns'
...
describe('useGridColumns', () => {
  ...
  it('includes actions column when canUpdate is true', () => {
    const { result } = renderHook(() => useGridColumns(defaultProps))
    const actionsColumn = result.current.columns.find((col) => col.id === 'actions')
  ...
  it('creates columns from config', () => {
    const { result } = renderHook(() => useGridColumns(defaultProps))
    const dataColumns = result.current.columns.filter((col) => col.id !== 'actions')
  ...
  it('uses custom render function when provided', () => {
    ...
    const config = {
      columns: [
        { dataIndex: 'name', title: 'Name', render: customRender },
      ],
      statusMap: {},
    }
    const { result } = renderHook(() =>
  ...
  it('renders status column with StatusTag', () => {
    ...
    const statusColumn = result.current.columns.find((col) => col.id === 'status')
  ...
  it('renders value as text for non-status column type', () => {
    ...
    const amountColumn = result.current.columns.find((col) => col.id === 'amount')
