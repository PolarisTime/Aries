...
vi.mock('@/module-system/module-adapter-actions', async (importOriginal) => {
  const original = await importOriginal<typeof import('@/module-system/module-adapter-actions')>()
  return {
    ...original,
    resolveModuleActionKind: (...args: Parameters<typeof original.resolveModuleActionKind>) =>
      resolveModuleActionKindMock(...args) ?? original.resolveModuleActionKind(...args),
  }
})
...
describe('useModuleToolbarActions', () => {
  ...
  it('disables delete when no rows selected', () => {
    ...
    expect(
      result.current.visibleToolbarActions[0]?.disabled,
    ).toBe(true)
  })
    ...
    const { result } = renderHook(() =>
      useModuleToolbarActions({
        ...
        config: {
          ...baseConfig,
          actions: [
            { key: 'create', label: '新增订单', type: 'primary' },
          ],
        },
        formFields: [],
        isMaterialModule: false,
  ...
  it('calls handlers for various action kinds', () => {
    const handlers = createHandlers()
    resolveModuleActionKindMock.mockReturnValue('openCustomerStatementGenerator')
    const { result } = renderHook(() =>
      useModuleToolbarActions({
        moduleKey: 'sales-order',
        config: {
  ...
  it('calls handler for supplier statement generator', () => {
    const handlers = createHandlers()
    resolveModuleActionKindMock.mockReturnValue('openSupplierStatementGenerator')
    const { result } = renderHook(() =>
      useModuleToolbarActions({
        moduleKey: 'sales-order',
        config: {
    ...
    const { result } = renderHook(() =>
      useModuleToolbarActions({
        ...
        config: {
          ...baseConfig,
          actions: [
            { key: 'pickup', label: '提货清单', type: 'default' },
          ],
        },
        formFields: [],
        isMaterialModule: false,
  ...
  it('calls handler for exportMaterialRows', () => {
    ...
    const { result } = renderHook(() =>
      useModuleToolbarActions({
        ...
        config: {
          ...baseConfig,
          actions: [{ key: 'export_material', label: '导出物料', type: 'default' }],
        },
        formFields: [],
  ...
  it('calls handler for markSelectedFreightDelivered', () => {
    ...
    const { result } = renderHook(() =>
      useModuleToolbarActions({
        ...
        config: {
          ...baseConfig,
          actions: [{ key: 'mark_delivered', label: '标记送达', type: 'default' }],
        },
        formFields: [],
        isMaterialModule: false,
        selectedRowCount: 1,
  ...
  it('canUseAction returns true when hasAnyModuleAction returns true', () => {
    ...
    expect(result.current.canUseAction({ key: 'create', label: '新增', type: 'primary' })).toBe(true)
  })
  ...
  it('canUseAction returns false when hasAnyModuleAction returns false', () => {
    ...
    expect(result.current.canUseAction({ key: 'create', label: '新增', type: 'primary' })).toBe(false)
  })
})
