...
describe('expandEachBlocks', () => {
  ...
  it('rejects an each block without a field name', () => {
    expect(() =>
      expandEachBlocks('{{#each}}<li>{{name}}</li>{{/each}}', []),
    ).toThrow('Missing print template each field')
    expect(() =>
      expandEachBlocks('{{#each }}<li>{{name}}</li>{{/each}}', []),
    ).toThrow('Missing print template each field')
  })

  it('warns when the each field name is unknown to the data context', () => {
    const warn = vi.spyOn(logger, 'warn').mockImplementation(() => {})
    const template = '{{#each detials}}<li>{{name}}</li>{{/each}}'
    // 字段名拼写错误时渲染继续，但必须给出明确告警，避免静默输出空内容
    const result = expandEachBlocks(template, [{ name: 'Item1' }])

    expect(result).toBe('<li>Item1</li>')
    expect(warn).toHaveBeenCalledWith(
      'Unknown print template each field: detials',
    )
  })

  it('does not warn when the each field exists in parent data or items', () => {
    const warn = vi.spyOn(logger, 'warn').mockImplementation(() => {})
    expandEachBlocks('{{#each details}}x{{/each}}', [{ details: 'row' }])
    expandEachBlocks('{{#each summary}}x{{/each}}', [], { summary: 'parent' })
