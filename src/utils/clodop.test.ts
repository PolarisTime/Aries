...
function setupLodop(lodop: LodopStub = makeLodop()) {
...
describe('execPrintCode', () => {
  afterEach(() => {
    ...
    Reflect.deleteProperty(globalThis, 'CLODOP')
    vi.restoreAllMocks()
  })
  ...
  it('handles code with PRINT_INITA', () => {
    ...
    const args = vi.mocked(lodop.PRINT_INITA).mock.calls[0]
  ...
  it('injects configured license once before printing', async () => {
    ...
    const { execPrintCode: execWithLicense } = await import('./clodop')

    expect(execWithLicense('LODOP.PRINT_INIT("test");')).toBe(true)
    expect(execWithLicense('LODOP.PRINT_INIT("test");')).toBe(true)
  ...
  it('logs warning when license injection fails', async () => {
    ...
    const { execPrintCode: execWithFailingLicense } = await import('./clodop')

    expect(execWithFailingLicense('LODOP.PRINT_INIT("test");')).toBe(true)
  ...
  it('returns false when getCLodop throws', async () => {
    ...
    const { execPrintCode: execWithThrowingGetter } = await import('./clodop')

    expect(execWithThrowingGetter('LODOP.PRINT_INIT("test");')).toBe(false)
  })
...
describe('loadCLodop', () => {
  ...
  it('resolves to true when CLODOP is available', async () => {
    vi.useFakeTimers()
    Reflect.set(globalThis, 'CLODOP', makeLodop())
    vi.resetModules()
  ...
  it('starts a fresh load attempt after a previous failure instead of caching it', async () => {
    vi.resetModules()
    const { loadCLodop } = await import('./clodop')
    const failed = loadCLodop()
    ...
    // 失败态不允许被永久缓存：下一次调用必须发起新的加载尝试
    const retried = loadCLodop()
